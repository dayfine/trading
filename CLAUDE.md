...
6. **Never nest `assert_that` inside matcher callbacks — use `field` or compose instead:**
   ```ocaml
   (* GOOD: declarative field extraction *)
   assert_that order_result
     (is_ok_and_holds (field (fun o -> o.status) (equal_to Filled)))

   (* GOOD: multiple fields with all_of + field *)
   assert_that order_result
     (is_ok_and_holds
       (all_of
          [
   ...
   (* AVOID: nested assert_that inside a callback — breaks declarative style *)
   assert_that order_result
     (is_ok_and_holds (fun order ->
       assert_that order.status (equal_to Filled)))
   ```

7. **Use `gt`/`ge`/`lt`/`le` with `Int_ord`/`Float_ord` instead of `assert_bool` for numeric comparisons:**
   ```ocaml
   (* GOOD: ordering matchers *)
   ...
   assert_bool "in range" Float.(price >= 100.0 && price <= 200.0)
   ```

8. **Use `matching` to assert a variant case without manual pattern matching:**
   ```ocaml
   (* GOOD: matching for variant extraction *)
   ...
   (* GOOD: when only the constructor presence matters, extract unit *)
   assert_that result.confirmation
     (matching (function Adequate _ -> Some () | _ -> None) (equal_to ()))
...
- Never nest `assert_that` inside `is_some_and`/`is_ok_and_holds`/`matching` callbacks — use `field` or `all_of [field ...]` instead
