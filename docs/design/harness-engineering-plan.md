...
Store per-function CC in `dev/metrics/cc-latest.json` — a single rolling
snapshot overwritten each deep-scan run; history lives in git log so the
tree doesn't accumulate one file per run.
...
**DONE (2026-04-16)**: T3-G deep scan Trends section implemented as Check 8 in
`trading/devtools/checks/deep_scan.sh`. Emits `## Trends` with two sub-sections:
(a) followup-item count per status file with delta vs. previous deep scan;
(b) CC distribution buckets (1-5 / 6-10 / >10) with delta vs. the prior
`dev/metrics/cc-latest.json` blob (read from `git HEAD` before overwrite),
plus top-5 highest-CC functions. The CC snapshot is generated by the
existing `cc_linter` binary on each deep scan run. Structural smoke test at
`trading/devtools/checks/deep_scan_trends_check.sh`, wired into `dune runtest`.
