...
(b) CC distribution buckets (1-5 / 6-10 / >10) with delta vs. previous
`dev/metrics/cc-*.json`, plus top-5 highest-CC functions. The CC snapshot is
generated by the existing `cc_linter` binary on each deep scan run. Structural
smoke test at `trading/devtools/checks/deep_scan_trends_check.sh`, wired into
`dune runtest`. First CC baseline written to `dev/metrics/cc-2026-04-16.json`.
