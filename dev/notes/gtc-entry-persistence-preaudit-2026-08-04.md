...
> Superseded in part by `dev/notes/bke-order-diagnosis-2026-08-05.md`: the
> engine-level persistence documented here is TRUE (and pinned by the
> companion tests) but moot for the BKE question — the deeper finding is that
> entry triggers sit at the current close (G14), so orders fill immediately
> and never rest at E. Read the diagnosis first.
