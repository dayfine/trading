...
3. **#2755 `deteriorating_blocks_longs` (feat-weinstein, local dispatch — fenced, do not GHA-dispatch):** default-off long-admission gate on `Macro.result.breadth_state = Deteriorating`, one read site at `weinstein_strategy_screening.ml:285`, inert unless `breadth_direction.enabled`. Rationale in the issue: Deteriorating entries are −$604k / n=80 on the record and nothing blocks them (gate blocks only `Bearish`; `neutral_blocks_longs` would also kill Recovering, the best cohort). Item 3 of 09-09 (per-state stop width) was the wrong instrument for the same observation. After merge: three salts on `_v10dedup` vs a0-v10, V6-gated, binary (realised P&L AND maxDD at ≥ 2 of 3 salts) — no per-state value search, cohorts are below the 230-trade floor. User decision 16:50 PT 09-09 to queue; dispatch from this session once item 1 frees disk.
4. **Re-measure the 09-05 cadence candidate (12%-weekly) against the v10 band** — `memory/project_stop_width_cadence_surface_2026_09_05` cleared 761% / DD 29.7 on the OLD basis and `_v7mark`; it is the next width lever and must run three salts on `_v10dedup` paired against a0-v10, V6-gated, with the open-MTM decomposition. If it does not clear the band on realised P&L AND maxDD at ≥ 2 of 3 salts, retire it the same way.
5. **Orchestrator summary push (D2, from the 09-09 daily summary #2744):** step 8 sets git identity, but jj reads its own (`jj config set --user user.name/user.email`) and `@` is never described (`jj describe`) — both independently fatal. Harness PR on `.github/workflows/` (full QC gates; needs an idle container).
6. Follow-ups carried: #2729 residuals; the `stop_loss` label hiding the `Per_position` breaker; `_v9gap` 2009 has no 5y spec (and is now superseded by `_v10dedup` 2009 — write the spec against that).
