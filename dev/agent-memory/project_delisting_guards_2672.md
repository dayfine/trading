...
**2026-09-08 00:46 PT — program code-complete.** #2705 retired runtime guard 3 (superseded by build-time `Series_tail`); #2708 (PR-B) added `Snapshot_pipeline.Series_splice` — drop interleaved series (≥20 findings), cut ticker-reuse / prefix-mis-scale at the LAST splice keeping the later segment (deep_bars included), one strict exceptions record `{keep_tail; splice}` parsed once (typos fatal), `splice_actions.csv`, `-no-splice-action`; detector still DEFAULT-OFF — arm it in the next rebuild; #2709 closed #2696 — `Delisted_ticket_cancel` retires resting entry tickets past the marker with `CancelEntry` reason `delisted` (fourth token, third category "data-driven death"; shared constant `Delisted_exit_runner.label`); V17 = 0 by construction. Remaining: rebuild with the detector armed → re-base again (salted); vintage gap fetch (2009: 968 / 2019: 792 missing names); the ~6pp maxDD dissection (window identical across warehouses to 15 digits → data-level).
