...
**2026-09-08 02:40 — gap FETCHED.** 1,531 names attempted (1,519 union + 12 of the 2000 remainder) → 1,359 fetched from EODHD, **1,247 of them dead at real death dates** (91.8%), 24 MISS (ticker reused by a new listing, or a <200-row real series like FHL), 148 synthetic `_old` twins skipped. Late-start check: 7 legit spring-2019 listings + **TMS = a 2025 listing under a 2019 name (347 rows beat the floor) → quarantined**. RULE: the >200-row floor is not a reuse detector; the vintage-date check (first bar ≤ vintage + ~90 d) is — run it BEFORE any rebuild. Rebuild `_v9gap` 2009/2019 warehouses next; until then the 2009/2019 `_v7mark` warehouses remain survivor-tilted (2,033 / 2,209 names). `dev/experiments/warehouse-rebuild-2026-09-06/fetch_gap.sh`.

**2026-09-08 04:17 — the tilt MEASURED (salt 0):** rec5y-2019 on `_v9gap` (2,936 names) = 21.67% / 181 / Sharpe 0.30 / DD 23.6, V16/V17 clean, vs 42.37% / 175 / 0.50 / 21.7 on `_v7mark` (2,209). Join: 111 shared trades earn MORE on v9gap; the gap is v7-only +$195k (BBWI, AN) vs v9-only −$8k (BBBY, ZS + losers); only 18 v9-only trades sit on the 727 widened names (−$50k). Mostly PATH LOTTERY from a bigger candidate pool + a direct −$50k dilution. Salts 1–2 on both warehouses running (chain4.log); quote a band, never the −20pp. CAVEAT: the s0 pair is NOT build-paired (v7 cell at #2695 tip b48537469, v9gap at main 077b48973 — #2705/#2708/#2709 differ); the salted chain IS paired (both from sweep-wh0908), so the band is the clean read. The 2019 5y cell in the 09-07 record is superseded by the `_v9gap` band.

**2026-09-08 06:05 — the BAND (paired salts 1–2, same build 077b48973):** `_v9gap` 2019 = 20.5–22.9% / 179–184 trades / Sharpe 0.30 / DD 23.5 (tight); `_v7mark` = 32.2–46.1% / Sharpe 0.41–0.53 / DD 21.7 (wide). Survivorship cost on 2019–23: −12 to −23pp, +2pp DD. DETERMINISTIC displacement, not lottery: BBWI/AN/NGLOY only ever screen in on the tilted pool, BBBY/ZS/FDX only on the widened one, at every salt; direct dilution from dead names only −$42–55k/salt (18–19 trades). The 09-07 2019 5y cell (42.37 / 175) is SUPERSEDED by the `_v9gap` band; every pre-09-08 "broad-5y cell B" read is above it for the same reason. `_v9gap` 2009 has no cell yet (no rec5y-2009 spec).

**Quality flag on the _v9gap cells:** `force_liquidations_count 1` at every salt = APLS 2023-07-17, a genuine gap on a safety-news day (prior close 84.50 → open 64.75 = −23% overnight, low 51.94, close 52.46 = −38% close-to-close; the stop at 74.90 was 13.5% above the open), Per_position breaker, −$50.8k; not data. Nit: trades.csv labels it `stop_loss` — the breaker is visible only in force_liquidations.sexp (follow-up: distinct exit_trigger token).
