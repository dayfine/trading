...
**Quality flag on the _v9gap cells:** `force_liquidations_count 1` at every salt = APLS 2023-07-17, a genuine −38% gap on a safety-news day (84.50 → open 64.75, low 51.94), Per_position breaker, −$50.8k; not data. Nit: trades.csv labels it `stop_loss` — the breaker is visible only in force_liquidations.sexp (follow-up: distinct exit_trigger token).
