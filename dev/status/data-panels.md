...
## Last updated: 2026-04-25 (PR-G pushed)

## Status
READY_FOR_REVIEW

Stage 0 MERGED as #555. Stage 1 MERGED as #557. Stage 2 foundation MERGED as #558. Stage 2 PR-B (Stage.classify reshape) MERGED as #559. Stage 2 PR-C (Rs.analyze reshape) MERGED as #560. Stage 2 PR-D (Stock_analysis.analyze reshape) MERGED as #561. Stage 2 PR-E (Sector.analyze reshape) MERGED as #562. Stage 2 PR-F (Macro.analyze reshape) MERGED as #563. Stage 2 PR-G (Weinstein_stops support floor reshape) on `feat/panels-stage02-pr-g-stops-support-floor` READY_FOR_REVIEW: adds `Weinstein_stops.compute_initial_stop_with_floor_with_callbacks ~callbacks` plus `Weinstein_stops.callbacks` bundle (re-export of new `Support_floor.callbacks`) wrapping five day-offset accessors `get_high`/`get_low`/`get_close`/`get_date`/`n_days`. The bar-list wrapper `compute_initial_stop_with_floor ~bars ~as_of` now builds the bundle via `callbacks_from_bars` and delegates. The bundle exposes a {pre-windowed} view: `as_of` filtering and `support_floor_lookback_bars` truncation are applied at construction time so the algorithm scans `[0..n_days-1]` without further bounds checks. Day offset 0 = newest bar in the window; the chronological "latest tie wins" rule for anchor identification maps to "smallest offset wins" in offset space (we walk offsets 0 → n_days-1 and take the first tying offset, comparing with strict `>` instead of `>=`). The post-anchor counter-move scan walks `[0..anchor_off-1]` (newer than the anchor). 10 new parity tests cover long-with-support-floor / no-floor-fallback, short-with-resistance-ceiling / no-ceiling-fallback, anchor-at-end (no post-anchor counter-move → fallback), tie-break-latest-peak / tie-break-latest-trough (mirrors `test_support_floor`'s tie scenarios; verifies the strict `>` rewrite returns the same anchor as `>=` in chrono space), lookback-truncates-old-correction (config override; exercises `callbacks_from_bars`'s lookback cap), and empty-bars (long+short); each runs both entry points and asserts bit-identical `stop_state` via structural `equal_to`. Net diff: 5 files, 287+/88-, plus a new 230-line parity test (10 tests).
