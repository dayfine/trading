...
- [x] **Calmar/Sortino as primary promote gate + CAGR-proxy floor** (branch `feat/promote-calmar-sortino-gate-2026-05-29`). Reframes `dev/scripts/promote_config.sh` per `dev/notes/layered-decomposition-synthesis-2026-05-29.md` § 4.6 + § 6 finding (per-symbol stage analysis loses absolute CAGR on 9/12 symbols but wins risk-adjusted on Calmar 6/12 + MaxDD reduced 12/12). The original Sharpe-Δ gate (2026-05-22) was the wrong yardstick for a defensive-tilt strategy. New primary gates: `PROMOTE_CALMAR_DELTA_MIN` (default -0.05, signed; positive to require improvement), `PROMOTE_SORTINO_DELTA_MIN` (default -0.05), `PROMOTE_CAGR_REGRESSION_MAX_PP` (default 2.0pp on `total_return_pct` as CAGR proxy). Sharpe + MaxDD + trades-ratio retained as secondary guardrails. Panel rows extended with cell_e_calmar + cell_e_sortino baselines pinned from each scenario's "Measured" headers (sp500-2010-2026: Calmar 0.52 / Sortino 1.25; sp500-2019-2023: Calmar 0.40 / Sortino 0.75). `validation.sexp` schema extended with the new gates + per-scenario calmar/sortino actual + delta rows. New smoke test `dev/scripts/test_extract_metrics.sh` (17/17 pass) pins Calmar+Sortino extraction, `regresses_by_more_than` semantics under both positive (regression-tolerance) and negative (require-improvement) thresholds, and a concrete dry-run that the 2026-05-29 laggard-OFF actual.sexp would correctly be REJECTED by the Sortino + CAGR-proxy gates against the existing Cell-E baseline. Verify: `bash dev/scripts/test_extract_metrics.sh` (17/17 pass) + `docker exec trading-1-dev bash -c 'cd /workspaces/trading-1/trading && eval $(opam env) && dune build && dune runtest'`.

- [x] **M1 T1.2 — `bayesian_runner.exe --fidelity-strategy successive_halving`** (branch `feat/tuning/m1-t1-2-successive-halving`; per `dev/plans/tuning-research-driven-program-v2-2026-05-25.md` Milestone M1). New module `Tuner_bin.Bayesian_runner_successive_halving` (`.ml` 240 LOC / `.mli` 148 LOC) orchestrates the BOHB-style multi-fidelity promotion pipeline: stage 0 (cheap tier) runs the full BO ask/tell loop via `Bayesian_runner_runner.run_and_write` on a single-tier walk-forward spec restricted to the cheap tier; stages 1+ re-evaluate the prior stage's top-K survivors on the next tier (no new BO sampling — candidates are fixed at the cheap-tier choices). Survivor count per stage = `ceil(prior * fraction)` with a minimum of 1 (plan defaults `[0.5; 0.5; 1.0]` matching cheap→medium 50%, medium→expensive 50% [= 25% of original], expensive→ambitious 100%). Final winner = highest-scoring survivor at the LAST tier. New CLI flag `--fidelity-strategy single|successive_halving` (default `single` preserves legacy behaviour bit-for-bit); `successive_halving` requires both `--walk-forward-spec` (with a `Tiered` window_spec — `Rolling`/`Explicit` are rejected) and `--baseline-aggregate`. Output layout under `out_dir`: per-tier `<tier-name>/bo_log.csv` + `convergence.md` + `bo_checkpoint.sexp` from the cheap-stage BO, `promotion_<tier-name>.csv` per tier, final `best.sexp` + `successive_halving_summary.md`. Pure-function helpers exposed for unit tests: `promote_top_n_by_score` (stable sort descending by score, take top n; empty for n≤0, full list for n≥size), `survivor_count` (validates fraction in (0.0, 1.0], raises `Invalid_argument` outside), `build_walk_forward_spec_for_tier` (substitutes the template's `window_spec` with a single-tier `Tiered` shape preserving start_date/end_date/train_days, leaving base_scenario/baseline_label/gate untouched). 20 unit tests in `test_bayesian_runner_successive_halving.ml`: 5 promote-top-n cases (highest-scores, n≥size, n=0, n<0, ties stable), 4 survivor_count cases (ceiling, minimum-1, fraction=1.0, invalid fractions raise), 2 spec-for-tier cases (single-tier isolation, template preservation), 9 orchestration cases using stub tier-aware evaluators (3 tiers cheap/medium/expensive with per-tier loss scaling 1×/2×/3×; pin per_tier length=3, survivor counts 10→5→3, evaluator-call counts 10/5/3 per tier proving NO re-sampling at higher tiers, summary+best.sexp written, per-tier promotion csvs written, best_params=last-tier-head, empty-tiers raises, custom promotion_fractions override defaults), 1 plan-defaults pin. All existing tuner tests still pass unchanged (default = `Single` mode bit-for-bit). T1.2 deliberately does NOT include T1.3 (paired-Δ scoring), T1.4 (calibration), or T1.5 (re-score historical checkpoints) — those are separate follow-up PRs. Verify: `docker exec trading-1-dev bash -c 'cd /workspaces/trading-1/trading && eval $(opam env) && dune build && dune runtest trading/backtest/tuner/ --force'` (215/215 pass; 20 new + 195 existing).
...
- [x] **M4 T4.3 — BAH SPY + BRK-A baseline aggregates for 1998-2026** (PR pending; branch `feat/m4-bah-baseline-1998-2026`). New OCaml exe `trading/trading/backtest/scripts/bah_baseline_aggregator/` (lib + bin + 3 tests) computes per-fold Buy-And-Hold metrics (total_return_pct / sharpe_ratio / max_drawdown_pct / calmar_ratio / cagr_pct / avg_holding_days) over the M4 T4.1 28-fold 1998-2026 fixture, aggregates via `Walk_forward.Walk_forward_report.compute`, and writes a `Walk_forward_types.aggregate.sexp` shaped exactly as the Bayesian production runner's `--baseline-aggregate` input expects (variant_label = walk-forward-spec's `baseline_label` = `cell-E` so `_lookup_stability` succeeds at runtime). Three checked-in baseline files under `dev/experiments/bayesian-production-sweep-2026-05-25/`: `baseline_aggregate_v7_spy.sexp` (SPY 1998-2025, mean Sharpe 0.89, mean total_return 10.9%/yr, mean max_dd 15.7%), `baseline_aggregate_v7_brka.sexp` (BRK-A 1998-2025, mean Sharpe 0.80, mean total_return 11.9%/yr, mean max_dd 17.0%), `baseline_aggregate_v7.sexp` (copy of SPY — the v7 launch script's primary baseline). All three round-trip via `aggregate_of_sexp`. Per plan `dev/plans/tuning-research-driven-program-v2-2026-05-25.md` §M4 T4.3. Unblocks the v7 1998-2026 + top-3000 production sweep. Verify: `docker exec trading-1-dev bash -c 'cd /workspaces/trading-1/trading && eval $(opam env) && dune runtest trading/backtest/scripts/bah_baseline_aggregator/'` (3/3 pass).
