...
- [ ] citation_defect: trading/trading/weinstein/stops/lib/support_floor.mli — line 4 cites `Weinstein, Ch. 6 §5.1 ("Initial Stop Placement")` for a block quoting BOTH a Long and a Short rule, but §5.1 of `docs/design/weinstein-book-reference.md` contains only the long rule ("Place below the significant support floor"); the short buy-stop rule is §6.3 ("Initial buy-stop: above prior rally peak"). Fix = split the citation (§5.1 long / §6.3 short). Orchestrator opened both sections and confirmed. Fourth citation defect in three days; the pattern is a section that *exists* but does not contain the claim, which defeats any linter that only checks the section resolves. (source: 2026-07-26 run 3 orchestrator verification; carried [medium] from run 2)
- [ ] test_gap: trading/trading/backtest/tuner/lib/bayesian_opt_cholesky.ml — `chol_with_nugget_escalation` (added by #2009 to fix the `LAPACKE: 9` GP non-PD failure) exhausts all 6 attempts and re-raises at line 35 on the orchestrator host: `Tuner.Bayesian_opt:11` and `:17` fail deterministically 3/3 on main. #2009 shipped with both QC gates APPROVED and green CI, so no test pins "the two previously-failing tests now pass" — the fix's own tests pin the retry mechanism only. Needs (a) a regression test asserting `:11`/`:17` pass, and (b) real root cause (jitter cap vs degenerate/NaN kernel inputs — unresolved). See dev/health/2026-07-26-fast-run3.md. (source: 2026-07-26 run 3 fast scan)
