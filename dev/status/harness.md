...
- **Pre-Work Setup boilerplate not enforced by `agent_compliance_check.sh`** (NEW 2026-05-04, follow-up to PR #839). Surfaced by qc-behavioral review of #839 (`dev/reviews/agent-prompts-jj-workspace-pr839-behavioral.md`). Today the compliance check (`trading/devtools/checks/agent_compliance_check.sh`) verifies feat-*.md files contain `## Acceptance Checklist`, `## Max-Iterations Policy`, and `## Allowed Tools` sections — but does NOT check for `## Pre-Work Setup`. A new feat-*/harness/ops-data agent definition could be added (or the section stripped) without the linter catching the regression, which would silently re-open the contamination class. **Fix:** extend `agent_compliance_check.sh` to require `## Pre-Work Setup` on every jj-writing agent (feat-*, harness-maintainer, ops-data) but NOT on read-only QC agents (qc-structural, qc-behavioral, health-scanner, track-pacer). One-rule extension; ~10 LOC. Verify via `dune runtest devtools/checks` — should FAIL on a deliberately-stripped agent file and PASS on the as-checked-in tree.

- **No automated smoke test for Pre-Work Setup runtime correctness** (NEW 2026-05-04, follow-up to PR #839). The boilerplate's bash was verified manually during the #839 dispatch (one-shot `jj workspace add` + `jj workspace list` + cleanup) but is not pinned by any test. If a future jj version changes the `jj workspace add` semantics (e.g. requires `-r <rev>` to be explicit, or changes `--name` syntax), agent dispatches would silently fail at runtime with no CI catch. **Fix:** add `trading/devtools/checks/jj_workspace_smoke.sh` that exec's the actual boilerplate against a tmp-dir target, asserts `jj workspace list` shows the new entry, and cleans up. Wire under `dune runtest`. ~30 LOC shell + dune rule. Skip on GHA (no jj installed in runner) — gate behind `command -v jj` or a `--skip-if-no-jj` flag. References: `dev/reviews/agent-prompts-jj-workspace-pr839-behavioral.md` §"Open follow-ups".
