...
- [~] **CI red-main watchdog** (issue #934, branch harness/ci-main-watchdog): `.github/workflows/ci-main-watchdog.yml` — triggers on `workflow_run` completion of `CI` workflow on `main`; on failure, auto-creates GitHub issue `[ci-watchdog] main CI red on <short-sha>` with run URL + first 20 lines of log + `cc @dayfine`; labels `ci-red`/`urgent`. Idempotent: comments on existing open watchdog issue instead of creating a duplicate. Manual test: `gh workflow run ci-main-watchdog.yml -F sha=<bad-sha>`. Closes #934.
