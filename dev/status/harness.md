...
- [~] **CI nondeterministic `LAPACKE: 9` — Bayesian_opt GP Cholesky non-positive-definite (residual tuner flake, DISTINCT from the #1961 SIGILL)** (NEW 2026-07-17, GHA orchestrator run 29618154628). **[~] 2026-07-19 (run 29683964833): dispatched a harness-maintainer for the adaptive-nugget-escalation fix; it produced a high-quality partial diff (`fit_gp` retries `Linalg.chol` with an escalating diagonal jitter only on the non-PD failure path — bit-identical on the success path; named constants, narrow exception match) but could NOT complete because in GHA the agent shares one git checkout + `_build` + dune lock with the orchestrator, and the orchestrator's concurrent Step-6 `dune runtest` held the lock — the agent stalled on `dune build @fmt` and never added the required guard test / verified / pushed. WIP diff preserved at `dev/notes/bayesian-gp-nugget-escalation-wip-2026-07-19.patch` (bayesian_opt.ml/.mli only; +61 lines → 355). Re-dispatch in ISOLATION next run (or maintainer LOCAL): `git apply` the patch, add the near-singular-kernel guard test in `test_bayesian_opt.ml`, run `dune build @fmt` + `dune runtest`, confirm file-length OK, push + PR. PROCESS FIX (also filed as escalation): the orchestrator must run its Step-6 build/runtest gate strictly AFTER dispatched build-agents finish, never concurrently, in the shared GHA checkout.] Symptom: `build-and-test` reds intermittently in `test_bayesian_opt.exe` with `Failure("LAPACKE: 9")` — LAPACK `potrf` (Cholesky) reporting the 9th leading minor of the GP kernel matrix is not positive-definite. Two OUnit errors: `Tuner.Bayesian_opt:17:BO converges on 2D Branin` + `Tuner.Bayesian_opt:11:suggest_next GP phase respects bounds` (`FAILED: Cases: 43 Tried: 43 Errors: 2 Failures: 0`). **NOT the #1961 SIGILL class**: that was `Command got signal ILL` from AVX-512 owl stubs, fixed by pinning `OWL_CFLAGS=-march=x86-64-v2` (confirmed active this run: `OWL_CFLAGS` env carries the portable flags). This is a residual *numerical* non-PD flake in the GP fit, not an ISA/SIGILL issue. **Confirmed flaky, not a regression**: occurred on `3a633f6d` (#1995, a docs-only orchestrator-summary commit — identical test code to green parent `ad0b29ab`); base rate = 4/4 prior main commits (`ad0b29ab`/`9cdc8055`/`36863498`/`093c75b9`) all `build-and-test=success`; local reproduction 0/6 (test passed 6/6 standalone) + full `dune runtest` exit 0 this run. Evidence: GHA run 29599087810 job 87946588479, step 7 `dune runtest`. **Recommended fix (no workflow PAT needed — test/lib code only)**: raise the GP jitter/nugget floor. `Bayesian_opt` adds `noise_variance · I` to the kernel before Cholesky (`bayesian_opt.mli:220`); either bump the test fixture's `noise_variance` above the current `~1e-6`, or add adaptive-nugget escalation in the fit path (retry Cholesky with a larger diagonal on LAPACKE non-PD). Best done LOCAL where the low-rate flake can be stress-reproduced (loop the exe under varied BLAS thread counts). Rerun-on-flake is the interim mitigation but the API token available to the GHA orchestrator lacks `actions:write` (403 on `rerun-failed-jobs`), so no auto-reroll from the orchestrator; a human `gh run rerun --failed` clears a spurious red.
