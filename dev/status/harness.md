...
- [ ] H-PREV-VERDICT-PIPEFAIL: `trading/devtools/checks/write_audit.sh` — the `prev_verdict` extraction (`grep -o '"overall_qc": *"[^"]*"' "$f" | head -1 | sed ...`) carries the same `set -euo pipefail` fragility that H-AUDIT-COLLISION's rework 2 just fixed eight lines above it, and is **unguarded**. Trigger: `cat > "$OUTPUT_FILE" <<ENDJSON` truncates before filling, so a run interrupted mid-write (SIGTERM on a cancelled CI job, or the ENOSPC `sweep-hygiene.md` documents as recurring) leaves a record carrying `recorded_at_ns` but no `overall_qc`. Seeded with one such record, the NEEDS_REWORK path exits 1 with empty output and writes nothing — one bad record permanently and silently disables audit writes for that feature on the escalation path. **Pre-existing, not a regression** (same unguarded line is on main at `fd41ed6a~1:write_audit.sh:155`); #2123 only marginally widened exposure, which is why it was held to follow-up rather than forced into a fourth iteration. **The fix is one line but the direction is a decision, not a patch:** an empty `prev_verdict` breaks the streak, which *under*-counts and suppresses the `>= 3` escalation — the unsafe direction. Decide deliberately whether an unparseable prior record breaks the streak or is skipped, then pin it with a truncated-record fixture on the NEEDS_REWORK path. Also note `|| true` swallows grep exit **2** (unreadable target) as well as no-match — smaller blast radius, same class, fix together. (source: 2026-07-27 run 4 qc-behavioral N4 on PR #2123)
- [ ] H-AUDIT-ATOMIC-WRITE: `trading/devtools/checks/write_audit.sh` — the record is written with `cat > "$OUTPUT_FILE" <<ENDJSON`, which truncates the target *before* the content is available, so any interruption leaves a partial or empty record. This is the root trigger for H-PREV-VERDICT-PIPEFAIL and would remove that failure at source. Fix: write to a temp file and `mv` into place (`cat > "$TMP" && mv "$TMP" "$OUT"`), making the write atomic on the same filesystem. (source: 2026-07-27 run 4 qc-behavioral N4 follow-up on PR #2123)
...
- [ ] H-FOLLOWUP-COUNT: `trading/devtools/checks/deep_scan/check_05_followup_items.sh` reports a follow-up count that is both wrong and self-contradictory. `dev/health/2026-07-27-deep.md` says `Follow-up accumulation: 76 open items across status files (threshold: 10)` in its Warnings while its own Trends section says `No open followup items in either scan.` Three distinct defects: (1) the counter counts **every** line beginning `- ` under a `## Follow-up*` / `## Followup*` heading — including completed `- [x]` entries and plain prose bullets — then labels the total "open"; an independent `- [ ]`-only count across all `dev/status/*.md` is **0**. (2) Its `case` patterns match only `## Follow-up*` / `## Followup*`, so the three status files using `### Follow-up` are invisible to it entirely. (3) The Check 05 -> Check 08 sidecar handoff yields an empty map, which is why Trends printed the "neither scan" message despite Check 05 having counted 76. Not cosmetic: this count is the input to the orchestrator's **Step 2b** maintenance-cycle decision (`followup_threshold: 10` in `dev/config/merge-policy.json`), so a spurious value can convert a feature slot into a maintenance pass against an empty backlog. Fix = count only `- [ ]`, match `###` headings too, and make the sidecar handoff verifiable. (source: 2026-07-27 run 5 orchestrator Step 6, `dev/health/2026-07-27-fast-run5.md`)
