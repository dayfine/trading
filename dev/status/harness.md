...
- [ ] H-QC-SCALE: `qc-structural` posted `## Quality Score` = **1** on PR #2115 while describing it in prose as "1 (Excellent)" — an inverted scale (1 is worst, 5 is best). `record_qc_audit.sh` takes the **last** score in the review file, so the behavioral 4 won and the audit record is correct by luck, not by construction. The agent definition should state the polarity explicitly, and the dispatch prompt now does.
