...
- [ ] H-RUNENV-WORKTREE-BLIND: `dev/lib/run-in-env.sh` — in `TRADING_IN_CONTAINER` (GHA) mode, `PROJECT_ROOT` is resolved from `${GITHUB_WORKSPACE}/trading` (line 59) rather than from the invoking directory, so any agent verifying a branch checked out into a `git worktree` (e.g. `/tmp/hm-<name>/trading`) via `run-in-env.sh dune build` / `dune runtest` silently builds and tests **`$GITHUB_WORKSPACE`'s main checkout**, not the worktree branch — a green result asserts nothing about the change under test. Contrast the non-GHA branch (line 65), which derives `PROJECT_ROOT` from the script's own location and therefore *does* follow a worktree correctly; only the GHA branch has this gap. Two agents independently hit this in orchestrator run 30300054307 on 2026-07-27 (one via a fix worktree, one via this harness item's own verification), each initially reporting a false-green result before the corrected direct-`dune`-in-worktree invocation was used. Fix direction: derive `PROJECT_ROOT` from the invoking worktree (e.g. `git rev-parse --show-toplevel` from cwd) with `$GITHUB_WORKSPACE` as a fallback only when cwd resolves to it, or fail loudly when cwd's toplevel differs from `$GITHUB_WORKSPACE`. Filed only — not fixed here (different file, different concern from this item's own fix).
