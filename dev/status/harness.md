...
- [x] **H-DAILY-SUMMARY-PR-LOST** (branch `harness/daily-summary-publisher`, 2026-09-08): **five consecutive GHA orchestrator runs lost their daily summary entirely** — the last summary landed on `main` via PR #2680 (`dev/daily/2026-09-05.md`, merged 2026-09-05); runs `34030886835` / `34042419476` (09-06), `34127853646` / `34148849699` (09-07), and `34224532158` (09-08) all completed with conclusion `success` (not no-op fast-exits — the 09-08 run alone cost `total_cost_usd = 20.17803125`, `dev/budget/2026-09-08-34224532158.json`), and zero `ops/daily-*` PRs exist for any of them (newest `ops/daily-*` head remains `ops/daily-2026-09-05`). Run `34224532158`'s own log proves the file WAS written and the workflow DID look for its PR: `Using daily summary: dev/daily/2026-09-08.md` followed by `No open ops/daily-2026-09-08 PR found (no-op run or lead-orchestrator failure).` — the summary existed on the runner and died with the ephemeral container, never pushed. Root-cause hypothesis, **corrected 2026-09-09 (issue #2741, measured in this same container image, run 34350513426)**: `jj git push` is NOT broken in this image — a probe branch pushed successfully with it. Step 8 as written hits two bugs of its own instead, neither one a jj/git version issue: (1) it sets **git's** identity (`git config user.email/user.name`) but jj does not read git's config, so jj sees an empty author and refuses to push; (2) it never runs `jj describe`, so `@` is descriptionless, which jj also refuses to push. Both are independently fatal — fixing only one still fails on the other; `jj config set --user user.name/user.email` + `jj describe` fixes both together. This is a *different* defect from `jst submit`'s failure recorded in H-JJ-JST-BROKEN-GHA below (that one is a real git-version incompatibility: image git 2.34.1 vs jj's `jj git fetch --porcelain` needing git >= 2.41.0) — the two must not be fixed with the same patch. Original (superseded) hypothesis, kept for the record: `.claude/agents/lead-orchestrator.md` Step 8 publishes via `jj bookmark set ...` + `jj git push -b ... --allow-new`, and `jj` is exactly the tool H-JJ-JST-BROKEN-GHA (below) already records as non-functional in the GHA orchestrator container image — the workflow's own sibling `git push origin ops/budget-...` step in the SAME run succeeded, proving plain git works fine from that container; only the jj-based path fails. **Fix shipped (script half only):** `dev/scripts/publish_daily_summary.sh` + `dev/scripts/publish_daily_summary_test.sh` (53 fixture-driven checks after the F2 correction below) is a plain-`git` + `curl`-REST publisher: resolves the newest `dev/daily/<date>*.md` (excluding `*-plan.md`), creates/reuses branch `ops/daily-<name>`, commits, pushes, and opens a PR via `POST /repos/$REPO/pulls`, idempotent on an already-open PR (422 fallback → look up by head, same shape as `orchestrator_fastexit_gate.sh`'s own PR-fallback), and fails loudly (non-zero exit, named stderr message) on every step that can silently drop the summary — no summary found, push rejected, PR-create returning no usable number. Wired into `dune runtest` via `trading/devtools/checks/publish_daily_summary_test_runner.sh` (same shim pattern as `prior_cell_check_test_runner.sh` / `prune_candidates_test_runner.sh`). The original 35-check suite shipped in PR #2721 was mutation-verified by hand during development but the mutation *list* lived only in a session report never committed to the repo; a qc-behavioral rework pass (2026-09-08, follow-up to #2721) independently ran 28 mutations, found 12 survivors — two critical (a `git reset --hard` before push that let the script report success while publishing nothing, and the production `git add`+`git commit` path being untested because every fixture pre-committed the summary) — and added the killing scenarios directly in `publish_daily_summary_test.sh`, with the reasoning for each recorded inline in that scenario's own comment rather than in an external report. **Wiring half CLOSED (corrected 2026-09-15, was falsely marked open):** this entry previously stated the wiring half remained open because `.claude/agents/lead-orchestrator.md` Step 8 still called `jj bookmark set` / `jj git push`. That was stale by 2026-09-15 — **#2757** (commit `68cd3b8c`, "Step 8 publishes via publish_daily_summary.sh, not jj") repointed Step 8 at this script; `main` today calls `sh dev/scripts/publish_daily_summary.sh publish --summary "$SUMMARY_FILE"` and explicitly forbids jj. Verified on `main` 2026-09-15 rather than re-inherited: `grep -n 'publish_daily_summary\|jj bookmark set' .claude/agents/lead-orchestrator.md` shows the script call, not jj. The residual failure class is NOT this item — it is the broader "green run, no durable artifact" class now tracked as H-FASTEXIT-VERIFY-FULL-MODE-BLIND (#2803, closed) and H-FASTEXIT-VERIFY-DISPATCH-ARTIFACTS (#2810, closed, see "Added 2026-09-15" below) — both mechanical `verify`-time checks, not wiring. Original entry (`dev/daily/2026-09-14-run2.md` §Escalations) flagged this exact staleness and declined to self-correct because two dispatched agents were writing this file concurrently at the time; corrected now that the conflict has cleared. Verify: `dev/lib/run-in-env.sh dune build && dev/lib/run-in-env.sh dune runtest` exit 0; `sh dev/scripts/publish_daily_summary_test.sh` directly prints `53/53 checks passed` (grew from 52 via the F2 correction below — see the "equivalent mutant" correction paragraph).
  ...
  **Follow-up (issue #2729, 2026-09-09): the "two disagreeing qc-behavioral passes" reconciled — not a contradiction, disjoint mutation sets.** #2721 got TWO GitHub PR reviews at the same tip SHA `6a1dfdcc`, 17:39:54Z (qc-structural, APPROVED 5) and 17:46:15Z (qc-behavioral, APPROVED 4) — the latter is "the interactive-session pass" the issue refers to. It ran three mutations (A: no-summary guard fail-open, B: drop the `-plan.md` exclusion, C: both no-PR-number guards removed) and, on CP3, asserted the push/422 scenarios "assert the branch really landed on the bare remote ( `git branch --list` ), not just an exit code" — a claim it reached by **reading** the assertion, not by mutating the git-push mechanics themselves. It never tested a content-losing mutation. Separately, the orchestrator dispatched its OWN qc-behavioral pass on the SAME SHA (`dev/reviews/harness.md`, NEEDS_REWORK quality 2) that ran a much broader 28-mutation sweep and specifically tried `git reset --hard` (to `$REMOTE/$BASE_BRANCH`) immediately before `git push` — exactly the content-losing mutation the interactive pass's CP3 claim rested on but never tried — and found it survives: prints `PR #<n> <url>`, exits 0, and the original 35-check suite reports 35/35 green while the pushed branch carries no summary. **Both passes are correct about what they actually tested; the interactive pass's CP3 PASS rested on an unverified assumption that a later, more adversarial mutation falsified.** This is not "both cannot be right about the same mutant" — they tested different mutants entirely, and only one of the two tried the one that mattered.
  ...
  Reproduced today (2026-09-09, unpiped exit codes) against the **current, already-fixed** script + suite on `main` (post-`a7775e3c`/#2727, which closed all 12 survivors including this one): (1) `git reset -q --hard "$REMOTE/$BASE_BRANCH"` inserted immediately before the `git push` in `cmd_publish` → `sh dev/scripts/publish_daily_summary_test.sh` reports **42/52, exit 1** (10 checks fail, led by "the pushed branch carries the summary CONTENT, not just the branch name") — the mutant does **NOT** survive on current `main`; the fix holds. (2) Re-ran mutation C for completeness. **`_create_pr` has THREE no-PR-number guards, not two**: the `if [ -z "$_num" ]; then ... return 1; fi` blocks in its 201 branch (line ~195) and its 422 branch (line ~207), plus an outer backstop in `cmd_publish` (`if [ -z "$_pr_num" ]; then ... return 1; fi`, line ~331) that independently re-checks the value `_create_pr` returned. **Exact recipe that reproduces the recorded number, verified 2026-09-09 by replaying it against this exact tree (PR #2749 rework, finding CP2-b): delete the two `_create_pr`-internal `if [ -z "$_num" ]; ...; fi` blocks outright** (201-branch and 422-branch — the whole `if`/`fi`, including its `echo`, removed, so control falls through to the `printf`/`return 0` below each), **leaving the line-331 backstop untouched** — matches #2721's original wording, "both no-PR-number guards removed." Replayed verbatim: `sh dev/scripts/publish_daily_summary_test.sh` reports **51/52, exit 1**, the single failure being `FAIL malformed-201 names the risk: expected output to contain no PR number in the response body` — the backstop still forces the correct exit code (`rc=1`), but the specific diagnostic wording from the deleted 201-guard is gone, so that one `check_contains` fails. Matches the interactive pass's original 34/35 finding; that part of its review was correct and remains correct today.

  **A prior draft of this entry described the recipe as "both no-PR-number guards → `return 0`" — replayed verbatim, that recipe does NOT reproduce 51/52.** Verified 2026-09-09 by making that exact edit instead (keep both `if`/`fi` blocks — and their `echo` lines — intact, change only `return 1` to `return 0` in each, leave line 331 untouched): `sh dev/scripts/publish_daily_summary_test.sh` reports **52/52, exit 0 — the mutant went fully undetected by the suite as it stood at that point.** Root cause of the discrepancy, confirmed against the log: with the `echo` left in place, the 201-guard still emits "no PR number in the response body" to stderr before returning — so `check_contains "malformed-201 names the risk"` still passes — and because that `echo` writes to fd 2, not stdout, `_create_pr`'s captured stdout (`_pr_out`) is still empty, so the line-331 backstop still independently fires and forces `rc=1` — so `check "... is treated as failure" 1 "$rc"` also still passes. The two recipes are not interchangeable, and only "guards removed" (delete the block) is the one that reproduces the 51/52 number this entry records.

  **Correction (PR #2749 rework iteration 2, qc-behavioral finding F2, 2026-09-09): "equivalent mutant" was the wrong label — the 52/52-green `return 0` mutation IS externally distinguishable, just not by any assertion the suite had at the time.** Production's push-then-create catch in `cmd_publish` (`_pr_out=$(_create_pr ...) || { ... echo "... branch $_branch is pushed but has NO PR. Do not treat the push alone as success." ...; return 1; }`, line ~324) fires only when `_create_pr` itself returns non-zero. Under the `return 1 → return 0` mutation, `_create_pr`'s 201-guard still emits its own "no PR number in the response body" message but then returns 0, so that `||` catch never runs and line 324's "pushed but has NO PR" message is never printed — only the separate line-331 backstop's "reported no PR number" message is. Production prints BOTH messages (the guard's own + the backstop's, since `_create_pr` genuinely fails there); the mutant prints only the backstop's. That is a real, checkable difference in stderr content, not merely internal redundancy — "defense-in-depth" was the wrong frame; the two guards protect two different observable claims (which layer failed), and the mutation silently collapses one of them into the other. Per `.claude/rules/mechanism-validation-rigor.md`'s spirit and this repo's own `s5`-row precedent (`pr_gate_status_mutation_test.sh`), a mutant only earns "equivalent" after verifying *no* assertion can ever distinguish it — that verification was never done here; a one-line `check_contains` closes the gap. Added to the malformed-201 scenario in `publish_daily_summary_test.sh` (52 → 53 checks): `check_contains "malformed-201 also hits the push-then-create catch (proves _create_pr itself returned non-zero, not just an empty PR number downstream)" "$_out" "pushed but has NO PR"`. Measured 2026-09-09, all three directions unpiped: **(1) unmutated: 53/53, exit 0. (2) `return 1 → return 0` mutation (both `_create_pr`-internal guards, echo lines intact, line 331 untouched): 52/53, exit 1 — the single failure is exactly the new assertion; the mutant is now a caught live survivor, not an equivalent mutant. (3) restored byte-identical to `git show HEAD:` and reverified: 53/53, exit 0.** The "guards removed" recipe two paragraphs up was also re-checked against the grown suite, since it drops the same line-324 message for the same reason (its `_create_pr` also returns 0 via the fallthrough `return 0` below the deleted guard, so the outer catch never fires there either): it now measures **51/53, exit 1 — two failures, not one** (the pre-existing "malformed-201 names the risk" wording failure, plus the new "pushed but has NO PR" failure) — not the 52/53 that a naive "suite grew by one, failure count unchanged" extrapolation would predict. Recorded as measured, not as predicted, per this rework's own instruction not to let a recorded number go stale silently. No new fixture was needed: the closing scenario already exists (`_remote_file_content` + the "pushed branch carries the summary CONTENT" checks in scenario groups 4/6/8, landed via #2727 on 2026-09-08); this fix only added one assertion to an existing scenario.

  Also corrected while in `dev/scripts/publish_daily_summary.sh`: its header asserted `jj` itself is non-functional in the GHA image (citing H-JJ-JST-BROKEN-GHA). That claim is measured false per issue #2741 (filed 2026-09-09, same container) — `jj git push` works once `jj config set --user user.name/user.email` and `jj describe` have run; Step 8's actual bug is that it configures GIT's identity (jj doesn't read it) and never sets a commit description, both independently fatal. Header rewritten to state the measured cause and cite #2741; the root-cause hypothesis above in this same entry was corrected to match. No script behavior changed — the script already used plain git throughout and needed no fix, only its rationale was wrong.

  **Follow-up (issue #2775 ask 1, 2026-09-13, branch `harness/publish-run-artifacts`): the wiring-half fix (PR #2757, which repointed Step 8 at this script) closed the "summary lost" failure but opened a NEW one — everything except the summary was still lost.** `git add "$_summary"` was the only file this script ever staged, so `dev/status/_index.md` (Step 5.5 reconcile), `dev/reviews/*.md` (QC records), `dev/audit/*.json` (audit records), and `dev/health/*.md` (health reports) rode along in the working tree with no publisher and were then destroyed by the orchestrator workflow's post-Step-8 `git reset --hard && git clean -fd` (whose own justifying comment, "everything intentional is already on the remote branch," stopped being true the moment Step 8 switched off `jj`'s auto-snapshotting). Run `34757914354` (2026-09-13) hit this; the artifacts survived only because the orchestrator committed them by hand before the bundling step ran. Since `.claude/agents/lead-orchestrator.md` Step 8 is write-gated in this runtime (same constraint as the wiring-half fix), the call site (`publish_daily_summary.sh publish --summary "$SUMMARY_FILE"`) cannot be changed to pass a new flag — so the fix is **default-ON**: `cmd_publish` now also stages, into the SAME commit as the summary, whichever of a fixed allowlist (`dev/status/_index.md`, `dev/reviews/`, `dev/audit/`, `dev/health/` — see `_RUN_ARTIFACT_PATHS` in the script) are actually dirty or untracked, via the new `_stage_run_artifacts()` helper. Nothing outside that allowlist is ever touched (`git status --porcelain -uall -- $_RUN_ARTIFACT_PATHS` scopes the scan; each dirty path is `git add`-ed individually rather than handed to one `git add` call, because `git add` fails FATAL under `set -eu` the instant one pathspec in a mixed existing/nonexistent list matches nothing on disk). `--summary-only` opts back into the old single-file behavior, as an escape hatch if the default-on staging ever misbehaves. `publish_daily_summary_test.sh` grew from 53 to 68 checks (new "Scenario group 10"): an artifact present-and-dirty IS staged and reaches the remote on the same branch as the summary; a dirty file OUTSIDE the allowlist (repo root) is never staged, remains dirty in the working tree, and never reaches the remote; `--summary-only` suppresses all artifact staging even with a dirty artifact present; and the "also staged" report line does not appear when nothing beyond the summary was committed. All four were mutation-verified by hand (mutation list + the specific check(s) each one reddens recorded inline in the comment directly above "Scenario group 10" in the test file, per this repo's own inline-mutation-list convention) — dropping the `_stage_run_artifacts` call, widening the allowlist to the repo root, making `--summary-only` a no-op, and hoisting the "also staged" echo out of its emptiness guard were each confirmed to turn their named check(s) red, then reverted. **Issue #2775 ask 2 (the now-stale `git reset --hard`/`git clean -fd` comment in the orchestrator workflow's bundling step) remains OUT OF SCOPE and unresolved** — it requires editing `.github/workflows/orchestrator.yml`, which needs the `workflow`-scoped PAT this runtime does not have (same constraint recorded for asks 1/2 in #2775 itself). Verify: `dev/lib/run-in-env.sh dune build && dev/lib/run-in-env.sh dune runtest` exit 0, zero `FAIL:` lines; `sh dev/scripts/publish_daily_summary_test.sh` directly prints `68/68 checks passed`.

  **Rework iteration 1 (PR #2778, qc-behavioral review 5191977193, 2026-09-13): the 68/68 suite pinned only ONE of the four allowlist entries, and a filename shape could lose the summary itself.** Two findings, both fixed: **CP1-A** — measured mutations that left the suite fully green: widening the `dev/status/_index.md` FILE entry to the whole `dev/status/` DIRECTORY (would sweep a routinely-dirty `dev/status/<track>.md` into the daily-summary commit), dropping `dev/audit` outright, and reducing the allowlist to `dev/health` alone (dropping three of four classes) — the single negative witness (`README.md`, outside `dev/` entirely) could not distinguish "the fixed allowlist" from "anything under `dev/`". **CP4-A** — `_stage_run_artifacts` read paths back out of `git status --porcelain`, which C-quotes any path containing a space/quote/backslash/non-ASCII byte (e.g. `"dev/health/2026-09-08 fast.md"`); feeding that quoted string to `git add` as a pathspec failed FATAL, and under `set -eu` that aborted `cmd_publish` **before `git commit`** — summary staged, never committed/pushed/PR'd, the exact H-DAILY-SUMMARY-PR-LOST outcome. Fix: `_stage_run_artifacts` no longer parses `git status` output at all — it iterates the four `_RUN_ARTIFACT_PATHS` entries directly (literal strings, never round-tripped through git's own quoting), `git add`-ing each that exists, and derives the reported list from `git diff --cached --name-only` before/after (safe: that output is only ever printed, never fed back as a pathspec). A per-entry `git add` failure (e.g. a `.gitignore`'d allowlist path) is now caught, logged as a named `WARNING`, and does NOT abort the publish — the summary and every OTHER healthy artifact still ship. `publish_daily_summary_test.sh` grew from 68 to 87 checks: a new all-four-artifacts scenario independently pins each of the four allowlist classes reaching the remote, plus two near-miss witnesses INSIDE `dev/` (a per-track `dev/status/<track>.md` file and a `dev/notes/` file, both of which must stay unpublished); a space-in-filename scenario (artifact staged and reaches the remote, summary too); and a forced-staging-failure scenario (`.gitignore`'d `dev/reviews/`) proving the summary and a sibling healthy artifact both still reach the remote with the WARNING line present. Mutation-verified (2026-09-13): widening `dev/status/_index.md` to `dev/status` → 85/87 (the two near-miss checks go red); dropping `dev/audit` → 86/87 (`all-four: dev/audit/ entry reaches the remote` red); reducing to `dev/health` alone → 83/87 (four named checks red, including the new WARNING-text check). Reproduced the pre-fix filename bug directly against `f50ee9435`'s real script (not the fixed copy): `rc=128`, `fatal: pathspec '"dev/health/2026-09-08 fast.md"' did not match any files`, whole script aborted before commit; the fixed script on the same fixture: `rc=0`, file staged correctly as `dev/health/2026-09-08 fast.md` (no quote corruption). Verify: `sh dev/scripts/publish_daily_summary_test.sh` prints `87/87 checks passed`; `sh trading/devtools/checks/posix_sh_check.sh` prints `OK: posix-sh linter -- 98 scripts clean.`

- [x] **H-PRUNE-ANCHOR-SELFREF — `prune_candidates.sh` checker2 sanity probe was self-defeating; replaced hardcoded real anchor with a synthetic self-test** (issue #2722, branch `harness/prune-anchor-selfref`). The `prune-candidates-weekly` cron went red (run [34143271364](https://github.com/dayfine/trading/actions/runs/34143271364), 2026-09-07) with `FAIL(checker2): sanity probe failed -- dev/experiments/fuzz-startdate-crash was not flagged as an orphan candidate`. Root cause: commit `efc25d68` (PR #2651, the prior `H-PRUNE-GIT-SAFEDIR` fix) documented that same anchor's literal name in this status file to explain the bug it was fixing -- and `dev/status/` is one of checker2's own citation sources (`CITE_DIRS_CHECKER2`), so from that commit on `_is_cited` correctly reported the anchor as cited, and the (then-hardcoded-real-anchor) probe correctly refused to trust the run. Renaming the anchor would only defer the identical failure to the next time anyone documents the new name -- including this very entry, which necessarily names the old anchor to explain the bug (see the self-defeat check below). Fix: replaced the probe with `_checker2_self_test()` in `dev/scripts/prune_candidates.sh`, which builds a throwaway git repo (`mktemp -d`) containing two freshly PID-suffixed synthetic directories -- one guaranteed-uncited-by-construction (it doesn't exist until the function creates it, so no document anywhere can ever name it) and one deliberately cited from a synthetic `dev/status/` file inside that same throwaway repo -- then exercises the exact production `_is_cited` / `_git_log_date` / `_age_days` primitives against both (via a temporary `ROOT` swap), asserting the first reads uncited and the second reads cited. Two-sided by construction: a citation-matching primitive that always answers the same way (matches everything, or matches nothing -- both historical failure shapes, `\s`-vs-`[[:space:]]` and the git safe.directory regression) fails one guard or the other. Tradeoff, stated explicitly: this no longer proves `find`'s enumeration of the real `dev/experiments/` tree works, only that the citation/dating primitives are correct in the same shell/grep/git as the live pass -- accepted because every historical failure of this probe was a primitive bug, never an enumeration bug. `dev/scripts/prune_candidates_test.sh`: removed the obsolete Fixture C (asserted the probe fired when the real anchor was absent -- no longer applicable, there is no real anchor); replaced with a Fixture C that is the direct regression pin for this issue -- an arbitrary old experiment dir is documented from a synthetic `dev/status/harness.md`, mirroring exactly what `efc25d68` did, and the suite asserts the run still exits 0 with no `FAIL(checker2)` and the documented dir still correctly excluded. Mutation-verified non-vacuous: (1) forcing `_is_cited` to `return 0` unconditionally (match-everything) trips the "reported as CITED" guard, 32 test failures; (2) forcing it to `return 1` unconditionally (match-nothing) trips the "reported as NOT cited" guard, 31 failures; (3) shortening the synthetic fixture's commit date from 40 to 5 days before `TODAY` trips the self-test's own age>=30 sanity check, 31 failures; all three restored to a byte-identical script and reverified green (36/36). Verify: `bash dev/scripts/prune_candidates.sh` on the real tree exits 0 (re-verified AFTER this status entry was written, specifically to confirm the fix doesn't recreate the bug it fixes -- the entry above unavoidably names `fuzz-startdate-crash`, but the self-test no longer looks up any real name at all so the citation is inert); `bash dev/scripts/prune_candidates_test.sh` -- `36 passed, 0 failed`; `dash -n dev/scripts/prune_candidates.sh` / `dash -n dev/scripts/prune_candidates_test.sh` both clean.
...
- [x] **H-SCHEDULED-WORKFLOW-HEALTH — scheduled (cron) workflow health check, script half of #2634** (branch `harness/scheduled-workflow-health`, GHA orchestrator run 33894722318). Nothing in this repo previously monitored whether its scheduled workflows were still succeeding: two weekly workflows had been red for weeks (one for five) and a daily cron silently missed a day, all found only when a human happened to look. **Built:** `trading/devtools/checks/scheduled_workflow_health.sh`, a standalone POSIX-sh script (no `.github/workflows/**` changes — see Scope below) that queries `GET /repos/{owner}/{repo}/actions/workflows` (fully paginated to completion, not just the first page) then, for every ACTIVE workflow, `GET .../workflows/{id}/runs?event=schedule&per_page=1` for its single newest scheduled run. Classifies each into RED (conclusion failure/cancelled/timed_out/action_required) / STALE (not RED, but older than a staleness window — default 216h/9 days, overridable via `--stale-hours` / `SCHEDULED_WF_HEALTH_STALE_HOURS`) / NO-SCHEDULE (zero observed scheduled runs — informational only, never fails the gate) / OK. Exit codes are distinct per failure class and never collapse "couldn't measure" into "measured green": 0 = clear, 1 = at least one RED/STALE (the real signal), 2 = no `GH_TOKEN` and no fetch hook (can't call the API at all), 3 = an API call failed or returned non-JSON, 64 = usage error. The HTTP call is injectable via `SCHEDULED_WF_HEALTH_FETCH` (an external-command hook, same seam shape as `PR_GATE_STATUS_LIB` in `dev/scripts/pr_gate_status.sh`), and "now" is injectable via `SCHEDULED_WF_HEALTH_NOW_EPOCH`, so the fixture test is fully hermetic (no network). Two traps this repo has been burned by before were addressed directly: (1) **pagination-is-a-floor** (the 2026-09-02 orchestrator undercounted a failure streak 3x by reading a `per_page=5` page boundary as the whole answer) — the workflow-list call actually loops to completion and the summary line states the real page count fetched, and the per-workflow runs call was `per_page=1` by design (it only ever claimed "the single newest run's conclusion," never a streak count, so there was no floor to mislabel) — **superseded by H-SCHEDULED-WORKFLOW-HEALTH-STREAK below**, which replaced the single-newest-run fetch with a multi-run page precisely because that "no floor to mislabel" claim stopped being true the moment the script started reporting a streak; (2) **a check that can't fail is not a check** — the missing-token and API-error paths were mutation-tested by hand (stripping the no-token guard, breaking RED-conclusion matching) to confirm the corresponding assertions actually go red, not just pass vacuously. **Real bug caught and fixed during development, not shipped:** an early version called `exit 3` from inside `_api_get_json`, which is always invoked via `x=$(...)` command substitution (a subshell) — `exit` there only kills the subshell, so the failure silently evaporated 1-2 subshell layers up and the script would have reported a false "0 active workflows, exit 0" on an API error. Fixed by having every network-calling helper `return` its failure code explicitly and having each caller (`_list_active_workflows`, `_newest_scheduled_run`, and `main()` itself) check and re-propagate it, rather than relying on `set -e` to cascade through multiple subshell boundaries. **Test:** `trading/devtools/checks/scheduled_workflow_health_test.sh`, 10 fixture-driven assertions (all-green; one RED named on its own line and in the summary; STALE via an aged-but-successful run; NO-SCHEDULE that does not force a non-zero exit; missing-token exit 2; API-error exit 3; malformed-JSON exit 3; full pagination across 2 pages proving `active=101` is a real total not a page-1 floor of 100; `--stale-hours` override actually reclassifying the same run age; malformed `--stale-hours` -> exit 64), wired into `dune runtest` via `trading/devtools/checks/dune` (explicit `(deps scheduled_workflow_health.sh)`, no `(universe)` needed — every fixture is a canned-JSON shell shim, the only real-repo file read is the script itself). Both `dash -n` (posix_sh_check.sh) and the full `dune build @fmt && dune build && dune runtest` are clean. **Live-API non-vacuity proof** (run against the real repo with a real `GH_TOKEN`, not just fixtures): reproduced all three known-red workflows unprompted — `RED Prune candidates weekly`, `RED Weekly track pacer`, `RED Weekly start sweep (BAH SPY)` — plus `SUMMARY: active=17 (1 page(s) fetched...) ok=7 red=3 stale=0 no-schedule=7`, exit 1. **Scope / what's still open:** wiring this into an actual cron or CI job (`.github/workflows/**`) is the other half of #2634 and is explicitly OUT of scope here — the orchestrator's token lacks the `workflow` OAuth scope (same standing blocker as #1636/H-BLAS/H-PRUNE-GIT-SAFEDIR above), and this script was deliberately built standalone so it can land without that scope. A human (or a workflow-scoped PAT) needs to add a scheduled job that runs it and surfaces a non-zero exit. Verify: `dev/lib/run-in-env.sh dune build && dev/lib/run-in-env.sh dune runtest` exit 0; `dune runtest devtools/checks/` shows `scheduled_workflow_health_test: ... OK: scheduled_workflow_health_test -- all assertions passed.`; `GH_TOKEN=... sh trading/devtools/checks/scheduled_workflow_health.sh` against the real repo.

- [x] **H-SCHEDULED-WORKFLOW-HEALTH-STREAK — fix the newest-run-only blind spot that let the health check itself go dark for 3 days** (branch `harness/schedhealth-streak`, dispatched 2026-09-13 in response to the GHA orchestrator's Daily-orchestrator workflow failing six consecutive scheduled runs, 2026-09-10 12:14Z through 2026-09-12 15:32Z, unnoticed because the health check above reported it `OK`). Root cause: H-SCHEDULED-WORKFLOW-HEALTH classified only the single newest scheduled run per workflow, and that newest run was the orchestrator's own in-flight, `status=in_progress` execution — the old code treated in_progress as `OK` ("succeeded, or is still in progress"), so the very workflow that runs this check could never see its own failure streak underneath its own live run. **Reproduced by hand before fixing**, against the pre-fix script with a fixture matching exactly what it requests (`per_page=1`, newest run `in_progress`): `OK Daily orchestrator ... EXIT=0`. **Fixed:** `scheduled_workflow_health.sh` now fetches up to `RUNS_PER_WORKFLOW` (default 10, one API call, `per_page=N`, override via `--runs-per-workflow` / `SCHEDULED_WF_HEALTH_RUNS_PER_WORKFLOW`) most recent scheduled runs per workflow instead of just the newest, skips leading `in_progress`/`queued` runs for classification purposes (they carry no verdict), and computes a real failure-STREAK from the newest COMPLETED run backwards. RED now fires whenever that streak is >= 1, with the count reported per-workflow (`streak=N`) and folded into the SUMMARY line; a streak that consumes the entire fetched page (never closed by an older success) is reported as a floor (`streak>=N`) rather than a false-exact count, mirroring the workflow-LIST call's own pagination-floor discipline. A new `UNOBSERVABLE` classification (informational only, never forces non-zero) covers the case where every fetched run is still in_progress/queued — the exact "no run has completed yet" state that must never read as `OK`. Header comments rewritten to match (the old "this script never reports a failure-streak COUNT" claim was the reason the incident happened and is corrected in place, with the incident recorded as a `WHY A SINGLE NEWEST RUN WAS NOT ENOUGH` section). **Test:** `scheduled_workflow_health_test.sh` grew from 16 to 20 fixture-driven assertions — added #17 (6 consecutive failures, no newer run, exact `streak=6`), #18 (the incident shape itself: 6 failures plus a newest `in_progress` run, still `RED streak=6 in_progress=1` — verified by hand to reproduce `OK`/exit 0 against the pre-fix script and `RED`/exit-non-zero against the fix), #19 (newest run recovered to success after an older failure -> not RED, `streak=0`), #20 (every fetched run in_progress/queued -> `UNOBSERVABLE`, not `OK`, exit 0). All pre-existing per-workflow-runs fixture paths updated from `per_page=1` to `per_page=10` to match the new default. Verify: `dev/lib/run-in-env.sh dune build @fmt && dev/lib/run-in-env.sh dune build && dev/lib/run-in-env.sh dune runtest --force` all exit 0; `sh trading/devtools/checks/scheduled_workflow_health_test.sh` shows `=== Results: 20 passed, 0 failed ===`. Scope unchanged from H-SCHEDULED-WORKFLOW-HEALTH above: still script-only, still blocked on a `workflow`-scoped PAT to wire into an actual `.github/workflows/**` cron.
  **Rework iteration 1 (qc-behavioral NEEDS_REWORK, quality 2):** headline fix confirmed correct by mutation, but four documented contracts had zero assertion coverage — each mutated-and-measured green before the fix. Grew the suite from 20 to 26 assertions: #21 pins the pagination-floor branch itself (a full `RUNS_PER_WORKFLOW=10` page of consecutive failures must report `streak=>=10`, not the false-exact `streak=10` — deleting the floor-detection `if` block left the suite at 20/0 before this); also fixed a real doc defect the mutation surfaced — the header and SUMMARY string rendered the floor as `streak>=N` when the code has always emitted `streak=>=N` (assertion #17 already knew the true form; the prose did not — now both agree). #22-24 close the `--runs-per-workflow` / `SCHEDULED_WF_HEALTH_RUNS_PER_WORKFLOW` knob, previously at zero coverage (`grep -c` over the test file returned 0): the flag driving `per_page=N`, the env var doing the same, and `--runs-per-workflow 0` -> exit 64. #25 pins that staleness is measured from the newest COMPLETED run, never an in_progress run sitting on top of it (a newest `in_progress` run stamped "now" over a 40-day-old completed success must still read `STALE`, not `OK` — swapping `_first_completed_created_at` for `_newest_created_at` in the age computation left the suite green before this). #26 pins "exactly one API call per workflow" via a call-logging shim (a duplicated `_recent_scheduled_runs` call per workflow left the suite green before this; the log now shows exactly 1 list call + 1 runs call per workflow). **Mutation-verified for all four** (measured, not predicted): floor-branch deletion -> 25/1 (assertion 21 fails), restore -> 26/0; flag-wiring removal -> 24/2 (assertions 22+24 fail) and env-wiring override -> 25/1 (assertion 23 fails), restore -> 26/0; staleness-source swap -> 25/1 (assertion 25 fails), restore -> 26/0; duplicated runs-call insertion -> 25/1 (assertion 26 fails, call log shows 5 calls instead of 3), restore -> 26/0. Verify: `sh trading/devtools/checks/scheduled_workflow_health_test.sh` shows `=== Results: 26 passed, 0 failed ===`, exit 0.
...
- [x] H-CI-DISPATCH: GitHub Actions created **no check-suite at all** for branch `fix/lapacke-gp-cholesky` (PR #2113) across three SHAs, a PR open, a close/reopen, and a PAT-authenticated `synchronize` push — while sibling PRs #2112/#2114/#2115 triggered normally in the same window. `ci.yml` and `perf-tier1.yml` are plain `on: pull_request` with no path/branch filters, so this is not configuration. A QC-APPROVED fix could not clear gate 1 and was correctly held. Worth a watchdog: if a PR has been open >N minutes with zero Actions check-suites, say so loudly rather than waiting forever. **Fixed**: `dev/scripts/pr_gate_status.sh` computed `ci=none` for zero check-runs but had no explicit case arm for it, so it silently fell through to the generic `*:none:*` (struct=none) arm and printed `dispatch qc-structural` — dispatching QC against a PR with no CI signal at all, the exact violation `.claude/rules/pr-gate-loop.md` forbids. Added a `none:*)` arm (ordered right after `FAIL:*`/`pending:*`, before the QC-dispatch arms) that prints `no CI -- check mergeable (conflicted PR?) or push to retrigger`, or, when the PR's `mergeable` state (read from the same already-fetched PR-meta payload, both gh and curl backends — `gh --json mergeable`, and REST's `mergeable_state == "dirty"` normalised to the same MERGEABLE/CONFLICTING/UNKNOWN vocabulary) is `CONFLICTING`, names the conflict explicitly. Regression coverage: 8 new e2e probes appended to `dev/scripts/pr_gate_status_test.sh` (4 gh-backend, 3 curl-backend — CI-column, no-dispatch, generic message, conflict message; suite now 156 tests). Mutation-verified by hand: reverting the `none:*` arm made 6 of the 8 new assertions fail red (want no/got yes for the qc-structural dispatch, want yes/got no for the diagnostics), reproducing the original bug exactly; restored and confirmed the restored file is byte-identical to the pre-mutation version. Verify: `sh dev/scripts/pr_gate_status_test.sh` (156 tests clean) and `flock /tmp/dune.lock dune runtest devtools/checks` (includes the pre-existing `pr_gate_status_mutation_test.sh` mutation-coverage harness, untouched by this change since no pinned mutation targets the CI-column case arm or the new `mergeable` field).
...
- [x] H-QC-SCORE-ADJECTIVE-LEXICON: Filed from PR #2135 rework (qc-behavioral N3, 2026-07-27), NOT built there — new scope. #2115's root cause was a digit/adjective mismatch (score 1 captioned "Excellent"); both agent definitions now state the polarity explicitly and instruct the agent to self-check the rationale's adjective against the digit before posting (H-QC-SCALE), but that check is still self-discipline with no mechanical backstop. Built: a warn-level, non-blocking companion library `trading/devtools/checks/qc_score_lexicon_check.sh` (not inlined into `record_qc_audit.sh` — kept as a small companion sourced by it, to avoid touching that file's already heavily-hardened 50+-scenario extraction logic for a 6-line feature). Curated lexicon: positive `excellent exemplary flawless impeccable outstanding pristine clean`; negative `significant fundamental egregious broken unacceptable wrong`; deliberately excludes "critical"/"sound"/"fine"/"good" as too ambiguous in review prose (see the script's own header for the rationale on each). Scoped to only the extracted one-line Quality Score rationale (never the Findings/Checklist body), which is also the stated, explicit answer to the backlog's named false-positive risk ("negative word inside a quoted finding") — see the header's "SCOPED TO THE QUALITY-SCORE RATIONALE LINE" section for why this was judged sufficient rather than adding quote-detection. `record_qc_audit.sh` sources it after quality-score validation and calls it for its stderr side effect only; exit code and JSON record are provably unaffected (proof: `record_qc_audit_test.sh` scenarios 54/55, added alongside its existing total (measured baseline 74 → 76 with scenarios 54/55; the earlier "59 → 76" figure was not reproducible — #2814 review), both assert rc=0 and correct `quality_score` in the written JSON — 54 with a real WARN present, 55 confirming silence on a correctly-paired score). Own suite `qc_score_lexicon_check_test.sh` (11 scenarios) covers non-vacuity (fires on 1+"Excellent", stays silent on 1+"Fundamental"/5+"Exemplary"), the middle digit-3 no-op, out-of-range/empty digit no-op, case-insensitivity, word-boundary (`unbroken` must not false-match `broken`), and extraction (last-section-wins across a rework blob, bold-stripping, no-heading-found). Mutation-verified by hand (4 mutations: widen digit gate to include 3 → scenario 5 RED; drop `-w` word-boundary → scenario 8 RED; keep first Quality-Score section instead of last → scenario 9 RED; drop bold-stripping → scenario 10 RED; each restored to GREEN). Verify: `dev/lib/run-in-env.sh dune runtest devtools/checks` (or `bash trading/devtools/checks/qc_score_lexicon_check_test.sh` / `bash trading/devtools/checks/record_qc_audit_test.sh` directly, no dune needed).
...
- [x] H-FOLLOWUP-THRESHOLD-RETUNE: **Fixed (harness/followup-threshold-retune).** Chose resolution **(b)**, not (a): `trading/devtools/checks/deep_scan/check_05_followup_items.sh` now EXCLUDES two self-describing shapes from the headline `FOLLOWUP_COUNT` metric instead of retuning `dev/config/merge-policy.json`'s `followup_threshold`. Rejected (a) because the non-actionable floor is not a constant — Tier items open and close over time — so a raised threshold decays the moment the roadmap's shape changes, whereas an exclusion keyed on the file's own structure (a heading, a code fence) tracks that shape automatically with no side-list to keep in sync. **Re-measured at fix time** (`sh trading/devtools/checks/deep_scan/check_05_followup_items.sh /tmp/r /tmp/f`, then `cat /tmp/f`): **67 total** open `- [ ]` items across `dev/status/*.md` at the rework tip (68 on the pre-PR corpus; grown from the 41 measured at filing time — real ad-hoc debt accumulated, not just the construction floor), of which **9 are excluded** (3 Tier-2 + 5 Tier-4 roadmap checkboxes in this file's `## Tier 2` / `## Tier 4` sections + 1 template line in `dev/status/cleanup.md`'s fenced Backlog-entry example) and **58 are actionable** — the two Tier-3 items (T3-C, T3-H) count as actionable, see the rework note below. The orchestrator's Step 2b comparison point moves from `68 > 10` to `58 > 10` — both cross the threshold, which is now honest signal (real accumulated ad-hoc debt, most of it non-blocking QC-filed residuals) rather than a number inflated by construction; whether 58 itself warrants a *separate* threshold retune is intentionally left open, not decided here. **Exclusion mechanism (self-describing, not a denylist):** (1) a `- [ ]` line under an H2 heading matching `## Tier 2` or `## Tier 4` — the two tiers lead-orchestrator Step 2c declares undispatchable (milestone-gated / human-request-only) — is excluded; **Tier 1 and Tier 3 items remain actionable** because Step 2c dispatches Tier 1 first and Tier 3 next (qc-behavioral rework iteration 1: the original version matched any `## Tier <digit>` heading, which silently excluded dispatchable Tier-1/Tier-3 work; the `track-c.md` fixture now pins Tier 1 and Tier 3 as actionable and Tier 4 as excluded); a reader sees the exclusion by reading the file's own heading; (2) a `- [ ]` line inside a fenced code block (` ``` `...` ``` `) is excluded — the standard Markdown "this is example text" convention, catches exactly `dev/status/cleanup.md`'s "How findings get here" template line and nothing else (verified: `awk` scan for `- [ ]` lines inside fences across all of `dev/status/*.md` returns exactly that one line). Both exclusions are counted, not silently dropped — new metrics `FOLLOWUP_EXCLUDED` and `FOLLOWUP_TOTAL` sit alongside `FOLLOWUP_COUNT`, and the Warnings/Info line now reads e.g. "58 actionable open ... (9 additional Tier-2/Tier-4-roadmap/template items excluded, 67 total open)" so a reader never has to reconcile prose against the number. **Fail-loud guard added** (the pre-flight brief's finding-1 shape, pre-empted directly): the check now `die`s (non-zero exit, `FAIL: check_05_followup_items: found zero dev/status/*.md files ...` to stderr) if it reads zero status files, instead of silently falling through to `FOLLOWUP_COUNT=0` — mutation-verified live (temporarily deleting the guard reproduced the exact defect: exit 0, a findings file containing `M: FOLLOWUP_COUNT=0`). **Regression coverage** in `trading/devtools/checks/deep_scan_followup_count_check.sh`: added a third fixture file (`track-c.md`) exercising both exclusion shapes plus their "still actionable outside the excluded region" controls (an ordinary-heading item, an after-the-fence item), and a fourth fixture root with an empty `dev/status/` proving the fail-loud guard. **GREEN→RED proof for every new assertion** (each mutation applied, suite re-run, reverted): (1) dropping the tier-heading exclusion condition → 5 assertions fail (fixture reads 7 actionable / 3 excluded / 10 total at the rework tip; widening the rule back to `## Tier <digit>` yields 5 / 5, 5 assertions red); (2) dropping the fence exclusion condition → 5 assertions fail; (3) deleting the fail-loud guard block entirely → 3 assertions fail, naming the exact false `FOLLOWUP_COUNT=0` metric it would have silently written; (4) wording-only mutation of the `die` message → exactly 1 assertion fails (the message-content check), proving it's independently load-bearing from the exit-code check. All four reverted cleanly (byte-identical `diff` against the pre-mutation file) and the suite returns green each time. `check_08_trends.sh` required no change — it only consumes the per-file sidecar (`file:actionable_count`), whose semantics are unchanged. **Verify:** `sh trading/devtools/checks/deep_scan_followup_count_check.sh`; `dev/lib/run-in-env.sh dune build @fmt && dune build && dune runtest` (0 `^FAIL:` lines). Escalation (not fixed here, out of scope per this task's explicit constraint — `.claude/agents/**` writes refused in this runtime): `lead-orchestrator.md` Step 2b's own prose ("Count total open items across all `## Followup / Known Improvements` sections") is independently stale relative to both this fix and the original H-FOLLOWUP-COUNT fix — it describes neither the unscoped-by-heading count nor the actionable/excluded split. Someone with write access to that file should update it to point at `check_05_followup_items.sh`'s actual behavior rather than restate a heading-scoped rule that hasn't matched reality since H-FOLLOWUP-COUNT shipped. (source: 2026-09-05 harness session, GHA run 33962894987)
...
- [x] H-AUDIT-TEST-DISABLE-COUNT-PAD-DIRECTION-WORDING (B3): Non-blocking wording finding from qc-behavioral on PR #2231's review of the H-AUDIT-TEST-DISABLE-COUNT-TAUTOLOGICAL fix. The *shrink* direction is now caught by `HOOK_DISABLE_EXPECTED_COUNT`, but the *pad* direction still self-contradicts in the message: adding a 7th spelling to `HOOK_DISABLE_VALUES` prints `for all 6 documented 'off' spellings` (the hardcoded `HOOK_DISABLE_EXPECTED_COUNT`) immediately followed by a 7-element list from `_disable_values_repr()`. Functionally harmless — the scenario still goes red on a pad because `seen != HOOK_DISABLE_EXPECTED_COUNT`, and the same line's `exercised 7` mitigates the confusion — but the wording itself is internally inconsistent the same way the pre-fix shrink-direction message was. Fix shape: word the message as "expected N documented spellings, exercised M" without asserting "all N ... (list of M)" as a single clause. `harness_gap: NONE`. (source: 2026-08-07 qc-behavioral on PR #2231) **Fixed:** reworded both scenario 25a and 26a's `fail` messages in `record_qc_audit_test.sh` to state the expected count and the exercised count (plus the actual `_disable_values_repr()` list) as separate clauses — `"expected ${HOOK_DISABLE_EXPECTED_COUNT} documented 'off' spellings, exercised ${seenNN} (<list>); each exercised value must show rc=0 [+ record] + 'OK: wrote'; ..."` — rather than fusing "for all N ... (list of M)" into one self-contradicting clause. Message text only, no behaviour change. Verified by hand: padded `HOOK_DISABLE_VALUES` with a 7th value in a scratch copy and confirmed the scenario still FAILs and the message no longer contradicts itself — before: `expected rc=0 + record + 'OK: wrote' for all 6 documented 'off' spellings ('0' 'false' 'no' 'yes' 'true' '' 'padvalue'); exercised 7; ...`; after: `expected 6 documented 'off' spellings, exercised 7 ('0' 'false' 'no' 'yes' 'true' '' 'padvalue'); each exercised value must show rc=0 + record + 'OK: wrote'; ...`. Full suite unaffected: 76 passed, 0 failed, unpadded. Verify: `bash trading/devtools/checks/record_qc_audit_test.sh`. (fixed 2026-09-25 harness-maintainer on harness/b3-pad-direction-wording, dispatched by GHA orchestrator)
...
- [x] H-AUDIT-27B-CUMULATIVE-COUNT: Filed by qc-behavioral on PR #2231 (residual R1, non-blocking). Scenario 27b asserts `walkup_count27b == 1` as a **cumulative** count over a `WALKUP_ROOT` fixture it shares with scenario 27. Robust today -- the reviewer's probe J2 (revert the precedence fix) reddens both scenarios rather than producing a false green, and J1 shows 27b passes even with 27's write neutralised, so it depends on the *fixture* rather than on 27's side effects. But it is sensitive to any **future** scenario that writes into `WALKUP_ROOT`. Fix shape: snapshot the count immediately before 27b and assert a delta of exactly 1, or give 27b its own fixture. `harness_gap: NONE`. (source: 2026-08-07 run 1 qc-behavioral on PR #2231) **Fixed:** scenario 27b now snapshots the file count immediately before its own `write_audit.sh` call and asserts a **delta** of exactly 1 new record, rather than an absolute post-run count. **Mutation-tested:** simulated the named future-scenario hazard (an extra write into `WALKUP_ROOT` before 27b runs) -- the old cumulative assertion mis-reported FAIL (expected count==1, actual count==2, even though 27b's own behaviour was correct); the new delta assertion correctly reported PASS (before=1, after=2, delta=1) under the identical hazard. Suite unchanged at 76 passed, 0 failed both before and after. Verify: `bash trading/devtools/checks/record_qc_audit_test.sh`. (fixed 2026-09-20 harness-maintainer on harness/audit-27b-residuals, dispatched by GHA orchestrator run 35521718664)

- [x] H-AUDIT-27B-LEAKS-UNDER-PWD-REGRESSION: Filed by qc-behavioral on PR #2231 (residual R2, non-blocking). Under a walk-up regression that resolves from `$PWD` instead of `dirname $0` (the reviewer's probe K4), scenario 27b goes red **and leaves a stray record in the live `dev/audit/`**. Inherent to exercising the unset-`REPO_ROOT` path with an in-tree script; the test does go red so a human looks, which is why this is a residual rather than a defect. Fix shape: `cd` to a scratch directory for the 27b invocation. `harness_gap: NONE`. (source: 2026-08-07 run 1 qc-behavioral on PR #2231) **Fixed:** scenario 27b's `write_audit.sh` invocation now runs with its cwd set to an empty scratch directory (no `.git`/`.claude` anywhere above it), inside the `$(...)` subshell so the suite's own cwd is untouched. **Mutation-tested:** reproduced probe K4 (mutated a scratch copy of `write_audit.sh`'s `_repo_root()` to resolve its walk-up start dir from `$PWD` instead of `dirname "$0"`) against the PRE-fix scenario: rc=0, `OK: wrote`, and a stray file was left in the real `dev/audit/` (195 -> 196 files, confirmed via `git status --porcelain dev/audit`). Against the POST-fix scenario, the same mutation still reddens 27b (rc=1, `FAIL: could not locate repo root`, delta=0) -- detection preserved, not just silenced -- and `dev/audit/` stayed clean (195 files before and after, `git status` empty). Suite unchanged at 76 passed, 0 failed. Verify: `bash trading/devtools/checks/record_qc_audit_test.sh`. (fixed 2026-09-20 harness-maintainer on harness/audit-27b-residuals, dispatched by GHA orchestrator run 35521718664)
...
- [ ] H-JJ-JST-BROKEN-GHA: **`jst submit` (and any `jj` subcommand that triggers `jj git fetch --all-remotes`) is non-functional in the GHA orchestrator container**, so **no dispatched agent can open its own PR via `jst`** — measured 2026-08-03 run 2. `jst submit <branch>` fails inside `jj git fetch --all-remotes` with `Git does not recognize required option: porcelain (note: supported version is 2.41.0)` — a real jj/git version incompatibility (image git 2.34.1 vs jj's requirement of git >= 2.41.0), unrelated to any branch's content. `gh` is also absent from this image. **Scope note (corrected 2026-09-09, issue #2741): this entry is about the `jj git fetch --porcelain` / `jst submit` incompatibility ("D1") only — it does NOT establish that plain `jj git push` is broken.** A separate, unrelated defect in the orchestrator's own `jj push` path ("D2": Step 8 sets *git's* identity instead of jj's, via `git config user.email/user.name`, and never runs `jj describe`, so `@` stays authorless/descriptionless) is measured and corrected in issue #2741 and the H-DAILY-SUMMARY-PR-LOST entry above — do not conflate the two, and do not read this entry's original blanket "`jj` ... non-functional" phrasing as extending to plain `jj git push`. Together D1 and the historical (pre-#2741) belief that `jj push` was also broken meant **every** agent-facing PR-creation path documented in `.claude/agents/lead-orchestrator.md` Step 4 (`GH_TOKEN=$GH_TOKEN jst submit feat/<feature>`) and in the `feat-*`/`ops-data` definitions was treated as dead in GHA; the only working path is the orchestrator's own Step 4.5 `curl` REST fallback (still true for `jst`/`gh` specifically). **This is the mechanical root cause of the recurring "branch pushed, no PR" pattern** — most visibly `feat/picks-slices-bcd`, which sat invisible for a full day with 931 insertions and no PR until run 1 surfaced it, and again this run for `harness/prev-verdict-pipefail` and `fix/picks-slices-lint` (both recovered by the REST fallback). Prior runs recorded this as agents "forgetting" the step; it is not — the tool is broken. Fix direction, pick one: (a) install `gh` in the orchestrator image and switch agent definitions to `gh pr create`; (b) pin a jj/git pair that interoperate; or (c) accept it, delete the `jst` instructions from the GHA path in the agent definitions, and document the orchestrator REST fallback as the sole supported mechanism. (c) is cheapest and matches observed reality. Note (a) and (c) both require editing `.claude/agents/**`, currently refused in this runtime. (source: 2026-08-03 run 2, harness-maintainer + feat-weinstein dispatches; scope corrected 2026-09-09 per issue #2741)
...
## Added 2026-09-24 (harness-maintainer, harness/2941-no-schedule-crowded, issue #2941)

- [x] **H-SCHEDULED-WORKFLOW-HEALTH-CROWDED — distinguish a crowded-page false NO-SCHEDULE from a real one (issue #2941)**. Since #2928's move to an unfiltered-then-client-side-filtered runs fetch, a workflow with both a schedule trigger and enough non-schedule (push / `workflow_dispatch` / pull_request) traffic to fill the whole fetched page could return zero `.event == "schedule"` runs even though `_recent_scheduled_runs` had already computed `page_full=1` -- `main()` discarded that signal on the empty-`run_line` path and printed the same `NO-SCHEDULE ... (no scheduled runs observed)` line as a workflow with a genuinely short, fully-observed history holding no cron run at all. The two claims are not the same: one says "there may be scheduled runs further back that this fetch never saw," the other says "the whole observed history was seen and it holds none." **Fixed:** `trading/devtools/checks/scheduled_workflow_health.sh` `main()` now branches on `_page_full` when `_run_line` is empty -- a full page prints a new class, `NO-SCHEDULE-CROWDED\t<name>\t(page of <RUNS_PAGE> runs held no scheduled run -- crowded; raise SCHEDULED_WF_HEALTH_RUNS_PAGE_FACTOR)`, counted in its own `_nosched_crowded_count` / `no-schedule-crowded=` SUMMARY bucket, kept out of `no-schedule=`; a short page keeps the plain `NO-SCHEDULE` line unchanged. **Classification decision (UNOBSERVABLE-like):** `NO-SCHEDULE-CROWDED` is informational only and never forces a non-zero exit -- same as `NO-SCHEDULE` and `UNOBSERVABLE` -- because there is no positive evidence of failure, only an absence of evidence past a crowded page; it is a distinct bucket rather than folded into either existing class specifically so it is never misread as a confident "no cron here." Header CLASSIFICATION section, the `0 exit code` line, `_usage()`, and the `_classify_recent_runs` docstring cross-reference all updated to name the third cause. `dev/scripts/scheduled_workflow_summary.sh`'s exit-0 branch now also treats a `^NO-SCHEDULE-CROWDED` line like `^UNOBSERVABLE` (renders `UNMEASURABLE (exit 0: ...)` instead of `all OK`), verified by hand against two synthetic reports (one crowded, one plain-NO-SCHEDULE) -- confirmed the plain case is unchanged. **Test:** `trading/devtools/checks/scheduled_workflow_health_test.sh` grew from 32 to 34 numbered assertions (35 pass-lines, assertion 30 has two sub-cases) -- #33 (a FULL page of 15 push-only runs, zero schedule -> `NO-SCHEDULE-CROWDED`, not plain `NO-SCHEDULE`, `no-schedule-crowded=1`/`no-schedule=0`) and #34 (a SHORT page of 5 push-only runs, zero schedule -> plain `NO-SCHEDULE` unchanged, `no-schedule=1`/`no-schedule-crowded=0`, proving #33's new branch is gated on `page_full` and not on "push-only" alone). **Mutation-verified:** forcing the `page_full` check on the empty-`run_line` path to always take the false branch (`_page_full -eq 9` instead of `-eq 1`) turned assertion 33 RED (`34 passed, 1 failed`, printing the old plain `NO-SCHEDULE` line and `no-schedule-crowded=0`) while leaving assertion 34 green; reverted to `35 passed, 0 failed`. Verify: `sh trading/devtools/checks/scheduled_workflow_health_test.sh` shows `=== Results: 35 passed, 0 failed ===`; `dune build @fmt && dune build && dune runtest` all exit 0.

## Added 2026-09-25 (harness-maintainer, harness/intent-to-add-counter)

- [x] **H-JJ-ITA-COUNTER — jj-colocation intent-to-add pollution corrupted `orchestrator_fastexit_gate.sh`'s daily run counter.** Observed 2026-09-24-run2 (§Escalations): after the orchestrator's own main-tree `dune runtest`, its untracked `dev/daily`/`dev/audit` files showed as ` A` (`git add -N`-shaped), flipping `current_summary_path`'s run count from `-run2` to `-run3`. The prior investigation found nothing by grepping for `add -N` / `intent-to-add` in `dev/scripts`/`dev/lib`/`trading/devtools` — the actual site was a `jj` invocation, not a git one.

  **Root cause (reproduced end to end with the real, unmodified pre-fix script, in hermetic sandboxes — never the orchestrator's own checkout).** `trading/devtools/checks/jj_workspace_smoke.sh` is wired into `dune runtest` and, whenever the repo it runs against is jj-colocated (true in every container image this suite runs in, GHA included — `.jj/` sits alongside `.git/` there despite dispatch prompts sometimes saying "no jj"), runs `jj -R "$REPO" workspace add/list/forget` against the repo root itself. **Any** `jj` command that resolves the working-copy commit of the DEFAULT workspace — which `workspace add` (targets a *different* workspace) and `workspace list` (looks read-only) both do as a side effect — snapshots that working copy and exports untracked files sitting there into the colocated git index as intent-to-add entries. `jj workspace forget` does not undo this. `git ls-files` (the run-counter's original data source) counts an intent-to-add entry as tracked, since it is a real index entry (just with an empty placeholder blob, `git ls-files -s` shows the empty-tree SHA `e69de29b...`). Confirmed the mechanism is real and isolated to `jj`, not `git`, by reproducing it in a from-scratch `git init` + `jj git init --colocate` sandbox with zero other tooling involved.

  A second, independent bug compounded the first: the guard added to `jj_workspace_smoke.sh`'s happy-path (Step 3, before `trap - EXIT`) was needed *in addition to* the trap-based cleanup, because the script explicitly disarms its `trap ... EXIT` on the success path after doing its own manual cleanup — so a guard installed only inside the `cleanup()` trap function silently never ran on the common (all-checks-pass) case. Found by writing the regression test below and watching it fail against the first fix attempt.

  **Fixed, two independent layers (both are worth doing — see below):**
  1. **`trading/devtools/checks/jj_workspace_smoke.sh` stops polluting the caller's real checkout.** Added `jj_ita_guard_snapshot` / `jj_ita_guard_restore` to `trading/devtools/checks/_check_lib.sh`: capture the repo's currently-intent-to-added path set (via `git status --porcelain=v1 | grep '^ A '` — NOT `git diff --cached --name-only`, which git deliberately blinds to intent-to-add entries; see the function's docstring for the porcelain-XY distinction between a real `git add` ("A ") and intent-to-add (" A") that makes this detectable at all) before running any `jj` command against `$REPO`, and unstage (`git reset --`) exactly the paths that are newly intent-to-add afterward — never touching anything the caller had staged before the guard started. Wired into both the happy-path Step 3 call site and the `cleanup()` trap, so both the success path and every `FAIL:`/signal exit path restore the caller's git state.
  2. **`dev/scripts/orchestrator_fastexit_gate.sh` `_current_summary_path` hardened independently, so the counter is immune regardless of what put an intent-to-add entry there** (belt and suspenders — a future caller of `jj` elsewhere in the suite shouldn't be able to reintroduce this class of bug). Switched its data source from `git ls-files` (lists index entries, intent-to-add included) to `git ls-tree -r --name-only HEAD` (reads committed history only, blind to index state — real staged-but-uncommitted `git add` and intent-to-add both excluded identically to a plain untracked file, which is the function's existing contract).

  **New regression coverage:**
  - `trading/devtools/checks/jj_ita_guard_check.sh` (new, wired into `dune runtest` via `trading/devtools/checks/dune`, `(universe)` dep on itself + `jj_workspace_smoke.sh`, skips cleanly when `jj` is not on PATH). Three hermetic-sandbox assertions, each in its OWN fresh `git init` + `jj git init --colocate` repo (reusing one sandbox across phases is unsound — once jj has snapshotted a file's content, a `git reset` on the git side doesn't un-teach jj that fact, and jj may skip re-exporting an unchanged file on a later call, producing a false-positive "fixed" reading for the wrong reason): (1) negative control — a bare unguarded `jj workspace add` really does turn an untracked `dev/daily/*.md` into ` A`, pinning the mechanism so a future jj version change that stops doing this is caught rather than silently mooting the test; (2) the real `jj_workspace_smoke.sh`, invoked via `REPO_ROOT` override, passes AND leaves the sandbox's untracked file untouched on the happy path; (3) same, but with no `main@origin` ref configured so Step 1 fails and only the trap-driven restore fires — proving the guard isn't reachable from only one call site. Verified this test genuinely catches the bug it's named for: reverting just the happy-path guard call turned assertion (2) red (`status-after=' A dev/daily/2099-01-01-test.md'`) while (1) and (3) stayed green, exactly pinpointing which call site regressed.
  - `dev/scripts/orchestrator_fastexit_gate_test.sh` Scenario 56b: a same-day `dev/daily/*.md` file staged via `git add -N` (the raw git primitive underlying what jj's export does) is not counted — would FAIL against the pre-fix `git ls-files`-based formula.

  **Verify:** `sh trading/devtools/checks/jj_ita_guard_check.sh`; `sh dev/scripts/orchestrator_fastexit_gate_test.sh` (89/89 checks, including new Scenario 56b); `dune build @fmt && dune build && dune runtest` all exit 0 (confirmed in this session: zero `^FAIL:` lines in the full runtest log, `posix_sh_check.sh` reports 132 scripts clean including the two new/changed files).

## Added 2026-09-25 (harness-maintainer, harness/file-length-scope-2876, issue #2876)

- [x] **H-FILE-LENGTH-LIB-SCOPE-BLIND-SPOT — widen `linter_file_length.sh`'s `.ml` scan repo-wide (issue #2876)**. The `.ml` loop scoped to `-path "*/lib/*.ml"` only, so any `.ml` file whose containing directory was not literally named `lib` — `bin/`, `scripts/<tool>/`, `devtools/<tool>/`, or a bare feature directory like `release_report/` or `scenarios/` — was invisible to the 300-soft / 500-hard limits, the same shape as the `.mli` blind spot the P0.1 fix closed. Nine files were measured over the hard limit with zero linter signal (surfaced during QC on #2875): `release_report.ml` 1005, `trade_audit_ratings.ml` 1001, `build_runner.ml` 771, `trade_audit_report.ml` 691, `scenario_runner.ml` 675, `sexp_default_drift_linter.ml` 674, `selection_trace.ml` 662, `shiller_weinstein_decades.ml` 626, `backtest_runner.ml` 522 — re-measured identical on this PR's base commit. **The issue's own suggested fix (widen to `*/bin/*.ml` + `*/scripts/*/*.ml`) does NOT close the gap**: of those nine, only three live under a `bin/` or `scripts/<tool>/` directory; the other six live in directories with arbitrary feature names no small allowlist would enumerate. Fixed instead as an EXCLUSION-based repo-wide scan (prune `_build/`, `.formatted/`, `*/test/*`), matching the pattern `fmt_check.sh` already uses for its own repo-wide `.ml`/`.mli` scan — the real invariant is "one `dune` stanza directory per module," not "modules live under `lib/`." The `.mli` loop is unchanged (`*/lib/*.mli` only; out of scope for this issue). Also added `linter_exceptions.conf` support to `linter_file_length.sh` (new `file_length` linter key, same substring-match convention as `linter_mli_coverage.sh`; excluded files are skipped before `TOTAL`/`LARGE_COUNT` accounting so an exception never subsidizes the `MAX_LARGE_PCT` cap).

  **21 files became newly visible** (9 over 500, 12 in the 300-500 soft-limit band); one (`list_active_exceptions.ml`, 499) already carried an `@large-module` marker and needed no action. Deliberate per-file disposition, per `.claude/rules/code-health-discipline.md` (extraction preferred, marker for genuine coordinators, dated exception as last resort — no `HARD_LIMIT`/`SOFT_LIMIT` bump): two devtools files I own that fit under the 500 hard limit (`nesting_linter.ml` 405, `cc_linter.ml` 337 — both AST-walking linters where the walk and the accounting are one coordinated pass) got an in-file `@large-module` marker with a written justification. The remaining 18 — 17 feature/analysis-owned files outside this agent's scope (`trading/trading/`, `analysis/`) plus `sexp_default_drift_linter.ml` (674 lines, over the hard limit even with a marker, and already carrying a `nesting` exception on the same "pinned byte-for-byte by tests" grounds) — got dated `linter_exceptions.conf` `file_length` entries (`review_at: 2026-11-30`, except `sexp_default_drift_linter.ml` aligned to the existing `2026-11-09` nesting exception) with per-file reasons plus a matching `dev/status/cleanup.md` §Backlog `file_length:` entry each, so a future feature-owned extraction pass has both the exception to retire and the backlog item to close. No extraction was attempted on any feature/analysis file — out of this agent's ownership scope per `CLAUDE.md`.

  **Test:** added fixtures F and G to `devtools/checks/linter_file_length_test.sh` (9 assertions total, up from 7): F — a 501-line `.ml` under `bin/` (not `lib/`) with no marker must FAIL, naming the file; G — a 501-line `.ml` under `test/` must still PASS (the pre-existing test-file exclusion, unchanged, now expressed as an explicit prune instead of an implicit non-match). **Mutation-verified by hand:** reverted the `.ml` loop's `find` pattern back to `-path "*/lib/*.ml"` — fixture F flipped from FAIL to a false `OK` (`0 declared-large of 0 total`), all other fixtures unaffected; reverted back, `9 passed, 0 failed` restored. Full-repo run post-fix: `OK: all *.ml files within limits (20 declared-large of 646 total); all lib/*.mli signatures within limits (0 declared-large of 478 total)` — the 20 = 18 pre-existing `@large-module` .ml markers + the 2 new devtools markers; well under the 11% cap (3.1%).

  `dev/status/cleanup.md`'s pre-existing `linter_coverage` entry (the sibling test-file-exemption policy gap) amended in place to note the `.ml` scope is now repo-wide and that entry's remaining open question is test-file policy specifically — left unchecked, since that policy decision is still open. Verify: `sh trading/devtools/checks/linter_file_length_test.sh` (9/9), `sh trading/devtools/checks/linter_file_length.sh` (repo-wide, exit 0), `dune build @fmt && dune build && dune runtest` all exit 0.

## Added 2026-09-25 (orchestrator run 36137000433, non-blocking QC follow-ups on #2956)

- [ ] H-ITA-GUARD-QUOTED-PATHS: `jj_ita_guard_snapshot`/`_restore` (`trading/devtools/checks/_check_lib.sh`) parse `git status --porcelain` with `cut -c4-`, so a path git C-quotes (space, quote, non-ASCII) is not matched and would be left polluted. Today's `dev/daily/*.md` / `dev/audit/*.json` names never hit it; switch to `--porcelain -z` if the guard is reused more widely. `harness_gap: NONE`. (source: qc-behavioral review 5320539648 on PR #2956)
- [ ] H-ITA-GUARD-SILENT-RESTORE-FAIL: the guard discards `git update-index --cacheinfo` / `git reset --` errors, so a failed restore still exits 0; print a named `WARN:` line on failure. `harness_gap: NONE`. (source: qc-behavioral review 5320539648 on PR #2956)
