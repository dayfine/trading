...
- [ ] H-AUDIT-COLLISION: `trading/devtools/checks/record_qc_audit.sh` — the output filename is keyed `<date>-<feature>.json`, so a **second QC of the same track on the same day silently clobbers the first**. Hit for real on 2026-07-27: run 2's record for `feat/picks-phase-c-v2` (#2114) overwrote run 1's for `feat/picks-phase-c` (#2105/#2107); recovered by hand and refiled as `2026-07-27-weekly-snapshot-run2.json`. Run 1 itself reviewed two PRs on this track and will have lost one record the same way. With 2-4 orchestrator runs per day this is a routine data loss, not an edge case. Fix: include the branch or a run counter in the filename, or refuse to overwrite an existing record.
