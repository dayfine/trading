...
- **Effect / seam**: `Weinstein_strategy_screening.screen_universe` computes the
  dawn-effective config just before `Entry_walk.entries_from_candidates` —
  swapping `initial_long_margin_req` → `dawn_initial_long_margin_req` on a dawn
  week only. That field's sole functional reader is the entry-walk buying-power
  ceiling (`Screening_notional.make_entry_walk_state`); all other margin mechanics
  (M1 rate, M2 maintenance/force-reduce, debit interest) are untouched. Deep index
  history is fetched via `Bar_reader.weekly_view_for` so the flip search sees past
  the default 52-week `lookback_bars` (armed weeks only).
- **Validation** (`Leverage_dawn.validate`, called at `Weinstein_strategy.make`):
  ...
  `dawn_initial_long_margin_req ∈ (0.0, 1.0]`; raises `Failure` otherwise. No-op
  when disabled.
- **Tests** (`test/test_leverage_dawn.ml`, 19 cases, all pass): flip-age on
  ...
  (dawn/not-dawn/disabled); `validate` (disabled no-op, enabled-requires-armed,
  armed-ok, req>1→raise, req=0→raise); R1 defaults-are-no-op + sexp round-trip;
  effect on a synthetic bar-reader dawn V-tape (levers to 0.75) vs decline tape
  (unchanged) vs disabled (unchanged).
- **WF spec (deliverable, NOT run)**:
  ...
  `Walk_forward.Spec.load` (7 variants: 6 cells + baseline; all axis keys resolve
  against the canonical config → R2 confirmed at the overlay surface).
