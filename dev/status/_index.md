...
Last updated: 2026-07-19 (orchestrator run 29701091765 — run-3 of 07-19: main GREEN, HEAD `6c0b46c3` (#2012 resistance-v2 promotion memo). Clean-tree local `dune build` exit 0 + `dune runtest` exit 0 (no FAIL/LAPACKE/SIGILL; fmt_check OK, magic_numbers OK), integrity exit 0, index_size exit 0; GHA CI on `6c0b46c3` build-and-test/perf-tier1/5y+custom goldens all success. Since run-2 (#2011): #2010 (margin M2 long-side force-reduce, default-off) MERGED + #2012 (resistance-v2 promotion memo, docs) MERGED — both maintainer LOCAL; reconciled margin-realism row (next M3–M4) + resistance-v2 row (Open PR #2013). **2 open PRs, both maintainer-authored active-LOCAL** — #2009 (`harness/bayesian-gp-nugget-escalation`, LAPACKE GP-Cholesky nugget fix, self-COMMENTED) + #2013 (`specs/bundle-studies`, resistance-v2 promotion-study spec bundle + priorities doc); orchestrator does NOT engage active-LOCAL per gha-local-coordination collision-avoidance. ZERO dispatches this run: every feature track deep-warehouse-data-gated [no EODHD_API_KEY] / human-gated / active-LOCAL; harness backlog otherwise done/milestone/PAT-gated (#1636 workflow-PAT); cleanup §Backlog 0 real `[ ]` (07-13 deep scan Action-required NO); ops-data no API key + data-gaps.md unchanged since #814. No `[drift]`, no still-real `[critical]`.)
...
|---|---|---|---|---|
| [resistance-v2](resistance-v2.md) | IN_PROGRESS | dayfine (maintainer LOCAL) | #2013 | grid 3/3 ACCEPT robust w=30; promotion memo #2012 MERGED; #2013 study-spec bundle open (LOCAL); R3 default-flip human-gated; WF-CV vs w30 data-gated |
| [margin-realism](margin-realism.md) | IN_PROGRESS | dayfine (maintainer LOCAL) | — | M1a/M1b-1/M1b-2(#2005)+M2 force-reduce (#2010) all MERGED default-off; next: M3–M4 (each default-off behind own flag; maintainer LOCAL) |
