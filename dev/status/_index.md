...
| [backtest-scale](backtest-scale.md) | IN_PROGRESS | feat-backtest | — | PRs #506 (missing-CSV tolerance) + #507 (strategy↔bar_loader integration) both merged 2026-04-22; Tiered path now actually throttles Bar_history + consumes Full-tier bars (was bookkeeping-only before #507). Nightly tiered-loader-ab cron live since 2026-04-22. Next: accumulate a few nights of nightly A/B runs (trade-count parity hard gate + PV drift inside warn threshold) before flipping `loader_strategy` default Legacy→Tiered in a ~20-line follow-up PR; then retire Legacy codepath. |
