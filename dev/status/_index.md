...
Last updated: 2026-04-25 (Stage 2 PR-C of `data-panels` opens on `feat/panels-stage02-pr-c-rs-analyze`: reshape `Rs.analyze` to accept `~get_stock_close`/`~get_benchmark_close`/`~get_date` indicator-callback inputs via new `analyze_with_callbacks`; existing bar-list `analyze` becomes a thin wrapper preserving byte-identical behavior, so all current callers keep working unchanged. 6 new parity tests cover positive/negative/near-zero/crossover/insufficient-data/exact-minimum cases. PR-B (Stage.classify) merged as #559. PR-C is the second of an eight-PR sequence (A–H per plan §Stage 2) progressively reshaping the strategy callees from bar-list to callback inputs; PR-H finally ports the 6 `Bar_history` reader sites and deletes `Bar_history`.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 2, PR-C in 8-PR sequence) | feat-backtest | feat/panels-stage02-pr-c-rs-analyze | Columnar data-shape redesign. Stages 0-1 + Stage 2 foundation merged (#555, #557, #558). PR-B (`Stage.classify`) merged as #559. **Stage 2 PR-C (this branch)**: reshape `Rs.analyze` to take `~get_stock_close`/`~get_benchmark_close`/`~get_date` indicator-callback inputs (`analyze_with_callbacks`); existing `analyze ~stock_bars ~benchmark_bars` becomes a thin wrapper preserving byte-identical behavior so all current callers keep working. 6 new parity tests across positive/negative/near-zero/crossover/insufficient-data/exact-minimum cases. Next: PRs D-G repeat the same recipe for Stock_analysis / Sector / Macro / Stops; PR-H ports the 6 `Bar_history` reader sites + deletes Bar_history. |
