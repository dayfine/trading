...
Last updated: 2026-07-08-run2 (orchestrator run 28971248840: **reconcile + health + PR-eval pass**. One open PR at run start — **#1890 `fix/warmup-364`** (maintainer dayfine, USER-APPROVED P0 warmup 210→364; actively in flight, updated 3 min pre-run). Evaluated, **not orchestrator-merged**: `build-and-test` CI is **RED on fmt_check only** (`optimal_strategy_runner.mli` needs `dune fmt` after the 364 docstring edit — diagnosed + posted as a non-interfering PR comment); branch also `behind`; QC gates unsatisfiable from GHA (no docker/jj); human-eyeball-gated maintainer P0 → LOCAL to finish. Main (this HEAD) **GREEN**: `dune build` / `dune runtest` / status-integrity all exit 0 (fmt_check OK, magic_numbers OK, no FAIL). No per-track row drift (only #1889 = run1 summary touched `dev/status/`, exempt). **No dispatch** — feature tracks LOCAL/data-gated (EODHD absent in GHA) or human-gated; harness T1 `[x]`, T3-H parked (low-value at ~0 PR throughput), T3-C superseded, T4 human-request-only, lone `[~]` ci.yml ENOSPC fix human-PAT-blocked (#1636); cleanup backlog drained; ops-data sentinel unchanged since #814. Prior: run1 (28960151036) reconcile+health.
