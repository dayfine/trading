...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 0 merged, awaiting human review for Stages 1+) | feat-backtest | — | Columnar data-shape redesign: replace per-symbol Hashtbl-of-bars with Bigarray panels (N × T per OHLCV field + per indicator). Collapses entire bar_loader tier system, post-#519 Friday cycle, and Bar_history. 5 stages over ~10 days. **Stage 0 spike merged 2026-04-25 as #555**: `Symbol_index`, `Ohlcv_panels`, `Ema_kernel`, `Panel_snapshot` under `trading/trading/data_panel/`; EMA parity bit-identical at N=100 T=252 P=50; snapshot round-trip bit-identical. Stages 1-5 blocked on human green-light per plan §Decision point. Projected RSS at N=5000 T=10y: ~1.2 GB (vs 12-22 GB today). Supersedes older `incremental-indicators` track (plan #551 marked SUPERSEDED, kept as historical record). |
