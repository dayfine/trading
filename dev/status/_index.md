...
Last updated: 2026-07-18 (orchestrator run 29631756603 — run-1 of 07-18: main GREEN — full `dune build` exit 0 + `dune runtest` exit 0 (no FAIL/LAPACKE/SIGILL; fmt/magic OK), integrity exit 0, index_size exit 0 (9816/20480). **2 open PRs** (fresh inflow from maintainer LOCAL session, both default-off): #1997 resistance virgin-crossing lever — QC ran, structural+behavioral APPROVED (score 5), CI green, **merge HELD** (author active LOCAL, shares `weinstein_strategy_screening.ml` with #1998 — avoid forcing mid-session rebase); #1998 margin M1b-1 leverage relaxation — CI in_progress + author iterating (pushed <30m), **QC deferred** to next run once stable. No feat/harness/cleanup/ops-data dispatch (tracks active-LOCAL / milestone/human/PAT-gated / no API key). Prior run-3 07-17 (29618154628): main GREEN — full `dune build` exit 0 + `dune runtest` exit 0, no FAIL, fmt/magic OK. **0 open PRs**; no inflow since run-2's summary (#1995 = HEAD). No dispatch. **CI finding:** HEAD `3a633f6d` (#1995, docs-only summary merge) showed `build-and-test=failure` from an intermittent `Failure("LAPACKE: 9")` GP-Cholesky non-PD in `test_bayesian_opt` — CONFIRMED FLAKE (base rate 4/4 prior commits green; local 6/6 + full runtest exit 0; DISTINCT from #1961 SIGILL, portable OWL_CFLAGS active). Logged new harness backlog `[ ]` (raise GP jitter). Every feat-* track deep-warehouse-data-gated / human-gated (M6.6) / active-LOCAL; harness milestone/human/PAT-gated; cleanup empty; ops-data blocked (no API key). Prior: run-2 (29597183213) no dispatch.)
...
|---|---|---|---|---|
| [resistance-v2](resistance-v2.md) | IN_PROGRESS | local-session (2026-07-17) | #1997 | Lever (a) virgin-crossing re-admission (default-off) QC-APPROVED (5)+CI green; merge HELD — author active LOCAL, shares strategy_screening.ml w/ #1998 |
| [margin-realism](margin-realism.md) | IN_PROGRESS | dayfine (maintainer LOCAL) | #1998 | M1b-1 entry-walk cash-gate leverage relaxation (default-off) — CI in_progress, author iterating LOCAL; QC deferred until stable |
