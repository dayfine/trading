...
Last updated: 2026-04-25 (Stage 2 PR-E of `data-panels` opens as PR #562 on `feat/panels-stage02-pr-e-sector-analyze`: reshape `Sector.analyze` with a callback path. Adds `Sector.analyze_with_callbacks ~callbacks` plus a `Sector.callbacks` bundle wrapping `Stage.callbacks` + `Rs.callbacks`. Sector reads no bar fields itself — it's a thin combinator delegating to `Stage.classify_with_callbacks` and `Rs.analyze_with_callbacks`, so the bundle just wraps the two nested callback records. Existing `Sector.analyze ~sector_bars ~benchmark_bars` is now a thin wrapper preserving byte-identical behavior. 4 new parity tests cover high-confidence Stage2 + strong RS / low-confidence Stage4 / mixed-stage constituents / insufficient bars. PR-D (Stock_analysis.analyze) merged as #561. PR-E is the fourth of an eight-PR sequence (A–H per plan §Stage 2); PRs F-G repeat the recipe for Macro / Stops, PR-H finally ports the 6 `Bar_history` reader sites and deletes `Bar_history`.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 2, PR-E in 8-PR sequence) | feat-backtest | #562 (feat/panels-stage02-pr-e-sector-analyze) | Columnar data-shape redesign. Stages 0-1 + Stage 2 foundation merged (#555, #557, #558). PR-B (`Stage.classify`) #559, PR-C (`Rs.analyze`) #560, PR-D (`Stock_analysis.analyze`) #561 — all merged. **Stage 2 PR-E (this branch, PR #562)**: reshape `Sector.analyze` with `analyze_with_callbacks ~callbacks` plus a `Sector.callbacks` bundle wrapping nested `Stage.callbacks` + `Rs.callbacks`. Sector itself reads no bar fields — it delegates Stage classification + RS computation to the corresponding callback APIs, then layers constituent breadth + composite confidence on the results. Existing `Sector.analyze ~sector_bars ~benchmark_bars` becomes a thin wrapper preserving byte-identical behavior. 4 new parity tests across high-confidence Stage2 + strong RS / low-confidence Stage4 / mixed-stage constituents / insufficient bars. Next: PRs F-G repeat the same recipe for Macro / Stops; PR-H ports the 6 `Bar_history` reader sites + deletes Bar_history + reshapes Volume/Resistance to drop the bars_for_volume_resistance parameter. |
