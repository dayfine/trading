...
Last updated: 2026-07-15 (orchestrator run 29445579168 — run-5: main GREEN at `8142d945`; no dispatch. Since run-4: **#1979 (PR-C supply-score), #1980 (status file + this row), #1982 (PR-B2 deep-history feed) all MERGED** — resolves run-4's "resistance-v2 lacks a status file/index row on main" [info]. One NEW maintainer-LOCAL PR opened on the fenced resistance-v2 track — **#1983** (`feat/resistance-v2-wiring`, PR-D screener wiring, default-off, feat). #1983 is maintainer-LOCAL (author dayfine; status file "Owner: local-session"; PR-D `[blocking: local session shepherds]`; row marked "do not dispatch") and the maintainer is actively shepherding it (own COMMENTED QC review) → HOLD per gha-local-coordination collision-avoidance (no dispatch/rework/QC-drive/merge). #1983 CI is GREEN (build-and-test + perf-tier1-smoke success) but it is a code PR with only a COMMENTED structural review and no behavioral APPROVED — not 3-gate green; the maintainer owns the merge. Live this run: `dune build` exit 0 + `dune runtest` exit 0 (fmt+magic-numbers OK, no FAIL) + status-integrity exit 0 + index-size exit 0. Main-HEAD GHA CI (`8142d945`) build-and-test + perf + both goldens = success. Every other track deep-warehouse-data-gated (EODHD_API_KEY absent) / human-gated (M6.6) / active-LOCAL. Prior: run-4 (29421382522) no dispatch; run-3 (29409957763) no dispatch; run-2 (29399168574) #1975 QC'd.)
...
|---|---|---|---|---|
| [resistance-v2](resistance-v2.md) | IN_PROGRESS | local-session (2026-07-15) | #1983 | PR-C #1979 + PR-B2 #1982 MERGED; PR-D screener wiring in flight (#1983, default-off); next PR-E WF-CV — in flight locally, do not dispatch |
