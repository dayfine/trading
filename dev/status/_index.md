...
Last updated: 2026-04-25 (Stage 3 PR 3.1 of `data-panels` opens on `feat/panels-stage03-pr-a-wire-and-gate` as PR #567: wires `~bar_panels` into `Panel_runner._build_strategy` and replaces the Tiered-vs-Panel `test_panel_loader_parity` with a Panel-mode round_trips golden gate. Stage 2 PR-H merged as #565. Stage 3 PRs 3.2–3.4 follow.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 3 PR 3.1 READY_FOR_REVIEW) | feat-backtest | #567 (Stage 3 PR 3.1) | Columnar data-shape redesign. Stages 0-1 + Stage 2 foundation merged (#555, #557, #558). Stage 2 PRs B–H all merged (#559 / #560 / #561 / #562 / #563 / #564 / #565). **Stage 3 PR 3.1 (this branch)**: wires `~bar_panels` into `Panel_runner._build_strategy` so Panel mode actually reads from the panel columns instead of `Bar_history`. Replaces the (vacuous) Tiered-vs-Panel `test_panel_loader_parity` with a Panel-mode round_trips golden test — sexp bit-equality against checked-in goldens for two scenarios (`smoke/tiered-loader-parity.sexp` and a new full-window `smoke/panel-golden-2019-full.sexp`). Tiered-mode behaviour is unchanged and still pinned by `test_tiered_loader_parity`. Stage 3 PRs 3.2–3.4 follow: delete Bar_history, delete the Tiered runner + Friday cycle + Bar_loader, then delete the Legacy runner. |
