...
Last updated: 2026-06-27 run 1 (orchestrator run 28275182617: ACTIVE RE-QC PASS — **2 QC subagents dispatched, 0 merges**. Maintainer opened **PR #1760** (`feat/liquidity-realism-overlay`, ready) after run 2 — a default-off liquidity-realism overlay (held-degradation exit + entry gate) addressing the ELCO illiquid-short −48% NAV crash from the short-realism forensics (#1759). Prior local QC at `573bd3f3` was structural-APPROVED + behavioral-NEEDS_REWORK (force-liq skip-guard gap); the maintainer pushed a rework to tip `44e1811b`. Per Step 1.5 (tip≠reviewed SHA) the orchestrator re-QC'd at `44e1811b`: **structural re-APPROVED (8/10)**, **behavioral re-NEEDS_REWORK (3/10)** — the force-liq fix is correct + well-tested, BUT the skip-set restructuring *regressed* Stage-3/laggard collision coverage (old skip set `stop∪stage3∪laggard` → new `force_liq∪stop`; `assemble_output` concatenates exit channels with no per-position dedup, so a Stage-3/laggard-exiting illiquid name now gets a duplicate `TriggerExit`), and the docstring misstates `force_exit_ts` contents. Verdict + exact fix posted as PR review comments; audit `dev/audit/2026-06-27-liquidity-realism-overlay.json`. **Not merged** (gate fails on NEEDS_REWORK; PR also `behind` main + `build-and-test` in-progress). **Not auto-reworked** — maintainer's actively-worked branch (collision-avoidance); fix is mechanical (union all four exited-id sets + fix docstring + add stage3/laggard collision test). All other tracks gated/blocked (data/human/LOCAL); 0 other open PRs; main CI GREEN on `014db4ca`; 0 open ci-red issues. Prior: 2026-06-26 run 2 (28226637122) ACTIVE VERIFICATION PASS, 0 subagents — investigated stale P0 plan #1757, no dispatch.)
...
| [short-side-strategy](short-side-strategy.md) | IN_PROGRESS | feat-weinstein | #1760 (NEEDS_REWORK) | #1760 liquidity overlay re-QC NEEDS_REWORK (stage3/laggard skip regression); #1659 short-sleeve MERGED; then sleeve screen |
