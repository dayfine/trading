...
Last updated: 2026-06-20 (orchestrator run 27861908373 [run 2]: NO-DISPATCH full pass. Since run 1, one new open PR — #1662 `feat/vol-scaled-stop` (maintainer, default-off ATR stop-distance floor), CI `build-and-test` in_progress + `mergeable_state: behind`, last touched ~13s before the PR check ⇒ maintainer actively mid-session; diagnosed + escalated, NOT raced/QC'd/merged (per gha-local-coordination + #1659 precedent). #1662 has no dedicated track status file (base-strategy stop-quality lever), so no row is invented; it will get QC'd next run if still open + green, or the maintainer self-merges. No `dev/status/*.md` changed since the run-1 summary commit `1db67325`, so all rows below are unchanged from run 1. 0 open ci-red issues. Main CI GREEN on HEAD `1db67325` + `f245b1fe` (build-and-test + perf-tier1-smoke + golden-sp500-5y + golden-custom-universe SUCCESS). Health CLEAN (status-integrity + index-size exit 0). Nothing GHA-dispatchable: every feature track data-/human-/LOCAL-gated or parked; harness lone `[~]` ci.yml ENOSPC fix workflow-PAT-blocked; cleanup §Backlog only the template placeholder open; ops-data needs EODHD key (absent in GHA) + data-gaps.md unchanged since 2026-05-03. 0 subagents.)
