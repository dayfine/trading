...
Last updated: 2026-04-25 (Stage 3 PR 3.2 of `data-panels` opens on `feat/panels-stage03-pr-b-delete-bar-history`: deletes the parallel `Bar_history` Hashtbl cache, the `Bar_reader.History` backend, and the Tiered Friday-seed step. `Bar_reader.t` collapses to a thin alias over `Bar_panels.t`. Stage 3 PR 3.1 merged as #567. Stage 3 PRs 3.3–3.4 follow.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 3 PR 3.2 READY_FOR_REVIEW) | feat-backtest | (Stage 3 PR 3.2 on `feat/panels-stage03-pr-b-delete-bar-history`) | Columnar data-shape redesign. Stages 0-1 + Stage 2 foundation merged (#555, #557, #558). Stage 2 PRs B–H all merged (#559–#565). Stage 3 PR 3.1 merged as #567. **Stage 3 PR 3.2 (this branch)**: deletes `Bar_history` (`bar_history.ml`/`mli` + tests, ~750 LOC removed), `Bar_reader.History` backend + `accumulate` no-op, the `Tiered_strategy_wrapper`'s Friday-seed step, and the `bar_history`/`seed_warmup_start` fields from its config. `Bar_reader.t` collapses to a thin alias over `Bar_panels.t`. Both Tiered and Legacy runners now build `Bar_panels` at simulator-construction time and pass to `Weinstein_strategy.make ~bar_panels`. `test_weinstein_backtest`'s pinned trade-counts relaxed to structural invariants (the bar-visibility-timing shift documented in PR 3.1's pre-flag is now realized). Stage 3 PRs 3.3–3.4 follow: delete Tiered runner + Friday cycle + Bar_loader, then delete Legacy runner. |
