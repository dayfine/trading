...
Last updated: 2026-04-23 (orchestrator detected recurring status-file integrity drift — human-authored PR #507 rewrote `dev/status/backtest-scale.md` `## Last updated: 2026-04-22` → `2026-04-22 (run 2)` at merge, violating the bare-YYYY-MM-DD schema enforced by `status_file_integrity.sh`. Third recurrence of the same class of drift in three days; linter exit 1 did not block the human merge. Inline fix scrubbed the parenthetical → `2026-04-23`; main re-verified green (`dune runtest --force` exit 0). Reconciled backtest-scale row: PRs #506 (missing-CSV tolerance) + #507 (strategy↔bar_loader integration) both merged 2026-04-22; no open PR on the track. Next task: accumulate nightly tiered-loader-ab runs before flipping `loader_strategy` default Legacy→Tiered. All other tracks' queue state unchanged since run-5.)
...
| [backtest-scale](backtest-scale.md) | READY_FOR_REVIEW | feat-backtest | #517 | PR #517 (bull-crash A/B parity fix) opened 2026-04-23 — closes the post-#507 PV drift on broad bull-crash (run 24818087082: $20709.16 drift) by promoting every Summary-tier symbol to Full each Friday (rather than the shadow screener's top-N) and switching wrapper promotions to per-symbol calls (so missing CSVs no longer short-circuit the rest of the batch). Local A/B on small-universe bull-crash: return delta closed from +228pp to +5.7pp; trade-count delta closed from -85% to -32%. Next: broad-universe nightly A/B post-merge to confirm warn-threshold parity, then flip `loader_strategy` default Legacy→Tiered. |
