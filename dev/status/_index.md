...
| [optimal-strategy](optimal-strategy.md) | READY_FOR_REVIEW | feat-backtest | feat/optimal-strategy-pr3 | PR-1 (#652) and PR-2 (#659) both merged 2026-04-28. PR-3 (`Optimal_portfolio_filler` + `Optimal_summary`) implemented this run on `feat/optimal-strategy-pr3` — heuristic A only (earliest-Friday + R-descending), 15 OUnit2 cases (10 filler + 5 summary), 1,073 LOC total (315 lib impl + 145 mli + 575 tests). All filler stress-tests pass (concurrent cap, sector cap, cash exhaustion, R-desc ordering, end-of-run, skip-already-held, cash recycling). Heuristics B (knapsack) and C (Monte-Carlo) deferred to PR-5 per plan §Phase C. Next: PR-4 (`Optimal_strategy_report` markdown renderer + `optimal_strategy.exe` binary, ~400 LOC) per plan §Phase D once PR-3 lands. |
