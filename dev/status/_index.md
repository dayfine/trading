...
Last updated: 2026-07-13 (orchestrator run 29256070034 — run-2, reconcile only: 0 open PRs, nothing dispatchable (all tracks data-gated / human-gated / LOCAL-fenced). Main build GREEN (`dune build` exit 0); `dune runtest` exit 1 is a dune sandbox-race in a devtools-check `find` (`.../_build/.sandbox/<hash>: No such file or directory`; 0 `FAIL:` lines, all linters print OK) — reproduced on main-HEAD CI (build-and-test) AND locally = infra flake, not a real red. Since run-1 (#1948): only #1949 landed — rename-twin-dedup dedup warehouse rebuilt + 28y record re-run (`-twin-basis returns`, 83 groups/91 legs dropped, all 10 known groups caught) → row reconciled (rebuild now DONE). Prior run-1 (29226291946) reconciled #1945 (index) / #1946 (twin-detector v2) / #1947 (validator audit-join-by-position_id). Earlier C1-C4 correctness-sweep history (#1939-#1942) is in the per-track status files.)
...
| [extension-stop](extension-stop.md) | IN_PROGRESS | dayfine (maintainer LOCAL) | — | MERGED #1934 (default-off, 3-gate auto-merge); next: insurance-basis acceptance audit — armed-vs-off deep-warehouse (LOCAL, non-blocking) |
