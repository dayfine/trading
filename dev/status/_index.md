...
Last updated: 2026-07-03 (orchestrator run 28665255259, run 5: **NO-DISPATCH full pass** — 0 open PRs. Since run 4, `#1840` (`docs(experiments): scale-in v1 surface REJECT`, maintainer `dayfine`) **MERGED 13:47:15Z** (HEAD `af2d47b4`): a formal two-cell ledger REJECT for scale-in v1 — Cell A SP500 (scale-in is a fat-tail tax, Sharpe 0.92→0.78) + Cell B top-3000 (the regime-diverse deep re-test run 4 flagged; return-flat, mild risk-smoothing that fails deflation) — both FAIL the gate → mechanism keeps default-off axis status, no promotion. Reconciled the scale-in status file (§Empirical status now records both cells + the ledger REJECT + transferable why) and this row (next: only an un-taxed press-the-winner *fresh* surface, data-gated). No GHA-dispatchable work: feature tracks data/human/LOCAL-gated (EODHD key absent); harness `[~]` workflow-PAT-blocked, rest milestone/human-gated; cleanup drained; ops-data sentinel-skip (data-gaps unchanged since 2026-05-03). Main GREEN (HEAD `af2d47b4` CI + perf-tier1 + custom golden SUCCESS; 5y golden + orchestrator in_progress). 0 open ci-red. Prior: run 4 (28656836722) reconcile #1837; run 3 (28648183235) NO-DISPATCH; run 2 (28640711416) NO-DISPATCH; run 1 (28633423533) DISPATCH — auto-merged #1831/#1832.
...
|---|---|---|---|---|
| [capital-management-scale-in](capital-management-scale-in.md) | IN_PROGRESS | dayfine | — | v1 default-off; 2-cell WF-CV REJECTED (ledger #1840, half-sizing=fat-tail tax); only path: un-taxed press-winner fresh surface (data-gated) |
