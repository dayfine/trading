...
Last updated: 2026-04-29 run-1 (orchestrator reconcile — between 2026-04-28 run-2 (13:30Z) and today: 5 PRs merged (#661 docs perf workflow removal, #662 split-day PR-2, #663 optimal-strategy PR-3, #664 split-day PR-3, #665 optimal-strategy PR-4 partial). This run dispatched two feat-backtest agents in parallel: (a) optimal-strategy PR-4b (`optimal_strategy.exe` binary) → PR #666 (combined QC APPROVED quality 4 by orchestrator override; structural verdict was NEEDS_REWORK on advisory-linter false-positive H3 + bin file_length, same recurring pattern as PR-1; orchestrator pushed `fix(status): drop parenthetical from 'Last updated'` commit 0efdb3c on the PR branch to clear the one real status_file_integrity finding; build+fmt+runtest exit 0 verified on the new tip); (b) simulation split-day PR-4 (broker-model verification + decisions promotion + status updates) → PR #667 (combined QC APPROVED quality 4; pure verification PR; smoke parity goldens bit-identical pre/post; sp500 baseline rerun deferred — universe-data missing in GHA; documented as maintainer follow-up). Index row deltas: optimal-strategy open PR feat/optimal-strategy-pr4 → feat/optimal-strategy-pr4b (PR #666); simulation open PR feat/split-day-pr3 → feat/split-day-pr4 (PR #667). Carried forward from run-2: A2 architecture rule still stale; qc-structural agent occasionally produces H3 false-positives on advisory linter text; bin-file refactor candidate for cleanup backlog.)
...
| [optimal-strategy](optimal-strategy.md) | IN_PROGRESS | feat-backtest | feat/optimal-strategy-pr4b (PR #666) | PR-1 (#652), PR-2 (#659), PR-3 (#663), PR-4 (#665) all merged 2026-04-28→29. PR-4b (`optimal_strategy.exe` binary scaffold) opened today as PR #666 — combined QC APPROVED quality 4 by orchestrator override (structural NEEDS_REWORK was H3 false-positive on advisory linter text + bin file_length advisory; the one real status-file-format finding was fixed in orchestrator commit 0efdb3c on the PR branch; build+fmt+runtest exit 0). Bin pipeline: `Stage_transition_scanner → Outcome_scorer → Optimal_portfolio_filler ×2 → Optimal_summary ×2 → Optimal_strategy_report.render`. Macro-trend collapse to `Neutral` documented in bin docstring (Constrained ≡ Relaxed_macro until per-Friday macro state persisted). Awaiting human merge. Next: PR-5 (optional release_perf_report integration, ~200 LOC) + Follow-up B (fuller renderer fixture tests). |
...
| [simulation](simulation.md) | IN_PROGRESS | feat-backtest | feat/split-day-pr4 (PR #667) | Split-day OHLC redesign (`dev/plans/split-day-ohlc-redesign-2026-04-28.md`). PR-1 (#658), PR-2 (#662), PR-3 (#664) all merged. PR-4 (verification + decisions promotion + status updates) opened today as PR #667 — combined QC APPROVED quality 4 (pure verification PR; smoke parity goldens `panel-golden-2019-full` and `tiered-loader-parity` bit-identical pre/post broker-model; `test_split_day_mtm` 3/3 PASS; full `dune build && dune runtest` exit 0; sp500-2019-2023 baseline rerun deferred — only ~12/491 universe symbols available in GHA fixture; documented as maintainer-local-run follow-up with reproduction recipe in `dev/notes/split-day-broker-model-verification-2026-04-29.md`). Decisions promoted: broker-model in `dev/decisions.md` Direction Changes (raw OHLC for execution; adjusted only for back-rolled smoothness; ledger-event approach to splits). Awaiting human merge. After merge: split-day work wraps; track stays IN_PROGRESS for the catch-all M5 walk-forward + tuner items. |
