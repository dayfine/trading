...
Last updated: 2026-04-25 (Stage 0 of `data-panels` columnar redesign LANDED as #555: `Symbol_index`, `Ohlcv_panels`, `Ema_kernel`, `Panel_snapshot` primitives under `trading/trading/data_panel/`. EMA parity bit-identical at N=100 T=252 P=50; snapshot round-trip bit-identical for single + multi-panel cases. QC structural + behavioral both APPROVED (3 Stage-1 pre-flags non-blocking: load_from_csv calendar-awareness, dump-twice byte-equality, unrounded-EMA → stage.ml boundary parity check). Stages 1-5 blocked on human review of spike result per plan §Decision point.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 1 READY_FOR_REVIEW; Stages 2-5 still gated) | feat-backtest | feat/panels-stage01-get-indicator | Columnar data-shape redesign: replace per-symbol Hashtbl-of-bars with Bigarray panels (N × T per OHLCV field + per indicator). 5 stages. **Stage 0 spike merged 2026-04-25 as #555**. **Stage 1 (this branch)** adds `Sma_kernel`, `Atr_kernel` (Wilder), `Rsi_kernel` (Wilder), `Indicator_spec`, `Indicator_panels` registry, `Get_indicator_adapter`, calendar-aware `Ohlcv_panels.load_from_csv_calendar`, and a Panel `loader_strategy` branch with `Panel_runner` + `Panel_strategy_wrapper`. SMA/ATR/RSI parity bit-identical (max_ulp=0) against scalar references at N=50–100 T=252. Calendar-aware load test pins date-aligned column placement. Tiered-vs-Panel integration parity gate (`test_panel_loader_parity`) holds: PV diff $0.0000 on the 7-symbol bull-2019h2 fixture. `Bar_history` left alive per Stage 1 invariant (Stage 2 deletes it). Projected RSS at N=5000 T=10y: ~1.2 GB (vs 12-22 GB today). |
