...
Last updated: 2026-07-01 (local session: **P0 faithfulness decision-audit BUILT + MERGED** — PR #1799, new `decision-audit` track. Phase-0 enriches `Trade_audit.alternative_candidate` with decision-time features (stage/weeks_advancing/rs_value/volume_ratio/sector/score_components) so cash-rejected near-misses are comparable to funded entries; Phase-1 adds `trading/trading/backtest/decision_audit/` lib+bin+tests rendering per-screen funded-vs-near-miss faithfulness roll-up + inversion flag. Additive, default-off read-only lens; qc-structural APPROVED + qc-behavioral APPROVED (5/5) + CI green (fixed a status-file-integrity lint miss first). Closed stale ci-red #1772. Next: run the report on a real CSV-mode `trade_audit.sexp`. Prior: 2026-06-30 run 2 (orchestrator run 28472005371: **1 QC + AUTO-MERGE**. Single open PR #1795 (maintainer dayfine, `feat/screener-tiebreak-controls`) run through the 3-gate pipeline: qc-structural APPROVED + qc-behavioral APPROVED (quality 5) + CI green → auto-merged squash → main `8bb0af20`; branch deleted. PR adds 3 **default-off DIAGNOSTIC** `candidate_ranking` control modes (`Reverse_alphabetical`, `Symbol_length`, `Hash_order` FNV-1a) to bracket the equal-score tiebreak noise floor; bit-identical (default stays `Alphabetical`). Result is **REJECT for default-flip** (ledger `2026-06-30-tiebreak-noise-floor.sexp`): no informative sort escapes the arbitrary-control noise band — RS-primary (#1788) + earliness (#1793) both sit inside it; "best" sort flips by cell = selection variance, not signal. FOURTH confirmation no equal-score tiebreak adds return (`edge_is_the_fat_tail`). Audit: `dev/audit/2026-06-30-screener-tiebreak-controls.json`. All other tracks data/human/LOCAL-gated; ops-data/harness/code-health skipped (data-gaps unchanged since 2026-05-03, harness T1 all `[x]`/lone `[~]` workflow-PAT-blocked, cleanup backlog drained). status-integrity + index-size + no-python exit 0 live. Stale ci-red **#1772 still STALE** (main green; token lacks `issues:write`). NOTE drift carried: data-foundations.md per-track heading READY_FOR_REVIEW but 0 open PRs (stale; #1790 merged). Prior: 2026-06-30 run 1 (28430953579) QC+AUTO-MERGE (#1793/#1794); 2026-06-29 run 4 NO-DISPATCH (#1792); runs 1–3 NO-DISPATCH.
