...
Last updated: 2026-07-15 (orchestrator run 29421382522 — run-4: main GREEN (code) at `8c970770`; no dispatch. Since run-3: **#1975 MERGED** (resistance-v2 sketch columns PR-B — schema-hash bump, warehouses now stale); two NEW maintainer-LOCAL PRs opened on the fenced resistance-v2 track — **#1979** (supply-score module PR-C, feat) and **#1980** (adds `dev/status/resistance-v2.md` + this index row, docs). Both are maintainer-LOCAL (status file: "Owner: local-session"; next steps `[blocking: local session shepherds]`) → HOLD per gha-local-coordination collision-avoidance (no dispatch, no rework, no merge). Both currently CI-red at real `dune runtest` (step #7, NOT infra): #1979 fails the test suite on the new module; #1980 fails `status_file_integrity` because the new `resistance-v2.md` uses inline `**Status:**` instead of the required `## Status` / `## Last updated:` / `## Interface stable` headings — maintainer to fix LOCAL. Live: `dune build` exit 0 + `dune runtest` exit 0 + status-integrity exit 0 + index-size exit 0 (9628/20480). Run-3's main-HEAD `build-and-test` false-red (on `d8bfabb3`) RESOLVED — new HEAD `8c970770` GHA `build-and-test` = success. resistance-v2 track still has no index row on main (#1980 adds it); orchestrator does not duplicate. Every other track deep-warehouse-data-gated (EODHD_API_KEY absent) / human-gated (M6.6) / active-LOCAL. Prior: run-3 (29409957763) no dispatch; run-2 (29399168574) #1975 QC'd; run-1 (29382905475) merged #1972.)
...
|---|---|---|---|---|
| [resistance-v2](resistance-v2.md) | IN_PROGRESS | local-session (2026-07-15) | #1979 | PR-B merged (#1975); PR-C in QC (#1979); next: PR-D wiring (design in track file) — in flight locally, do not dispatch |
