...
Last updated: 2026-07-04 (orchestrator run 28717547657, run 1: **NO-DISPATCH full pass** — 1 open PR. Since run 6, maintainer landed `#1846` (docs correction: retracts #1843's add-channel root cause — adds DO fill; real bug is a **sibling round-trip chimera** in `Metrics.extract_round_trips`) and opened `#1847` (`fix/sibling-round-trip-pairing`, ready): position-faithful FIFO/quantity round-trip pairing for sibling positions. `#1847` is qc-behavioral-**APPROVED** (19:26Z, score 4) but **CI-red** — `FAIL: nesting linter` on `metrics.ml:158 _pair_trades_for_symbol` (max 6 > 5), a real fail that landed at 19:30Z, after the approval — and **behind main**. NOT merged (CI hard-stop) and NOT reworked (fresh maintainer-in-flight thread; per run-3 precedent + gha-local-coordination the orchestrator surfaced the exact blocker+fix on the PR, comment 4883620797, rather than racing). Once nesting fix → CI green + branch updated, orchestrator runs qc-structural + 3-gate auto-merges. Reconciled the simulation status file + this row. No other GHA-dispatchable work: feature tracks data/human/LOCAL-gated (EODHD key absent); harness `[~]` workflow-PAT-blocked, rest milestone/human-gated; cleanup drained; ops-data sentinel-skip (data-gaps unchanged since 2026-05-03). Main GREEN (HEAD `081545d5` CI + perf-tier1 + both goldens SUCCESS). 0 open ci-red. Prior: run 6 (28680604616) reconcile #1843/#1844; run 5 (28665255259) reconcile #1840; run 1 (28633423533) DISPATCH — auto-merged #1831/#1832.
...
| [simulation](simulation.md) | IN_PROGRESS | feat-backtest | #1847 | #1847 sibling round-trip pairing fix: behavioral-APPROVED, CI-red (nesting linter) + behind main; then stale-exit grid via WF-CV (data-gated) |
