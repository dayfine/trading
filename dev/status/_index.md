...
Last updated: 2026-04-25 (Stage 2 PR-B of `data-panels` opens on `feat/panels-stage02-pr-b-stage-classify`: reshape `Stage.classify` to accept `~get_ma`/`~get_close` indicator-callback inputs via new `classify_with_callbacks`; existing bar-list `classify` becomes a thin wrapper preserving byte-identical behavior, so all current callers keep working unchanged. 6 new parity tests cover all four Stage variants + late-Stage-2 + insufficient-data. PR-B is the first of an eight-PR sequence (A–H per plan §Stage 2) progressively reshaping the strategy callees from bar-list to callback inputs; PR-H finally ports the 6 `Bar_history` reader sites and deletes `Bar_history`.)
...
| [data-panels](data-panels.md) | IN_PROGRESS (Stage 2, PR-B in 8-PR sequence) | feat-backtest | feat/panels-stage02-pr-b-stage-classify | Columnar data-shape redesign. Stages 0-1 + Stage 2 foundation merged (#555, #557, #558). **Stage 2 PR-B (this branch)**: reshape `Stage.classify` to take `~get_ma`/`~get_close` indicator-callback inputs (`classify_with_callbacks`); existing `classify ~bars` becomes a thin wrapper preserving byte-identical behavior so all current callers keep working. 6 new parity tests across all four Stage variants + late-Stage-2 + insufficient-data. Next: PRs C-G repeat the same recipe for Rs / Stock_analysis / Sector / Macro / Stops; PR-H ports the 6 `Bar_history` reader sites + deletes Bar_history. |
