...
Last updated: 2026-07-14 (orchestrator run 29299867815 — run-1: MERGED 3 PRs. #1961 (`b7628dcb`) pins owl to portable SIMD flags in `.devcontainer/Dockerfile` (fix option (d), no PAT) — auto-triggered `image.yml` rebuild (run 29300387751) of `trading-ci:latest`, killing the #1955 owl/OpenBLAS AVX-512 SIGILL runner lottery (RESOLVED; harness item closed). Docs-only #1951 (weekly deep-scan; run-3 HOLD lifted — CI green) and #1959 (armed-run matrix + 07-14 priorities) merged (CI-only gate). #1960 (arming P0: extension_stop insurance-ACCEPT + reject_declining_ma + live config-overrides) left OPEN — maintainer "on sign-off" gate + CI red on owl until the rebuilt image propagates. Main `dune build` GREEN (exit 0); `dune runtest` deferred (this runner is non-AVX512 → owl SIGILL false-red until the #1961 image is live). No feature/harness/cleanup/ops dispatch — all tracks deep-warehouse-data-gated / human-gated / active-LOCAL.)
...
| [extension-stop](extension-stop.md) | IN_PROGRESS | dayfine (maintainer LOCAL) | #1960 | insurance-ACCEPT + arming package OPEN in #1960 (extension_stop 2.0/0.25); awaiting maintainer sign-off + post-#1961 CI |
