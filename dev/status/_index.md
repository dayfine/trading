...
Last updated: 2026-04-25 (Strategic redirect: `incremental-indicators` track replaced by `data-panels` (PR #554, ratified 2026-04-25). The columnar-data-shape redesign uses Bigarray panels (N × T per OHLCV field + per indicator) to collapse the +95% Tiered RSS gap structurally — the entire bar_loader tier system, post-#519 Friday cycle, and Bar_history all disappear, projecting ~1.2 GB at N=5000 T=10y vs 12-22 GB extrapolated today. PR #551's incremental plan marked SUPERSEDED but kept as historical record + indicator audit. Sibling: `backtest-perf` (PR #550) tier-4 criteria now feasible once data-panels stages 0-3 land.)
...
| [data-panels](data-panels.md) | PENDING | feat-backtest | #554 (plan, ratified) | Columnar data-shape redesign: replace per-symbol Hashtbl-of-bars with Bigarray panels (N × T per OHLCV field + per indicator). Collapses entire bar_loader tier system, post-#519 Friday cycle, and Bar_history. 5 stages over ~10 days. Stage 0 is the spike (EMA panel + parity test + snapshot serialization, ~450 LOC). Projected RSS at N=5000 T=10y: ~1.2 GB (vs 12-22 GB today). Supersedes the older `incremental-indicators` track (plan #551 marked SUPERSEDED, kept as historical record). |
