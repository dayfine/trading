...
Last updated: 2026-08-06 run1 (orchestrator run 31083470712 -- main green by direct measurement at `444f4163`: `dune build` exit 0, `dune runtest` exit 0, zero `FAIL:` lines, status-integrity + index-size linters OK). **Step 0.5 did NOT fast-exit: the queue was EMPTY (0 open PRs)** -- the non-empty-queue precondition from #2180 firing correctly for the second run running. Shipped **2 merges, both through all three gates**, with **2 rework iterations** absorbed intra-run: **#2220** (F5 split-safe fallback telemetry -- the whole-window fallback was silent, so a walk-forward surface over the `split_safe_floors` axis could be substantially **inert** with no way to measure how much; a stated promotion blocker) and **#2221** (H-AUDIT-HOOK-GATE-TRUTHY -- both `write_audit.sh` test hooks gated on `[ -n ... ]`, so `VAR=0` **fired** them). Both reworks came from qc-behavioral catching a **claim that outran its evidence**: on #2220 an advertised mutation ("MT2 -> 3/2/1") did not reproduce and was **retracted** rather than propped up with a manufactured test; on #2221 an `ACCEPTED SPELLING:` block quantified over seven values while the tests pinned two, and a plausible widening put the original published-record bug back with the suite green.
...
| [support-floor-stops](support-floor-stops.md) | IN_PROGRESS | feat-weinstein | — | F5 fallback telemetry MERGED #2220 `480a59b7`; next: F3 raw-vs-adjusted in trade_audit_report_bin (needs golden re-pin), then F5's F6 render |
...
| [harness](harness.md) | IN_PROGRESS | harness-maintainer | — | H-AUDIT-HOOK-GATE-TRUTHY + FAILS-OPEN-WORDING MERGED `444f4163` (#2221); next: H-REWORK-STREAK-ESCALATION-UNTESTED or H-AUDIT-TEST-DISABLE-COUNT-TAUTOLOGICAL |
