...
# re-run to confirm they flip to FAIL. See the harness-maintainer session
# report for the mutation list; not shipped as a permanent mutation-coverage
# harness (that heavier machinery, per H-GATEPARSER-NO-MUTATION-COVERAGE,
# exists today only for pr_gate_status.sh).
set -eu
...
_reset_mock_env() {
  unset MOCK_CREATE_FAIL MOCK_CREATE_HTTP_CODE MOCK_CREATE_PR_NUMBER \
    MOCK_CREATE_PR_URL MOCK_LOOKUP_FAIL MOCK_LOOKUP_PR_NUMBER \
    MOCK_LOOKUP_PR_NUMBER_FROM_CALL 2>/dev/null || true
...
_pushed=$(git --git-dir="$BARE_DIR" branch --list ops/daily-2026-09-08)
check_contains "the branch actually landed on the bare remote" "$_pushed" "ops/daily-2026-09-08"
_reset_mock_env
...
_pushed=$(git --git-dir="$BARE_DIR" branch --list ops/daily-2026-09-08)
check_contains "422 fallback still landed the push before falling back" "$_pushed" "ops/daily-2026-09-08"
_reset_mock_env
...
_pushed=$(git --git-dir="$BARE_DIR" branch --list ops/daily-2026-09-08)
check_contains "create-failure scenario still really pushed the branch" "$_pushed" "ops/daily-2026-09-08"
_reset_mock_env
...
# =============================================================================
# Scenario group 8: CLI argument hygiene
# =============================================================================
rc=0
