...
awk '/# REST publication:/ {copy=1} copy {sub(/^          /, ""); print}' \
  "$ROOT/.github/workflows/prune-candidates-weekly.yml" > "$TMP/fragment.sh"
...
DATE=2026-09-16 BRANCH=ops/prune-candidates-2026-09-16
...
check jq -e --arg body "$BODY" --arg head "$BRANCH" --arg title "ops(health): prune-candidates report $DATE" \
