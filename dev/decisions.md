...
**DONE — `feat/weinstein-split-day-stop-adjustment`**: Adds `Weinstein_stops.Stop_split_adjust.scale` (pure helper; divides every absolute price field of a `stop_state` by the split factor — `Initial.{stop_level, reference_level}`, `Trailing.{stop_level, last_correction_extreme, last_trend_extreme, ma_at_last_adjustment}`, `Tightened.{stop_level, last_correction_extreme}`; non-price fields like `correction_count` and `reason` pass through). Adds `Weinstein_strategy.Stops_split_runner.adjust` which, for every held position, reads the prior + current daily bars from the panel-backed `Bar_reader`, runs `Types.Split_detector.detect_split`, and rescales the matching `stop_states` entry. Wired into `_on_market_close` immediately before `Stops_runner.update`. Tests: 7 unit tests (`test_stop_split_adjust.ml`) pin Initial/Trailing/Tightened scaling for forward 4:1 and reverse 1:5 splits, plus `Invalid_argument` on non-positive factors; 6 integration tests (`test_stops_split_runner.ml`) pin the no-split no-op, the 4:1 scaling, and the strategy-level regression — pre-split $440 stop scales to $110 so the post-split bar's $124 low does NOT trip `check_stop_hit`, while a real adverse move past the adjusted stop DOES trip it.
