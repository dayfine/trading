...
;;
;; Bands are ±10%, tighter than the ±15% house default: post-#2279 this run is
;; deterministic, so the only legitimate movement is an intentional behaviour
;; change, which should re-pin rather than slip through a wide band. If the
;; first postsubmit run lands outside these bands, do NOT widen them — that is
