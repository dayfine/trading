#!/usr/bin/env bash
# summarize.sh — emit a one-line summary of a 15y Cell-E run dir.
# Usage: summarize.sh <run-dir>
#
# Copy of dev/experiments/capital-recycling-combined-2026-05-07/summarize.sh
# (kept inline so this experiment is self-contained per
# `dev/agent-feature-workflow.md` "one directory per experiment").
set -eu
