#!/bin/sh
# Item-4 confirmation grid: 5y cells on the 2009 / 2019 _v10dedup vintages, null (n5-<vintage>) and arm (a4-<vintage>),
# salts 0/1/2; the warehouse is chosen from the spec name's vintage suffix. Same pinned build as the 26y cell
# (sweep-detgate @ 31e4bb9c3). Usage: sh chain-grid.sh <lane> <spec>:<salt>...   Artifacts: /tmp/sweeps/detgate-grid/.
set -u
LANE=$1; shift
...
SPECS_HOST=/tmp/grid-run/specs; WORK=/tmp/grid-run/$LANE; ART=/tmp/sweeps/detgate-grid
LOG_HOST=/tmp/grid-run/chain-$LANE.log; LOCK=/tmp/grid-run/chain-$LANE.lock; CELL_TIMEOUT=14400
...
:
for tok in "$@"; do name=${tok%%:*}; salt=${tok#*:}; tag="$name-s$salt-v10"; d=$WORK/$tag; W2000=/tmp/snap_top3000_${name##*-}_v10dedup; docker exec $C test -f $W2000/manifest.sexp || { log "RESULT $tag => <no warehouse $W2000>"; continue; }
