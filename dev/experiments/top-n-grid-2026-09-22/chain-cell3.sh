#!/bin/sh
# Top-N confirmation grid, CELL 3 = breadth tier (dev/experiments/top-n-grid-2026-09-22 README §Cell 3): a0-pit1000-null (the
# null) and t1-topn-40-1000 on the 26y record window (2000-01-01..2026-06-26) of a yearly TOP-1000 PIT schedule, salts
# 0/1/2. Same shape as chain-grid.sh (cell 2): no committed null for this schedule, so the null cells run in this lane and
# every arm cell pairs (validator_diff -check V6) against the null cell of the same salt from $ART - null cells FIRST.
# The top-1000 lists (universe/top-1000-YYYY.sexp, build-top1000.sh) are staged from $UNIV_HOST into the pinned run
# tree as pit-v11/composition/top-1000-YYYY.sexp - UNTRACKED files, so the dirty check (--untracked-files=no) still
# guards tracked code; their md5 is logged so the results PR can show which lists every cell read.
# Usage: EXPECT_HEAD=<sha> sh chain-cell3.sh <lane> <spec>:<salt>...   e.g.  a0-pit1000-null:0 t1-topn-40-1000:0
# Artifacts: /tmp/sweeps/top-n-grid-cell3/ (container, bind-mounted). Specs staged OUTSIDE any VCS tree at /tmp/grid-run/specs.
# CELL_TIMEOUT 28,800 s: the 26y top-3000 arm cells of cell 1 ran 4h48m / 4h16m / 3h59m at cap 12,000 (top-of-funnel
# README §Log); no 26y top-1000 cell has been measured, so the guard is 1.67x the slowest of those. Re-size from the first
# cell's wall (>= 1.5x, perf-review-weekly.md rule 3) before the rest run.
set -u
LANE=$1; shift
...
SPECS_HOST=/tmp/grid-run/specs; WORK=/tmp/grid-run/$LANE; ART=/tmp/sweeps/top-n-grid-cell3; UNIV_HOST=/tmp/grid-run/universe; WH=/tmp/snap_top3000_pit_v11pit
LOG_HOST=/tmp/grid-run/chain-$LANE.log; LOCK=/tmp/grid-run/chain-$LANE.lock; CELL_TIMEOUT=${CELL_TIMEOUT:-28800}
...
log "lane $LANE HEAD=$head (expect $EXPECT_HEAD)"; log "cell guard CELL_TIMEOUT=${CELL_TIMEOUT}s (1.67x the slowest measured 26y top-3000 arm cell, 4h48m; no top-1000 26y cell has been measured); SNAPSHOT_MAX_MMAP_HANDLES=$MMAP_HANDLES"
...
n_lists=$(ls $UNIV_HOST/top-1000-*.sexp 2>/dev/null | wc -l | tr -d " "); [ "$n_lists" = "27" ] || { log "ABORT: expected 27 top-1000 lists at $UNIV_HOST, found $n_lists"; exit 1; }
cp $UNIV_HOST/top-1000-*.sexp $REPO/$WTREL/trading/test_data/backtest_scenarios/pit-v11/composition/ || { log "ABORT: could not stage the top-1000 lists"; exit 1; }
log "top-1000 lists staged: 27 files, md5(cat)=$(cat $UNIV_HOST/top-1000-*.sexp | md5 -q) entries/yr=$(for f in $UNIV_HOST/top-1000-*.sexp; do grep -c "(symbol " $f; done | sort -n | sed -n "1p;27p" | tr "\n" - )"
...
for tok in "$@"; do name=${tok%%:*}; salt=${tok#*:}
  ...
  v6="n/a(null cell)"
  case "$name" in t1-*)
    if docker exec $C test -f $ART/a0-pit1000-null-s$salt-validator.sexp.sexp; then
      run "./_build/default/trading/backtest/validation/bin/validator_diff.exe -check V6 -report null=$ART/a0-pit1000-null-s$salt-validator.sexp.sexp -report arm=$ART/${tag}-validator.sexp.sexp > $WORK/$tag.v6diff.log 2>&1; echo exit=\$? >> $WORK/$tag.v6diff.log"
