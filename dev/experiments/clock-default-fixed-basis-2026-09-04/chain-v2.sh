#!/bin/sh
# v2 (21:23 PDT, after salt 0): the 2000-04 cells reproduced the exit-lever chain byte-for-byte
# (cell-C lineage == record convention), so 2000-04 salts 1-2 already exist there. Only the
# 2019-23 pairs run for salts 1-2. See README "Chain trimmed".
...
until grep -q 'RESULT cd5y-2019-c0-s1 ' "$LOG_HOST" && grep -q 'RESULT cd5y-2019-c52-s1 ' "$LOG_HOST"; do sleep 30; done
log "v2 resume after 2019-s1"; log "SALT 1 COMPLETE (2019 only; 2000-04 s1 = sw5y-2000-b1.0-s1 / xl5y-2000-clock52-s1)"
pair2 cd5y-2019-c0 2  cd5y-2019-c52 2
log "SALT 2 COMPLETE (2019 only)"
