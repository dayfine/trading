#!/bin/sh
# clock-surface cell-D null spread (#2600): the one residual qc-behavioral
...
# time. Specs: committed specs/goldens-paired/clockD-{0,52}.sexp, staged at
# /tmp/clock52g-d-specs (sweep-hygiene).
set -u
...
WORK=/tmp/clock52dnull
LOG_HOST=/tmp/clock52g-dnull-chain.log
SNAP=/tmp/snap_top3000_dedup_v5thin_adj
LOCK=/tmp/clock52g-dnull.lock
...
for salt in 1 2; do
  ...
  run_cell clockD-0 "$salt"
  run_cell clockD-52 "$salt"
done
log "CLOCK52 D NULL SWEEP DONE"
