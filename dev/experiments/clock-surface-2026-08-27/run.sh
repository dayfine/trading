...
# on broad cells (A+B here; cell C via run-c.sh). See README.md for the decision rule.
#
...
# --- Cell B: 5y broad, 2 concurrent (each ~2.2GB) ---
require_disk pre-B; require_memory pre-B
( run_cell clockB-0 ) &  p1=$!
( run_cell clockB-26 ) & p2=$!
wait $p1 $p2
require_disk mid-B; require_memory mid-B
( run_cell clockB-13 ) & p1=$!
( run_cell clockB-52 ) & p2=$!
wait $p1 $p2
require_disk mid-B2; require_memory mid-B2
run_cell clockB-156
log "CELL B COMPLETE"

# --- Cell A: 26y, strictly one at a time ---
for arm in clockA-0 clockA-26 clockA-13 clockA-52 clockA-156; do
  require_disk "pre-$arm"; require_memory "pre-$arm"
  run_cell "$arm"
done
log "CLOCK SURFACE DONE"
