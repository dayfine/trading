#!/bin/sh
# clock-surface cell-B null spread (#2405 / PR #2587 G2 rework, observation 2):
# "No null spread was measured for cell B at all." Runs clockB-0 and clockB-52
# at path salts {0,1,2} — 6 cells — so cell B's +7.3pp margin can be judged
# against its OWN salt-to-salt null, not an imported 26y floor
# ("every base needs its own null"). All 6 run at ONE build (the clock52
# pinned worktree, 5dc61da07); the original B arms ran at 90dfd6e97 (worktree
# since removed), so salt 0 is re-run here rather than reused — cross-build
# comparison is not valid, within-build pairing is. Specs are the committed
# specs/clockB-{0,52}.sexp, staged at /tmp/clock52g-b-specs (sweep-hygiene).
# 5y broad cells: 2-concurrent per the capacity table.
set -u
...
SPECS_HOST=/tmp/clock52g-b-specs
FIX=$ROOT/test_data/backtest_scenarios
WORK=/tmp/clock52b
LOG_HOST=/tmp/clock52g-b-chain.log
SNAP=/tmp/snap_top3000_dedup_v5thin_adj
LOCK=/tmp/clock52g-b.lock
...
for salt in 0 1 2; do
  ...
  ( run_cell clockB-0 "$salt" ) &  p1=$!
  ( run_cell clockB-52 "$salt" ) & p2=$!
  wait $p1 $p2
done
log "CLOCK52 B NULL SWEEP DONE"
