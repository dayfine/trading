;; Deteriorating-gate arm (dev/experiments/deteriorating-gate-2026-09-13/README.md): the a0-breadth-on-null spec of
;; stop-width-by-state-2026-09-08 with name/description changed and ONE override appended: deteriorating_blocks_longs true.
;; Runs on /tmp/snap_top3000_2000_v10dedup in the pinned worktree sweep-detgate. NOT a golden — staging scenario, sentinel bands.
((name "a3-deteriorating-gate")
 (description "Deteriorating-breadth long gate (#2755, PR #2759): record convention + breadth_direction read ON + deteriorating_blocks_longs ON. Pairs against a0-breadth-on-null-s<salt>-v10 (same warehouse, same salt).")
 ...
 (config_overrides
  (((enable_sim_entry_stoplimit true)) ((entry_order_max_rest_weeks 0))
   ...
   ((deteriorating_blocks_longs true))))
