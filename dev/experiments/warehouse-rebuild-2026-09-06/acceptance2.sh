#!/bin/sh
# Step 1: V16/V17 acceptance — the record spec (rec26y-new lineage, guards at
...
set -u
C=trading-1-dev
WT=/workspaces/trading-1/.claude/worktrees/sweep-wh0906c   # PR #2695 tip b48537469 (merged as b0b411df6): admission exclusion + #2693 marker fix
ROOT=$WT/trading
FIX=$ROOT/test_data/backtest_scenarios
SPECS_HOST=/tmp/wh-rebuild/specs
WORK=/tmp/wh-run2; ART=/tmp/sweeps/wh0906v7
LOG_HOST=/tmp/wh-rebuild/chain2.log; LOCK=/tmp/wh-rebuild/chain2.lock
W2000=/tmp/snap_top3000_2000_v7mark; W2019=/tmp/snap_top3000_2019_v7mark
CELL_TIMEOUT=28800
...
run_cell() { # name snapdir salt
  name=$1; snap=$2; salt=$3; tag="$name-s$salt"; d=$WORK/$tag
...
# Step 1 — acceptance at salt 0 (also the first re-base cell)
run_cell rec26y-new $W2000 0
# Step 2 — salts + windows
run_cell rec26y-new $W2000 1
run_cell rec26y-new $W2000 2
run_cell rec5y-2000-new $W2000 0
run_cell rec5y-2019-new $W2019 0
log "WH0906 CHAIN DONE"
