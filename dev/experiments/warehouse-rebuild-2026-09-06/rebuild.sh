...
set -u
C=trading-1-dev
WT=/workspaces/trading-1/.claude/worktrees/sweep-wh0906
ROOT=$WT/trading
EXE=$ROOT/_build/default/analysis/scripts/build_snapshots/build_snapshots.exe
CSV=/workspaces/trading-1/data
END=2026-09-06
LOG_HOST=/tmp/wh-rebuild/rebuild.log
LOCK=/tmp/wh-rebuild/rebuild.lock
...
log "run tree HEAD=$(git -C .claude/worktrees/sweep-wh0906 rev-parse --short HEAD) (expect cccd06813 for the rebuild; the acceptance run re-pins after #2692)"
for v in 2000 2009 2019; do
  case $v in 2000) start=1999-01-01;; 2009) start=2008-01-01;; 2019) start=2018-01-01;; esac
  out=/tmp/snap_top3000_${v}_v6tail
  ...
  log "BUILD $v -> $out"
  docker exec $C bash -c "cd $ROOT && eval \$(opam env) && $EXE -universe-path $spec -csv-data-dir $CSV -output-dir $out -benchmark-symbol GSPC.INDX -start-date $start -end-date $END > /tmp/wh-rebuild/build-$v.log 2>&1; echo exit=\$? >> /tmp/wh-rebuild/build-$v.log"
  ...
  log "RESULT $v snaps=$n_snap manifest=$n_man active_through=$n_at $(docker exec $C tail -1 /tmp/wh-rebuild/build-$v.log)"
  docker cp $C:$out/terminal_runs.csv "dev/experiments/warehouse-rebuild-2026-09-06/results/terminal_runs_$v.csv" 2>/dev/null || log "no terminal_runs.csv for $v"
done
log "REBUILD DONE"
