...
| 12 | **MERGED as #2713** (squash 722b3e93e, 03:08 PT Sep 8; closes #2711): `Series_splice` reuse defaults to report-only (`Kept`); cuts only by `cut_at`/`drop` exception; `min_kept_bars` 250 guard (secondary — refuses only 259/518); `n_dropped`/`n_kept` populated in report-only | fix PR, full gates | merged, goldens bit-identical |
| 13 | Vintage gap fetch (§"Vintage gap fetch" below): 1,359 of 1,531 attempted names fetched (1,247 of them dead — real death dates); 24 MISS (ticker reused by a new listing, or a <200-row real series); 148 synthetic twins skipped; TMS (2025 listing) quarantined 03:25 | `fetch_gap.sh` | done 02:42 PT Sep 8 |
| 14 | Rebuild the 2009 / 2019 warehouses on the widened store (`rebuild4.sh` → `_v9gap`, #2691/#2695 build, splice detector OFF) | build | **DONE 03:50 PT Sep 8**: 2009 = 2,886 snaps / 2,818 markers (old 2,033); 2019 = 2,936 / 2,869 (old 2,209); `skip TMS: no CSV` confirmed (quarantine held); 115 / 65 no-CSV skips remain (the 24 MISS names + synthetic twins) |
| 15 | Acceptance on `_v9gap` 2019: `acceptance4.sh` = rec5y-2019-new salt 0 (V16/V17 + level vs the survivor-tilted 42.37% / 175) | scenario cell | **DONE 06:05 PT Sep 8** — band: `_v9gap` 20.5–22.9% vs `_v7mark` 32.2–46.1% (paired salts 1–2); cost −12 to −23pp; see §"The survivorship cost as a BAND" |
...
- The 5y-2019 cell on the rebuilt 2019 warehouse now runs on ~2,200 names (superseded 09-08: the `_v9gap` 2019 warehouse has 2,936 after the gap fetch), not the ~980
...
## Vintage gap fetch (2026-09-08 02:38 → 02:42 PT) — the 2009 / 2019 stores widened (corrected after QC rework iteration 1)

`fetch_gap.sh` (fetch-historical-data skill; Phase 1 probe green: AAPL/GE from 1995, LEH → 2008-09-17, BS → 2004-01-05, gap-sample names at real dates). Attempted: the 1,519-name union of composition names with no CSV in the 2009 (968) and 2019 (792) stores, plus the 2000 vintage's 14 non-synthetic remainder = **1,531 distinct names**. Result (`results/fetch_gap_2026-09-08.log`, whose tail carries the 2000 remainder after its own `FETCH DONE` line): **1,359 fetched, 24 distinct MISS, 148 SKIP**. The skips are the synthetic `_old` rename twins. The misses: 12 from the union (BITA, BRKL, EROC, FHL, KBC, MIC, NHP, OPI, PINC, TRAD, WR, WX) + 12 from the 2000 remainder (ABS, ARPT, ASTN, BR1, EC1, GRP1, HOMG, ONDS1, PD1, QEDI, RCI1, UST1; NHP and WR repeat). Most return a few dozen 2026 rows or "Ticker Not Found" — the composition company's ticker has since been reused by a new listing, which the >200-row floor rejects; two are floor near-misses of a different kind, **FHL (160 rows, a real 2019 series) and ABS (195 rows)** — short genuine series, not reuse. All 24 stay absent.

**Survivorship proof (skill §Validate):** of the 1,359 fetched series, **1,247 (91.8%) end before 2026-06-01** — real death dates spread across 2007–2026 (25 in 2009, 95 in 2019, 122 in 2020, 130 in 2022, 91 in 2025); 112 are still live. Spot checks on disk match the log (ARUN last 2015-05-18, CELSIA 2020-06-05, CDI 2017-09-12). So the widened store is substantially the dead names — which is what the survivor tilt was made of.

**Late-start check** (first bar > vintage + 60 d, `results/fetch_gap_late_start.txt`): 2009 = 1 (IDCBY, an ADR from 2009-03-17); 2019 = 8, of which 7 list within 65–87 days of the vintage (AEPPL, BHFAP, DUK-PA, MGR, NEE-PN, SWAV, TRWH — preferreds / ADRs / a spring-2019 IPO, consistent with a mid-year composition as-of) and **one is a ticker reuse that the row floor could not catch: TMS, first bar 2025-04-21, 347 rows — a 2025 listing under a 2019-vintage name. Quarantined at 03:25 PT** (moved to `/tmp/wh-rebuild/quarantine/TMS`, before `rebuild4.sh` reached the 2019 arm, so `_v9gap` 2019 will log `skip TMS: no CSV`). Lesson: the >200-row floor rejects *short* reuses only; the reuse test that works is the vintage-date check — a series whose first bar is years after the vintage cannot be the composition member — so run it before any rebuild, not after.

Store after the fetch: **9,093 `data.csv` files** (the earlier "8,149 + 1,359" arithmetic mixed a shell-glob count with the fetch count; the on-disk count is the number). The 7-column header the fetch writes is the CSV parser's base shape (`test_parse_line_valid`); the 8-column `active_through` form is its documented compat sister. Next: `rebuild4.sh` (`_v9gap` 2009/2019, launched 03:08) → the 5y-2019 cell (`acceptance4.sh`) to measure what the widened store does to the survivor-tilted 42.37% / 175.

## `_v9gap` 2019 acceptance (04:17 PT Sep 8) — the survivor tilt measured: 42.4% → 21.7% at salt 0, V16/V17 clean

`rebuild4.sh` on the gap-filled store: 2009 = 2,886 snaps / 2,818 markers (was 2,033); **2019 = 2,936 / 2,869 (was 2,209)**; `skip TMS: no CSV` in the 2019 arm (the quarantine held). `acceptance4.sh` → `rec5y-2019-new` salt 0 on `_v9gap` 2019 (`results/rec5y-2019-v9gap-s0-*`): **21.67% / 181 trades / Sharpe 0.30 / maxDD 23.63**, validator **V16 PASS, V17 PASS**, vs the same spec at salt 0 on the survivor-tilted `_v7mark` 2019 (`rec5y-2019-new-s0-v7`): 42.37% / 175 / 0.50 / 21.65. Realised P&L $246k → $60k.

Dissection (`symbol|entry_date` join): **111 shared trades** earn slightly MORE on the widened warehouse (+$51k → +$68k); the gap is **v7-only 64 trades +$195k** (BBWI +$142k, AN +$89k, NGLOY +$42k, all `laggard_rotation` exits) vs **v9-only 70 trades −$8k** (BBBY +$132k and ZS +$126k offset by a long tail of losers). Only **18 of the v9-only trades are ON the 727 widened names** (−$50k together); the other 52 v9-only entries are on names both warehouses hold — the widened universe changes the screen's *ranking path* (727 more candidates compete for the same top-N slots), so this is mostly the PATH LOTTERY the 26y band already showed, with a direct dilution of −$50k from the dead names themselves. Exit mix nearly identical (stop_loss 135 → 137, laggard_rotation 39 → 41, delisted 1 → 2). **One draw**: salts 1–2 on BOTH warehouses launched 04:39 (`chain4.log`) to state the survivorship cost as a band; until then quote "≈ −20pp at salt 0, mostly path" — not a level.

**Build provenance caveat (QC #2715):** the salt-0 pair is NOT build-paired — the `_v7mark` cell ran at the #2695 branch tip b48537469 (`sweep-wh0906c`, 09-06) and the `_v9gap` cell at main 077b48973 (`sweep-wh0908`, after #2705 runtime guard-3 retirement, #2708 build-time splice action (detector off), #2709 fill-time ticket cancel), so the −20.7pp at salt 0 mixes the warehouse axis with three runtime/build changes. The salts-1–2 chain IS paired: `acceptance4.sh` runs both warehouses from the same `sweep-wh0908` build, so the band is the clean read and the salt-0 pair is only the first look. Two nits from the same review: "727 widened names" is the snap-count delta (2,936 − 2,209); the composition-derived widened set is 792 (= 727 + 65 with no CSV; the 2009 leg reconciles the same way: 968 = 853 + 115) — the 18-trade attribution is unchanged either way; the exit mix also gains `extension_stop` 0 → 1. Reproducibility inputs now committed: `results/rebuild4.log`, `results/old_warehouse_{2009,2019}_symbols.txt`, `results/fetch_gap_attempted_2009_2019.txt`. Consequence for the record: the 2019-vintage 5y cell in §Step 6 (42.37% / 175) was measured on the survivor-tilted warehouse and is superseded by the `_v9gap` band once the salts land; the 26y record (2000 vintage, 94% covered before the fetch; the 14-name remainder all MISS) is unaffected.

## The survivorship cost as a BAND (paired salts, 06:05 PT Sep 8) — −12 to −23pp on 2019–2023; the widened warehouse is the stable one

`acceptance4.sh` salts 1–2 on BOTH 2019 warehouses from the same build (`sweep-wh0908` @ 077b48973; `results/rec5y-2019-{v9gap,v7mark}-s{1,2}-*`, `chain4.log`). Every cell V16 PASS / V17 PASS.

| salt | `_v7mark` (2,209 names, survivor-tilted) | `_v9gap` (2,936 names) | Δ return | build-paired |
|---|---|---|---:|---|
| 0 | 42.37% / 175 / Sharpe 0.50 / DD 21.65 | 21.67% / 181 / 0.30 / 23.63 | −20.7pp | **no** (v7 cell at b48537469) |
| 1 | 32.16% / 181 / 0.41 / 21.61 | 20.47% / 184 / 0.30 / 23.55 | −11.7pp | yes |
| 2 | 46.09% / 172 / 0.53 / 21.72 | 22.85% / 179 / 0.31 / 23.55 | −23.2pp | yes |

**Bands:** `_v9gap` 20.5–22.9% (2.4pp wide, Sharpe 0.30–0.31, DD 23.5–23.6) vs `_v7mark` 32.2–46.1% (13.9pp wide, Sharpe 0.41–0.53, DD 21.6–21.7). **Survivorship cost on this window: −12 to −23pp of 5y return, −0.1 to −0.2 Sharpe, +2pp maxDD.** The survivor-tilted warehouse is not just higher, it is the salt-sensitive one; the widened universe is tight across salts. Quote the band; the −20.7pp at salt 0 is the unpaired first look.

**Dissection, salt 2 (`symbol|entry_date`):** shared 112 trades earn MORE on `_v9gap` (+$35.7k → +$52.2k); v7-only 60 = +$209k; v9-only 67 = +$46k; **18 v9-only trades on the 792 widened names = −$42k** (salt 0: 18 trades −$50k; salt 1: 19 trades −$54.7k — join: shared 110 +$48k → +$58k, v7-only 71 +$93k, v9-only 74 +$40k). Realised P&L $245k vs $99k. The arm-only names are the SAME across all three salts — BBWI +$142k / AN +$88k / NGLOY +$42k only ever on `_v7mark`; BBBY +$133k / ZS +$127k / FDX +$60k only ever on `_v9gap` — so this is **deterministic displacement, not lottery**: with 727 more candidates the top-N screen on the same dates ranks different breakouts into the slots, and the survivor-tilted pool happens to rank in the 2020–21 monsters (BBWI, AN) while the honest pool ranks in BBBY/ZS and a longer tail of small losers. The direct dilution from the dead names themselves is only −$42k to −$55k per salt (18–19 trades); the rest of the cost is the displaced monster set.

**Consequences:** (1) the §Step 6 2019 5y cell (42.37% / 175) is superseded — the record for that cell is now the `_v9gap` band 20.5–22.9% / 179–184 / Sharpe 0.30 / DD 23.5; (2) every earlier "broad-5y cell B" read (`project_warehouse_vintage_coverage`: ~980 survivor names) sits above this band for the same reason and must be re-measured on `_v9gap` before it is quoted; (3) the 26y record on the 2000 vintage is unaffected (94% covered before the fetch; the remainder all MISS); (4) `_v9gap` 2009 (2,886 names) has no cell yet — the rec5y-2009 spec does not exist; build it before any 2009-window read.

**Quality flag reviewed (QC #2717):** every `_v9gap` cell reports `force_liquidations_count 1` (every `_v7mark` cell 0). `results/rec5y-2019-v9gap-s0-force_liquidations.sexp`: APLS, position `APLS-wein-3299`, entered 2023-04-03 at 74.22, breaker `Per_position` on 2023-07-17 at 52.46 (−28.5% unrealised), exit filled next open 2023-07-18 at 53.89, −$50.8k; the same trade at every salt. It is a genuine market gap, not a data defect: APLS closed 84.50 on 2023-07-14 and opened 64.75 on 07-17 (−23% overnight; low 51.94, close 52.46 = −38% close-to-close, 20.3M shares — the safety-signal news day; 40.00 / 40.49 the next two closes), so the stop was jumped through and the per-position circuit breaker did its job. APLS is in BOTH warehouses (and not among the 792 widened names); the tilted path never enters it at salts 0 and 2 and enters it once in 2021 at salt 1 (−$10.9k stop) — never the 2023 trade. The stop sat at 74.90, 13.5% above the 07-17 open of 64.75, so no fill model could have honoured it. Nit for a follow-up: `trades.csv` labels the exit `stop_loss`, so the breaker is visible only in `force_liquidations.sexp` — per the 09-06 principle that fallbacks must never be silent, the `exit_trigger` should carry a distinct token.
