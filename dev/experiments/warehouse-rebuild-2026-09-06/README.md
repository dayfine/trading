...
| 13 | Vintage gap fetch (§"Vintage gap fetch" below): 1,359 of 1,519 missing 2009/2019 composition names fetched; 12 = ticker reused by a new listing (rejected); 148 synthetic twins skipped | `fetch_gap.sh` | done 02:40 PT Sep 8 |
...
## Vintage gap fetch (2026-09-08 02:38 → 02:40 PT) — the 2009 / 2019 stores widened

`fetch_gap.sh` (fetch-historical-data skill, Phase 1 probe green: AAPL/GE from 1995, LEH → 2008-09-17, BS → 2004-01-05, gap-sample names fetch at real dates): the union of composition names with no CSV in the store — 968 (2009) ∪ 792 (2019) = 1,519 → **1,359 fetched, 12 MISS, 148 SKIP** (`results/fetch_gap_2026-09-08.log`). The 148 skips are the synthetic `_old` rename twins (not vendor tickers). The 12 misses all return a few dozen 2026 rows or "Ticker Not Found" — BITA, BRKL, EROC, FHL, KBC, MIC, NHP, OPI, PINC, TRAD, WR, WX — i.e. the composition company's ticker has since been reused by a new listing, which the >200-row floor rejects correctly; those names stay absent (a reuse fetch would store the wrong company). Late-start check (first bar > vintage + 60 d, `results/fetch_gap_late_start.txt`): 2009 = 1 (IDCBY, an ADR from 2009-03-17), 2019 = 8 (AEPPL, BHFAP, DUK-PA, MGR, … preferred shares / ADRs listing within weeks of the vintage) — consistent with a mid-year composition as-of, nothing quarantined. Store now 8,149 + 1,359 symbol dirs; the 7-column header the fetch writes is the CSV parser's base shape (the 8-column `active_through` form is its documented compat sister). Next: rebuild the 2009 and 2019 warehouses (`_v9gap`) on the widened store with the #2691/#2695 build — they were 2,033 / 2,209 names, survivor-tilted — then re-run the 5y-2019 cell.
