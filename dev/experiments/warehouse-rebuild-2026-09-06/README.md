...
**Quality flag reviewed (QC #2717):** every `_v9gap` cell reports `force_liquidations_count 1` (every `_v7mark` cell 0). `results/rec5y-2019-v9gap-s0-force_liquidations.sexp`: APLS, position `APLS-wein-3299`, entered 2023-04-03 at 74.22, breaker `Per_position` on 2023-07-17 at 52.46 (−28.5% unrealised), exit filled next open 2023-07-18 at 53.89, −$50.8k; the same trade at every salt. It is a genuine market gap, not a data defect: APLS closed 84.50 on 2023-07-14 and opened 64.75 on 07-17 (low 51.94, close 52.46, 20.3M shares — the safety-signal news day), so the stop was jumped through and the per-position circuit breaker did its job. APLS is in BOTH warehouses; the tilted path happened to enter it in 2021 (−$10.9k stop) rather than 2023. Nit for a follow-up: `trades.csv` labels the exit `stop_loss`, so the breaker is visible only in `force_liquidations.sexp` — per the 09-06 principle that fallbacks must never be silent, the `exit_trigger` should carry a distinct token.
