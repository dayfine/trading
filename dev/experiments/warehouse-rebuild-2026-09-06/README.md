...
**Dissection, salt 2 (`symbol|entry_date`):** shared 112 trades earn MORE on `_v9gap` (+$35.7k → +$52.2k); v7-only 60 = +$209k; v9-only 67 = +$46k; **18 v9-only trades on the 792 widened names = −$42k** (salt 0: −$50k; salt 1 join: shared 110 +$48k → +$58k, v7-only 71 +$93k, v9-only 74 +$40k). Realised P&L $245k vs $99k. The arm-only names are the SAME across all three salts — BBWI +$142k / AN +$88k / NGLOY +$42k only ever on `_v7mark`; BBBY +$133k / ZS +$127k / FDX +$60k only ever on `_v9gap` — so this is **deterministic displacement, not lottery**: with 727 more candidates the top-N screen on the same dates ranks different breakouts into the slots, and the survivor-tilted pool happens to rank in the 2020–21 monsters (BBWI, AN) while the honest pool ranks in BBBY/ZS and a longer tail of small losers. The direct dilution from the dead names themselves is only −$42k to −$50k per salt; the rest of the cost is the displaced monster set.
