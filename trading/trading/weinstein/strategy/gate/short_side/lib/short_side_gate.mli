(** The [enable_short_side] entry gate.
    ...
    test. The [enable_short_side = false] contract — "the strategy emits
    {b zero} short entries from the production screening path" — is
    load-bearing: every long-only baseline (Cell-E grids, broad-universe PIT
    re-baselines) sets [((enable_short_side false))] and relies on it to
    suppress shorts entirely while the short-side gaps in
    [dev/notes/short-side-gaps-2026-04-29.md] are open. Extracting it to a
    named, unit-tested function pins that contract so a future refactor cannot
    silently let shorts leak back into a "long" run.
