(** Entry-candidate assembly: the single seam where the per-candidate entry
    ...
    {!Short_borrow_gate.apply} (short-side borrow-availability ADV floor, margin
    M3a, default-off). With every gate at its no-op default the result is the
    plain [buy_candidates @ short_candidates], bit-identical to prior behaviour. *)
