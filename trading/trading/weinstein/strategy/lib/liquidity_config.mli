(** Configuration for the liquidity-realism overlay.
    ...
    Motivation: a delisted micro-cap held into illiquidity (e.g. ELCO trading
    ~2 shares/day) produced a fake −48% single-day NAV crash when a spurious
...
type t = {
  adv_lookback_days : int;
      (** Trailing window (in daily bars) over which dollar-ADV is averaged.
          A harmless positive default (e.g. 20) — it is only consulted when one
          of the thresholds below is positive. *)
