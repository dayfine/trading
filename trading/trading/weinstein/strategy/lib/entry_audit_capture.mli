...
type entry_meta = {
  ...
  effective_entry_price : float;
      (** The price the strategy installs into [Position.t] state — most recent
          ...
          [initial_risk_dollars] in {!Entry_audit_emit.build_entry_event}) key
          off this rather than [candidate.suggested_entry] so the audit reflects
          the realised entry rather than the screener's pre-fill intent. *)
  ...
  sized_down_wide_stop : bool;
      (** F3 audit tag ([Sized_down_wide_stop]). [true] when
          ...
          PR-2's deferral: {!Entry_audit_emit.build_entry_event} now also
          carries the tag onto
...
type entry_attempt_result =
  ...
  | No_structural_stop
      (** Investor-preset gate ([?require_structural_stop] on): the installed
          ...
          ([stop_floor_kind = Buffer_fallback]) — no qualifying structural floor
          / ceiling — so the candidate is not entered. Mapped to
...
(** Try to build a [CreateEntering] transition for [cand]. Returns:
    ...
    - [Stop_too_wide]: the support-floor-derived initial stop sits more than
      ...
      consumed.
    - [No_structural_stop]: only under [?require_structural_stop]; the initial
      stop is the [Buffer_fallback] automatic percentage. Checked between steps
      (2) and (3) below, so it takes precedence over [Stop_too_wide]. No
      [stop_states] entry written; no position id consumed.
...
(** Classify one candidate as [Kept] or [Skipped reason]. The skip reasons match
    ...
    running long-notional cap, or rejected by the running sector-exposure cap. A
    [make_entry] result of [No_structural_stop] maps to
    [Skipped No_structural_stop] (no cash touched). Order: held-check, sizing
    via [make_entry], cash check, short-notional cap, long-notional cap, then
    sector-exposure cap.
