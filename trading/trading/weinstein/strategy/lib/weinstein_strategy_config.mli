...
type config = {
  ...
  max_long_exposure_pct_entry : float; [@sexp.default 0.0]
      (** Cap on aggregate NEW long-entry notional as a fraction of {e current
          portfolio value}, applied at the Friday entry walk
          ({!Weinstein_strategy.entries_from_candidates}). Entry-price-denominated
          committed-at-entry notional across held [Holding] longs plus the
          candidates funded this walk may not exceed
          ...
          matrix showed the long book {e levering on short proceeds} (marked long
          exposure > NAV in 269 sampled weeks, peak 158%) with that dead knob set
          to 0.70 and ignored. This field is the honest, live-path cap: it is
          consumed at the one seam that actually gates entry funding (the
          entry-walk long-notional accumulator), mirroring
          {!check_short_notional_cap}'s machinery exactly.

          {b Basis — entry-price-denominated notional, NOT marked value.} The cap
          counts [shares * entry_price] committed at entry, matching the short
          cap ([max_short_notional_fraction]). Marked exposure exceeding 100% of
          NAV purely from {e unrealized appreciation} of held winners is
          ...
          this gate targets. Entry-denominated also keeps the long and short caps
          symmetric and avoids threading a [get_price] mark into the walk.
          ...
          notional — i.e. shorts can fund longs, but only up to this cap. This is
          THE margin convention for long-short backtests: it bounds how far the
          long book may lever on short proceeds.

          {b Scope — NEW entries only.} The cap narrows only new long entry
          funding. Exits, covers, stop orders, and force-liquidations do not flow
          through the entry walk and are structurally unaffected, so the cap can
          never block an exit (the #1553 exit-fill-reject lesson).
          ...
          {b Faithfulness} (W1/W2, [.claude/rules/weinstein-faithful-core.md]). A
          portfolio-risk / exposure dial that {e tightens} deployment without
          ...
          ([((max_long_exposure_pct_entry) (values (0.0 0.7 1.0)))]). Default-off
          until an experiment-ledger ACCEPT (per
          [.claude/rules/experiment-flag-discipline.md] +
          [.claude/rules/promotion-confirmation.md]). *)
