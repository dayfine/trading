...
let _screen_universe ~config ~index_bars ~macro_trend ~stop_states ~positions
    ~portfolio_value ~bar_history ~current_date =
...
let _on_market_close ~config ~stop_states ~prior_macro ~bar_history ~get_price
    ~get_indicator:_ ~positions ?portfolio_value () =
  ...
  let entry_transitions =
    if not (_is_screening_day index_bars) then []
      ...
      if Weinstein_types.(equal_market_trend !prior_macro Bearish) then []
        ...
        let pv = Option.value portfolio_value ~default:0.0 in
        _screen_universe ~config ~index_bars ~macro_trend:macro_result.trend
          ~stop_states ~positions ~portfolio_value:pv ~bar_history ~current_date
  in
  Ok
    {
