...
(** Build the inline [get_ma] from the view's closes (current allocational path;
    ...
    bar-list-only path. *)
let _inline_get_ma ~(config : Stage.config)
...
let _stage_get_ma ?ma_cache ?symbol ~(config : Stage.config)
    ~(weekly : Snapshot_bar_views.weekly_view) () :
...
let resistance_callbacks_of_weekly_view
    ~(weekly : Snapshot_bar_views.weekly_view) : Resistance.callbacks =
  {
...
let stock_analysis_callbacks_of_weekly_views ?ma_cache ?stock_symbol
  ...
  let resistance_view = Option.value resistance_stock ~default:stock in
  {
    ...
    resistance = resistance_callbacks_of_weekly_view ~weekly:resistance_view;
  }
...
let _named_global_stage ?ma_cache (config : Macro.config)
    ((name, view) : string * Snapshot_bar_views.weekly_view) :
    string * Stage.callbacks =
  ( name,
...
(** Build the two breadth closures from an optional precomputed series. [None]
    (the default) reproduces the pre-breadth bundle exactly. *)
let _breadth_callbacks breadth_series ~as_of =
  match breadth_series with
  | None -> ((fun ~week_offset:_ -> None), fun ~week_offset:_ -> None)
  | Some series -> Breadth_series_cache.callbacks_at series ~as_of
...
let macro_callbacks_of_weekly_views_cached ?ma_cache ?index_symbol
    ?breadth_series ~(config : Macro.config)
    ~(index : Snapshot_bar_views.weekly_view)
    ~(globals : (string * Snapshot_bar_views.weekly_view) list)
  ...
  let get_pct_above_ma, get_new_lows_pct =
    _breadth_callbacks breadth_series ~as_of
  in
  {
    index_stage =
      stage_callbacks_of_weekly_view ?ma_cache ?symbol:index_symbol
