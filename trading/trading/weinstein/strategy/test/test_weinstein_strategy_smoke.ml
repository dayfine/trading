...
let test_weinstein_breakout_trade _ =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      in
      assert_that result.steps (not_ is_empty);
      (* At least one step should have submitted orders (from screener) *)
      let has_orders =
        List.exists result.steps ~f:(fun step ->
            not (List.is_empty step.orders_submitted))
      in
      assert_bool "Expected at least one step with orders submitted" has_orders;
      (* At least one trade should have been executed *)
      let total_trades =
        List.sum
          (module Int)
          result.steps
          ~f:(fun step -> List.length step.trades)
      in
      assert_that total_trades (gt (module Int_ord) 0);
