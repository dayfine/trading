(** End-to-end smoke test: Simulator.run with Weinstein_strategy.
    ...
    - Portfolio value: [?portfolio_value:float] optional param on
      [STRATEGY.on_market_close], supplied by the simulator from portfolio
      snapshot each step. Used for position sizing.
