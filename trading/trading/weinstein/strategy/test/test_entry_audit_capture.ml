...
(** Run [make_entry_transition] for [cand] over a ONE-bar reader (close $100):
    ...
    percentage ([Buffer_fallback]). Returns the result and the [stop_states] ref
    so callers can pin the no-side-effect contract. *)
...
let test_require_structural_stop_on_skips_buffer_fallback _ =
  ...
  in
  assert_that
    (result, Map.length !stop_states)
    (all_of [ field fst _is_no_structural_stop; field snd (equal_to 0) ])
...
let test_require_structural_stop_applies_to_shorts _ =
  ...
  in
  let attempt flag =
    fst (_no_floor_attempt ~require_structural_stop:flag cand)
  in
  assert_that
...
let test_entries_from_candidates_threads_require_structural_stop _ =
  ...
  in
  assert_that
    [ entries false; entries true ]
    (elements_are [ equal_to 1; equal_to 0 ])
