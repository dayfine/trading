(** Weinstein trading state persistence.
    ...
    The state is serialised as a sexp via [File_sexp.Sexp].
...
(** Action taken in a trade log entry. *)
...
(** [save state ~path] serialises [state] as sexp and writes it to [path].

    Returns [Error] if the file cannot be written. *)

val load : path:Fpath.t -> t Status.status_or
(** [load ~path] deserialises state from the sexp file at [path].

    Returns [Error] if the file does not exist or the sexp is malformed. *)
