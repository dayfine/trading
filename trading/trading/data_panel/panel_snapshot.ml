...
let dump ~path symbol_index ~panels ~panel_names =
  ...
  in
  let header_str = header |> Header.sexp_of_t |> Sexp.to_string in
  let header_bytes = Bytes.of_string header_str in
  ...
  let total_size = body_offset + (n_panels * panel_size_bytes) in
  (* Step 1: write header + zero-padded body region. *)
  Out_channel.with_file path ~f:(fun oc ->
      _write_int64_le oc (Int64.of_int header_len);
      Out_channel.output_bytes oc header_bytes;
      let pad_len = body_offset - 8 - header_len in
      let pad = Bytes.make pad_len '\x00' in
      Out_channel.output_bytes oc pad;
      (* Pre-extend the file to total size by writing zero pages. We use a
         simple zero-fill; for very large panels [ftruncate] would be faster
         but the spike's footprint is small. *)
      let zero_chunk = Bytes.make 65536 '\x00' in
      let remaining = ref (n_panels * panel_size_bytes) in
  ...
  ignore total_size;
  (* Step 2: mmap the body region for write and copy each panel into it. *)
