...
let get_symbol t = match t.state with
  | Entering s -> s.symbol
  | Holding s -> s.symbol
  | Exiting s -> s.symbol
  | Closed s -> s.symbol
...
let apply_transition t transition =
  ...
  | ( Entering entering,
      ...
      if Float.(entering.filled_quantity <= 0.0) then
        Error (Status.invalid_argument_error "Cannot complete entry with no fills")
      else
        let holding =
          {
            id = entering.id;
  ...
  | Exiting exiting, ExitComplete { position_id; completion_date } ->
      ...
      if Float.(exiting.filled_quantity <= 0.0) then
        Error (Status.invalid_argument_error "Cannot complete exit with no fills")
      else
