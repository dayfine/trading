...
let setup_test_data () =
  (* Create test data directory and sample CSV files for testing *)
  ...
  let aapl_storage = Csv.Csv_storage.create ~data_dir:test_data_dir "AAPL" |> ok_or_fail_status in
  ...
  let googl_storage = Csv.Csv_storage.create ~data_dir:test_data_dir "GOOGL" |> ok_or_fail_status in
  ignore (Csv.Csv_storage.save googl_storage googl_prices |> ok_or_fail_status)

let teardown_test_data () =
  let dir_str = Fpath.to_string test_data_dir in
  match Sys_unix.file_exists dir_str with
  | `Yes -> ignore (Bos.OS.Dir.delete ~recurse:true test_data_dir)
  | _ -> ()
...
let test_lazy_loading _ =
  ...
  assert_bool "AAPL should be cached" (List.mem cached "AAPL" ~equal:String.equal);
...
let test_get_prices_with_date_filter _ =
  ...
  let result = get_prices backend ~symbol:"AAPL"
    ~start_date:(Date.create_exn ~y:2024 ~m:Month.Jan ~d:2) () in
  assert_that result
    (is_ok_and_holds (fun prices ->
  ...
  let result2 = get_prices backend ~symbol:"AAPL"
    ~end_date:(Date.create_exn ~y:2024 ~m:Month.Jan ~d:2) () in
  assert_that result2
    (is_ok_and_holds (fun prices ->
         assert_that prices (size_is 2)));

  (* Get prices for specific date range *)
  let result3 = get_prices backend ~symbol:"AAPL"
