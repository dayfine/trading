...
let test_create_entering_generates_buy_order _ =
  ...
  in
  let result = transition_to_order transition in
  assert_that result
    (is_ok_and_holds
...
let test_trigger_exit_returns_none _ =
  (* TriggerExit doesn't generate an order directly because it doesn't
     have the position quantity - the simulator handles this *)
  let transition =
...
let test_empty_transitions_returns_empty_orders _ =
  let result = transitions_to_orders [] in
  assert_that result
...
let test_single_create_entering_generates_one_order _ =
  ...
  in
  let result = transitions_to_orders transitions in
  assert_that result
    (is_ok_and_holds (fun orders ->
...
let test_mixed_transitions_filters_non_order_generating _ =
  ...
  assert_that result
    (is_ok_and_holds (fun orders ->
         (* Only CreateEntering generates an order; others are filtered out *)
         assert_that (List.length orders) (equal_to 1);
...
let suite =
  ...
  >::: [
         ...
         >:: test_create_entering_generates_buy_order;
         "test_trigger_exit_returns_none" >:: test_trigger_exit_returns_none;
