(** Price-banded tier tables for short-side margin mechanics (margin M3a).
    ...
    provides the lookup mechanism. Concrete tier values live in default-disarmed
    example configs / tests, per [.claude/rules/experiment-flag-discipline.md]
    R1 (default-off) — the code ships an empty table.
...
(** [tier_value ~tiers ~flat_fallback ~price] resolves the tier value for a
    short marked at [price].
    ...
    Examples with
    [tiers = [ {price_below=5.0; value=1.0}; {price_below=17.0; value=0.83} ]]
    and [flat_fallback = 0.30]:
