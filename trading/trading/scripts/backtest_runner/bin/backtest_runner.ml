(** Backtest runner CLI — thin wrapper around {!Backtest_runner_lib}.
    ...
    Writes params.sexp, summary.sexp, trades.csv to a timestamped directory
    under dev/backtest/ and prints the summary sexp to stdout. *)
...
let () =
  ...
  let result =
    Backtest_runner_lib.run_backtest ~start_date ~end_date ~overrides ()
  in
  let output_dir = _make_output_dir () in
  eprintf "Writing output to %s/\n%!" output_dir;
  Backtest_runner_lib.write_output_dir ~output_dir result;
  eprintf "Output written to: %s/\n%!" output_dir;
  Out_channel.output_string stdout (Sexp.to_string_hum result.summary_sexp);
