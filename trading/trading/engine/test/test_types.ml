...
let test_engine_config_construction _ =
  ...
  let config = { commission = commission_config; slippage_bps = 0 } in
...
let test_engine_config_equality _ =
  let commission_config = { per_share = 0.01; minimum = 1.0 } in
  let config1 = { commission = commission_config; slippage_bps = 0 } in
  let config2 = { commission = commission_config; slippage_bps = 0 } in
  let config_diff_slip = { commission = commission_config; slippage_bps = 5 } in
  assert_bool "Identical engine configs should be equal"
    (equal_engine_config config1 config2);
  assert_bool "engine configs with different slippage_bps should not be equal"
    (not (equal_engine_config config1 config_diff_slip))
