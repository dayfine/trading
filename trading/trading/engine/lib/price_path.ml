...
let _generate_bridge_segment_into ~random_state ~pool ~out ~out_start
  ...
  for i = 1 to n_points do
    ...
    let clamped_price =
      Bar_shape.clamp ~min_val:low_bound ~max_val:high_bound new_price
    in
...
(* Re-exported so callers already depending on [Price_path] need not also
   depend on [Bar_shape]; the definition lives with the other bar-derived
   quantities. *)
let seed_for_bar = Bar_shape.seed_for_bar
...
let _generate_path_with_scratch ~scratch ~config (bar : price_bar) :
  ...
  in
  let high_first = Bar_shape.decide_high_first random_state bar in
  ...
  in
  let volatility_scale = Bar_shape.infer_volatility_scale bar in
