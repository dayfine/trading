...
let _v12_pred (c : check_config) (row : trade_row) (ctx : entry_context) =
  (* Skip un-evaluable rows: legacy audits with no captured [installed_stop]
     ([0.0] default) or a non-positive fill. *)
  if Float.( <= ) ctx.installed_stop 0.0 || Float.( <= ) row.entry_price 0.0
  then Skip
  else
    let dist = _v12_dist ~row ~ctx in
    if Float.( > ) dist c.gate_max_stop_distance_pct then
