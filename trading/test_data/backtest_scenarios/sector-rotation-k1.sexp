...
;; (long/flat), INVESTOR preset (30-week MA), K=1 — hold the single strongest
...
;;
;; Strategy: [Sector_rotation_weinstein (k 1) (ma_period_weeks 30)] — see
...
;; keeps the Stage-2 names, ranks them by RS vs SPY, and holds the top-1:
;;   - enters (all cash, k=1) the strongest Stage-2 sector when a slot is open,
;;   - exits a held sector when it leaves the top-1 set or rolls to Stage 3/4, or
...
((name "sector-rotation-k1")
 (description "Sector-rotation Weinstein investor preset (30wk MA, long/flat), K=1 — top-1 Stage-2 SPDR sector by RS vs SPY, 2009-06-01 to 2025-12-31.")
 (period ((start_date 2009-06-01) (end_date 2025-12-31)))
 (universe_path "universes/spdr-sectors-11-plus-spy.sexp")
 (universe_size 12)
 (config_overrides ())
 (strategy (Sector_rotation_weinstein (k 1) (ma_period_weeks 30)))
 (expected
