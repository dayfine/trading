;; LADDER V4 — cell 23 "rt-ttl4-nearfloor-maxstop25": composed — F1 + 4-week TTL + nearest-floor anchor + a 0.25 width gate.
...
;; v4 coordinate: anchor=4w freshness=Range_top_breakout ttl=4w width=Nearest@0.25 volume_confirm=false
;; Asks whether the book's own distance-shrinking remedy plus one notch of gate loosening substitutes for the diagnostic-only wide cells. The most defensible of the loosened-gate arms, but 0.25 still deviates from §5.1's 15%.
;;
...
((name "top3000-2000-2026-v4-23-rt-ttl4-nearfloor-maxstop25")
 (description "Ladder v4 cell 23 (rt-ttl4-nearfloor-maxstop25): anchor=4w freshness=Range_top_breakout ttl=4w width=Nearest@0.25 volume_confirm=false. composed — F1 + 4-week TTL + nearest-floor anchor + a 0.25 width gate.")
 ...
 (config_overrides
  (;; --- v2-core base (verbatim from the ladder-v3 faithful arm) ---
   ...
   ((stops_config ((max_stop_distance_pct 0.25))))
