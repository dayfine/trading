;; LADDER V4 — cell 10 "volconf": volume axis delta — F5 volume confirmed AT FILL rather than at placement.
...
;; v4 coordinate: anchor=4w freshness=Ma_cross ttl=0w width=Drop@0.15 volume_confirm=true
;; Placement drops the screen-week volume requirement (book §4.7: the breakout week's volume cannot be known when the GTC order is written); confirmation moves to the fill week (§4.2, both branches). The eject is inseparable from the flag — a held volume-unconfirmed breakout would be a W1 spine FAIL.
;;
...
((name "top3000-2000-2026-v4-10-volconf")
 (description "Ladder v4 cell 10 (volconf): anchor=4w freshness=Ma_cross ttl=0w width=Drop@0.15 volume_confirm=true. volume axis delta — F5 volume confirmed AT FILL rather than at placement.")
