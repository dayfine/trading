...
LOG="${ROOT}/trading/devtools/checks/fixtures/perf_long_cells/dev/experiments/demo-2026-01-01/chain.log"
[ -f "$SCRIPT" ] || { echo "FAIL: script under test not found: $SCRIPT" >&2; exit 1; }
[ -f "$LOG" ] || { echo "FAIL: fixture not found: $LOG" >&2; exit 1; }
...
expect_eq "row shape for n0-s0" "demo-2026-01-01,n0-s0-v11,pit-schedule-3000,10.00,12000,abc1234,1000,100" \
  "$(printf '%s\n' "$rows" | grep ',n0-s0-v11,' | cut -d, -f2-)"
expect_eq "missing spec keeps ?" "?,?" "$(printf '%s\n' "$rows" | grep ',missing-spec-s0,' | cut -d, -f4,5)"

sh "$SCRIPT" update "$LOG" >/dev/null
sh "$SCRIPT" update "$LOG" >/dev/null
expect_eq "update dedupes on (date, tag)" 5 "$(wc -l < "$PERF_LONG_LEDGER" | tr -d ' ')"

rc=0; sh "$SCRIPT" check >"$TMP/check.txt" || rc=$?
expect_eq "newest cell 43 % slower than the median -> exit 1" 1 "$rc"
expect_eq "check names the shape" 1 "$(grep -c 'SLOWER *pit-schedule-3000|10y|cap12000' "$TMP/check.txt")"
