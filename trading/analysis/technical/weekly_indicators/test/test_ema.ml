...
let test_calculate_30_week_ema _ =
  ...
  let data =
    List.init 35 ~f:(fun i ->
        let date = Date.add_days base_date (i * 7) in
        create_test_data date (float_of_int (100 + i)))
  in
  let ema_result = calculate_30_week_ema data in
  assert_equal 5 (List.length ema_result);
  (* should have 5 EMA values (35-30) *)
  match List.rev ema_result with
  | (last_date, last_ema) :: _ ->
      assert_bool "Last EMA should be > 100.0" (Float.(>) last_ema 100.0);
      assert_equal 2024 (Date.year last_date);
      assert_equal Month.Aug (Date.month last_date)
