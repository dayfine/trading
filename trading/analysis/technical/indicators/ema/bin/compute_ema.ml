...
let daily_prices_to_indicator_values prices =
  List.map prices ~f:(fun p -> { date = p.Types.Daily_price.date; value = p.close_price })
      ...
      | Ok data ->
          let indicator_values =
            if weekly then
              ...
              daily_prices_to_indicator_values data
          in
...
let command =
  Command.basic ~summary:"Calculate EMA from daily price data"
    (let%map_open.Command input_file =
     ...
     and weekly =
       flag "weekly" no_arg
         ~doc:"Calculate weekly EMA instead of daily"
     in
