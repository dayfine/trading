...
(** Convert daily data to weekly by taking the last entry of each week.
    ...
    @param include_partial_week
      If true, includes incomplete weeks at the end (default: true for backward
      compatibility). Set to false to only return complete weeks ending on
      Friday. Use true for provisional values (intra-week computation).
    ...
    - [Mon-Fri, Mon-Wed] with include_partial_week=false → [Fri] (only complete
      week) *)
