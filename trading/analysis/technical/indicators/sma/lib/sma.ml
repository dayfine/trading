...
let _calculate_ma (data : indicator_value list) (period : int) ~(weight_fn : int -> float)
    ~error_label : indicator_value list =
  ...
  if n < period then []
    ...
    List.init (n - period + 1) ~f:(fun i ->
        ...
        let weighted_sum =
          List.foldi window ~init:0.0 ~f:(fun j acc v -> acc +. (v.value *. weight_fn j))
        in
        let weight_total =
...
let calculate_sma (data : indicator_value list) (period : int) : indicator_value list =
  _calculate_ma data period ~weight_fn:(fun _ -> 1.0) ~error_label:"SMA"

let calculate_weighted_ma (data : indicator_value list) (period : int) : indicator_value list =
  (* weight: j=0 is oldest (weight 1), j=period-1 is newest (weight period) *)
  _calculate_ma data period ~weight_fn:(fun j -> Float.of_int (j + 1)) ~error_label:"WMA"
