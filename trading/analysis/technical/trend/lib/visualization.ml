...
let create_plot data segments =
  ...
  Plot.(plot ~h ~spec:[ RGB (100, 100, 100); Marker "*"; MarkerSize 1.0 ] x_mat y_mat);
  ...
  List.iter (fun segment ->
    ...
    let trend_y = Array.init segment_length (fun i ->
    ...
    Plot.(text ~h ~spec:[ color ] mid_x mid_y (Printf.sprintf "R²=%.2f" segment.r_squared))
  ) segments;
