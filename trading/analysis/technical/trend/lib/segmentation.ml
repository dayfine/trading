...
let segment_by_trends ?(min_segment_length = 3) ?(preferred_segment_length = 10)
    ?(length_flexibility = 0.5) ?(min_r_squared = 0.6) ?(min_slope = 0.01)
    ?(max_segments = 10) ?(preferred_channel_width = 0.5)
    ?(max_channel_width = 2.0) ?(width_penalty_factor = 0.5) data_array =
  ...
  if n < min_segment_length * 2 then
    ...
    let length_penalty segment_length =
      let ratio =
        if segment_length < preferred_segment_length then
          float_of_int segment_length /. float_of_int preferred_segment_length
        else
          float_of_int preferred_segment_length /. float_of_int segment_length
      in
    ...
    let width_penalty channel_width =
      if channel_width > max_channel_width then 1.0
      else
        let ratio =
          if channel_width < preferred_channel_width then
            channel_width /. preferred_channel_width
    ...
    let rec find_segments segments remaining_splits start_idx end_idx =
      ...
      if
        remaining_splits <= 0
        || end_idx - start_idx + 1 < min_segment_length * 2
      then
        ...
        let _, whole_slope, whole_r2, whole_std =
          segment_stats start_idx end_idx
        in

        (* If current segment fits well, don't split further *)
        let whole_width_penalty = width_penalty whole_std in
        if whole_r2 >= min_r_squared +. 0.1 && whole_width_penalty < 0.3 then
          make_segment start_idx end_idx whole_slope whole_r2 whole_std
          :: segments
        else
          ...
          for
            split_idx = start_idx + min_segment_length - 1
            to end_idx - min_segment_length
          do
            (* Get stats for both segments *)
            let _, left_slope, left_r2, left_std =
              segment_stats start_idx split_idx
            in
            let _, right_slope, right_r2, right_std =
              segment_stats (split_idx + 1) end_idx
            in
            ...
            let length_penalty_score =
              ...
              +. (right_len_penalty *. float_of_int right_len))
            ...
            let width_penalty_score =
              ...
              +. (right_width_penalty *. float_of_int right_len))
              /. float_of_int total_len
            in
            ...
            let weighted_r2 =
              ...
              +. (right_r2 *. float_of_int right_len))
              /. float_of_int total_len
            in
            ...
            let trend_change_bonus =
              if
                left_trend != 0 && right_trend != 0 && left_trend != right_trend
              then 0.2
              else 0.0
            in
            ...
            (* Combined score *)
            let split_score =
              weighted_r2 +. trend_change_bonus
              -. (length_penalty_score *. 0.3)
              -. (width_penalty_score *. 0.5)
            in

            (* Update best split if better *)
            if split_score > !best_score then (
              best_score := split_score;
              best_split_idx := split_idx)
          done;

          (* Process the best split or keep as is *)
          if !best_split_idx <> -1 && !best_score > whole_r2 -. 0.05 then
            let left_segments =
              find_segments segments (remaining_splits - 1) start_idx
                !best_split_idx
            in
            find_segments left_segments (remaining_splits - 1)
              (!best_split_idx + 1) end_idx
          else (* No good split found, keep segment as is *)
            make_segment start_idx end_idx whole_slope whole_r2 whole_std
            :: segments
    in
