...
let test_linear_regression _ =
  ...
  assert_equal ~msg:"Intercept should be 0.0" ~cmp:float_equal 0.0 stats.intercept;
  assert_equal ~msg:"R-squared should be 1.0" ~cmp:float_equal 1.0 stats.r_squared
...
let test_perfect_line _ =
  ...
  let stats = calculate_stats x y in
  assert_equal ~msg:"Slope should be 2.0" ~cmp:float_equal 2.0 stats.slope;
  assert_equal ~msg:"Intercept should be 1.0" ~cmp:float_equal 1.0 stats.intercept;
  assert_equal ~msg:"R-squared should be 1.0" ~cmp:float_equal 1.0 stats.r_squared
