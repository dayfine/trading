...
let test_basic_segmentation _ =
  ...
  assert_bool "First segment should have high R²"
    (float_equal seg1.r_squared 1.0);
  ...
  assert_bool "Second segment should have high R²"
    (float_equal seg2.r_squared 1.0);
  ...
  assert_bool "Third segment should have high R²"
    (float_equal seg3.r_squared 1.0)
...
let test_short_data _ =
  ...
  assert_bool "Should return single segment for short data"
    (Array.length segments = 1);
  let seg = segments.(0) in
  assert_bool "Segment should span entire data"
    (seg.start_idx = 0 && seg.end_idx = 2);
