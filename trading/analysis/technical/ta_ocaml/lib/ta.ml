...
let check_retcode code =
  if code <> 0 then
    failwith (Printf.sprintf "TA-Lib error: %d" code)
...
let extract_results out out_begin out_nbelement =
  let begin_idx = !@out_begin in
  let nb_elements = !@out_nbelement in
  if nb_elements <= 0 then
    Error "Not enough data to calculate indicator"
  else
    let result = Array.make nb_elements 0.0 in
    for i = 0 to nb_elements - 1 do
      result.(i) <- CArray.get out (begin_idx + i)
    done;
    Ok result

(* Common function to handle technical indicators *)
let calculate_indicator
    (indicator_fn : int -> int -> float ptr -> int -> float ptr -> int ptr -> int ptr -> int)
    (data : float array)
    (period : int)
    : (float array, string) result =
  let len = Array.length data in
  if len < period then
    Error (Printf.sprintf "Not enough data: need at least %d elements, got %d" period len)
  else
    begin
      ...
      Printf.printf "Debug: begin_idx=%d, nb_elements=%d\n" !@out_begin !@out_nbelement;

      if !@out_nbelement <= 0 then
        Error "Not enough data to calculate indicator"
      else
