...
let compare_float_arrays arr1 arr2 =
  if Array.length arr1 <> Array.length arr2 then
    false
  else
    Array.for_all2 (fun a b -> abs_float (a -. b) < 0.01) arr1 arr2
...
let%expect_test "SMA with valid data" =
  ...
  print_endline (match result with
    | Ok arr -> Printf.sprintf "Ok %s" (Array.to_list arr |> List.map string_of_float |> String.concat "; " |> Printf.sprintf "[|%s|]")
...
let%expect_test "SMA with insufficient data" =
  let data = [|10.0; 12.0|] in  (* Only 2 elements *)
  let period = 3 in             (* Need 3 elements *)
  let result = sma data period in
  print_endline (match result with
    | Ok arr -> Printf.sprintf "Ok %s" (Array.to_list arr |> List.map string_of_float |> String.concat "; " |> Printf.sprintf "[|%s|]")
    | Error msg -> Printf.sprintf "Error %S" msg);
...
let test_ema () =
  ...
  | Error msg ->
      Printf.printf "EMA test failed with error: %s\n" msg
...
let test_rsi () =
  ...
  | Error msg ->
      Printf.printf "RSI test failed with error: %s\n" msg
