  ...
  | _header :: rows ->
      let prices =
        List.filter_map rows ~f:(fun line ->
            ...
            | _ -> None)
      in
      List.sort prices ~compare:(fun (d1, _) (d2, _) -> String.compare d1 d2)
...
let _load_golden_breadth path =
  ...
  List.fold lines ~init:String.Map.empty ~f:(fun acc line ->
      ...
      | Some (date_str, count_str) -> (
          ...
          | Some count when count > 0 -> Map.set acc ~key:date_str ~data:count
          | _ -> acc)
      | None -> acc)
...
let _write_breadth_csv path pairs =
  Out_channel.with_file path ~f:(fun oc ->
      List.iter pairs ~f:(fun (date_str, count) ->
          let yyyymmdd = _format_date_yyyymmdd date_str in
...
let _compute_daily_changes ~min_stocks all_prices =
  (* Accumulate directions per date across all symbols. *)
  let tbl = Hashtbl.create (module String) in
  List.iter all_prices ~f:(fun prices ->
      let len = List.length prices in
      if len >= 2 then
        let _ =
          List.fold prices ~init:None ~f:(fun prev (date, close) ->
              (match prev with
              | Some prev_close when Float.( > ) close prev_close ->
                  let adv, dec, tot =
                    Hashtbl.find_or_add tbl date ~default:(fun () -> (0, 0, 0))
                  in
                  Hashtbl.set tbl ~key:date ~data:(adv + 1, dec, tot + 1)
              | Some prev_close when Float.( < ) close prev_close ->
                  let adv, dec, tot =
                    Hashtbl.find_or_add tbl date ~default:(fun () -> (0, 0, 0))
                  in
                  Hashtbl.set tbl ~key:date ~data:(adv, dec + 1, tot + 1)
              | Some _ ->
                  let adv, dec, tot =
                    Hashtbl.find_or_add tbl date ~default:(fun () -> (0, 0, 0))
                  in
                  Hashtbl.set tbl ~key:date ~data:(adv, dec, tot + 1)
              | None -> ());
              Some close)
        in
        ());
  (* Convert to sorted list, filtering by min_stocks. *)
  let entries =
  ...
  in
  List.sort entries ~compare:(fun (d1, _) (d2, _) -> String.compare d1 d2)
...
let _pearson_correlation xs ys =
  ...
  if n = 0 then 0.0
    ...
    let my = _mean ys in
    let cov =
      List.fold2_exn xs ys ~init:0.0 ~f:(fun acc x y ->
          acc +. ((x -. mx) *. (y -. my)))
    in
    let var_x =
      List.fold xs ~init:0.0 ~f:(fun acc x -> acc +. ((x -. mx) *. (x -. mx)))
    in
    let var_y =
      List.fold ys ~init:0.0 ~f:(fun acc y -> acc +. ((y -. my) *. (y -. my)))
    in
...
let _validate_against_golden ~label synthetic golden =
  let overlap_dates =
    Map.fold synthetic ~init:[] ~f:(fun ~key:date ~data:_ acc ->
        match Map.find golden date with Some _ -> date :: acc | None -> acc)
...
let main ~data_dir_str () =
  ...
  | `Yes ->
      ...
      in
      (* Net breadth correlation *)
      let overlap_dates =
        Map.fold syn_advn ~init:[] ~f:(fun ~key:date ~data:_ acc ->
            if
      ...
      let net_corr =
        if List.is_empty overlap_dates then 0.0
          ...
          let gold_net =
            List.map overlap_dates ~f:(fun d ->
                Float.of_int
                  (Map.find_exn golden_advn d - Map.find_exn golden_decln d))
          in
      ...
      if not (List.is_empty overlap_dates) then
