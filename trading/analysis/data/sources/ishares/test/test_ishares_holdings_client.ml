...
(* Pre-2012 quarterly responses ascend by market value. Pin the full source
   ordering by ticker so any future fixture or parser regression that
   reorders rows is caught deterministically — not just the "is it sorted"
   property, but the exact 9-row sequence. *)
let test_quarterly_era_preserves_ascending_market_value_order _ =
  let snap = _parsed_or_fail (_read _quarterly_fixture) in
    ...
    (is_some_and
       (all_of
          [
            ...
            field (fun h -> h.sector) (equal_to "Information Technology");
            field (fun h -> h.asset_class) (equal_to "Equity");
            field (fun h -> h.market_value) (float_equal 234567890.12);
            field (fun h -> h.weight_pct) (float_equal 4.85);
            field (fun h -> h.notional_value) (float_equal 234567890.12);
            ...
            field (fun h -> h.market_currency) (equal_to "USD");
            field (fun h -> h.accrual_date) (equal_to "-");
          ]))

(* Source order must be preserved — parser does not sort. Modern era is
   descending by market value with futures and cash hedges at end-of-data.
   Pin the full 9-row ticker sequence so any silent reorder is caught. *)
...
(* Unparseable as-of date — the [Fund Holdings as of] line is structurally
   present but the date cell is gibberish. The parser must fail loudly rather
   than fall through to a default date or skip the row. *)
let test_rejects_unparseable_as_of_date _ =
  let body =
    "iShares Russell 3000 ETF\n\
     Fund Holdings as of,\"not-a-date\"\n\n\
     Inception Date,\"May 22, 2000\"\n\n\
...
(* Wrong column count on a data row — header is canonical and as-of date is
   valid, but a data row has 14 cells instead of 15. The parser must reject
   rather than silently truncate / pad. *)
let test_rejects_wrong_column_count_row _ =
  let body =
    "iShares Russell 3000 ETF\n\
     Fund Holdings as of,\"Jun 01, 2020\"\n\n\
     Inception Date,\"May 22, 2000\"\n\n\
...
(* The URL pattern is the Phase 1.4 verified shape. The load-bearing checks
   are: host, IWV product id [239714] in the path, file name, and
   zero-padded YYYYMMDD asOfDate. *)
  ...
  assert_that uri
    (all_of
       [
         ...
         field Uri.path (contains_substring "/239714/");
         field
...
let suite =
  ...
  >::: [
         ...
         "quarterly_era_preserves_ascending_market_value_order"
         >:: test_quarterly_era_preserves_ascending_market_value_order;
         ...
         "rejects_unparseable_as_of_date"
         >:: test_rejects_unparseable_as_of_date;
         "rejects_wrong_column_count_row"
         >:: test_rejects_wrong_column_count_row;
