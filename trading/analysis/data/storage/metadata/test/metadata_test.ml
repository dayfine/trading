...
let create_test_price_data ~start_date ~end_date ~prices ~volumes =
  let rec generate_dates current acc =
    if Date.compare current end_date > 0 then List.rev acc
    else generate_dates (Date.add_days current 1) (current :: acc)
  in
  let dates = generate_dates start_date [] in
  List.map2_exn dates (List.zip_exn prices volumes)
    ~f:(fun date (price, volume) ->
      {
...
let test_low_prices _ =
  ...
  in
  let metadata = generate_metadata ~price_data ~symbol:"TEST" () in
  assert_bool "Should have average price below 10"
    metadata.last_n_prices_avg_below_10
...
let test_high_prices _ =
  ...
  in
  let metadata = generate_metadata ~price_data ~symbol:"TEST" () in
  assert_bool "Should have average price above 500"
    metadata.last_n_prices_avg_above_500
...
let test_custom_n _ =
  ...
  assert_equal ~printer:show_verification_status Verified
