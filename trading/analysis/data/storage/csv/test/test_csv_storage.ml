...
(* Each test allocates its own fresh data_dir under /tmp to ensure complete
   isolation — no shared state between tests regardless of dune sandbox
   behavior or test parallelism. *)
let _make_test_dir () =
  let dir = Stdlib.Filename.temp_dir ~temp_dir:"/tmp" "csv_test_data_" "" in
  Fpath.v dir

let _cleanup_test_dir data_dir =
  let _ : (unit, _) result = OS.Dir.delete ~recurse:true data_dir in
  ()

(* Run [f data_dir] with cleanup guaranteed via Fun.protect. *)
let _with_test_dir f =
  let data_dir = _make_test_dir () in
  Fun.protect
    ~finally:(fun () -> _cleanup_test_dir data_dir)
    (fun () -> f data_dir)
...
let test_save_and_get_prices _ =
  _with_test_dir (fun data_dir ->
      let symbol = "AAPL" in
      let storage = create ~data_dir symbol |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
      let expected_path = Fpath.(data_dir / "A" / "L" / "AAPL" / "data.csv") in
...
let test_single_char_symbol _ =
  _with_test_dir (fun data_dir ->
      ...
      let storage = create ~data_dir symbol |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
      let expected_path = Fpath.(data_dir / "X" / "X" / "X" / "data.csv") in
      assert_equal `Yes (Sys_unix.file_exists (Fpath.to_string expected_path));
      let retrieved_prices = get storage () |> ok_or_failwith_status in
      assert_equal
        ~printer:(fun ps ->
...
let test_create_twice _ =
  _with_test_dir (fun data_dir ->
      let symbol = "MSFT" in
      ignore (create ~data_dir symbol |> ok_or_failwith_status);
      ignore (create ~data_dir symbol |> ok_or_failwith_status))
...
let test_multiple_writes_non_overlapping _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "META" |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
...
let test_idempotent_writes _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "NFLX" |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
      ok_or_failwith_status (save storage prices);
      ok_or_failwith_status (save storage prices);
      let retrieved_prices = get storage () |> ok_or_failwith_status in
...
let test_reject_overlapping_contradictory_data _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "TSLA" |> ok_or_failwith_status in
      ...
      | Ok _ ->
          assert_failure
            "Expected validation error for overlapping dates with different \
             data"
      | Error status ->
          assert_equal ~printer:Status.show status
            (Status.invalid_argument_error
...
let test_allow_overlapping_with_override _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "AMZN" |> ok_or_failwith_status in
      let prices1 = List.take prices 2 in
      let prices2 =
        [
          {
...
let test_stream_start_date_only_filter _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "STREAM1" |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
...
let test_stream_header_only_returns_empty _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "STREAM3" |> ok_or_failwith_status in
      (* `save` with an empty list is a no-op (see
         test_empty_list_after_write_with_override), so write the header-only
         file by hand to exercise the header-only branch directly. `create` has
         already made the symbol directory. *)
      let path =
        Fpath.(data_dir / "S" / "3" / "STREAM3" / "data.csv") |> Fpath.to_string
      in
...
let test_save_and_get_with_active_through _ =
  _with_test_dir (fun data_dir ->
      let storage = create ~data_dir "DELIST" |> ok_or_failwith_status in
...
let test_read_legacy_7col_csv _ =
  _with_test_dir (fun data_dir ->
      ...
      let storage = create ~data_dir symbol |> ok_or_failwith_status in
      let path =
        Fpath.(data_dir / "L" / "Y" / "LEGACY" / "data.csv") |> Fpath.to_string
      in
      let oc = Stdlib.open_out path in
      Out_channel.output_string oc
...
let _manifest_path_for ~data_dir symbol =
  Csv.Csv_storage.shard_manifest_path ~data_dir symbol |> Fpath.to_string
...
let test_save_writes_manifest_entry _ =
  _with_test_dir (fun data_dir ->
      ...
      ok_or_failwith_status
        (save storage ~source:"EODHD" ~endpoint:"/eod/MAN1" ~fetch_id:"req-1"
           prices);
      let manifest =
        Manifest.read ~path:(_manifest_path_for ~data_dir symbol)
        |> ok_or_failwith_status
      in
      let csv_hash =
        Manifest.sha256_of_file ~path:(_csv_path_for ~data_dir symbol)
        |> ok_or_failwith_status
      in
      assert_that
...
let test_load_with_verify_strict_detects_tampering _ =
  _with_test_dir (fun data_dir ->
      ...
      let storage = create ~data_dir symbol |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
      _tamper_csv (_csv_path_for ~data_dir symbol);
      assert_that
        (load_with_verify storage ~strictness:`Strict ())
...
let test_load_with_verify_no_manifest_strict _ =
  _with_test_dir (fun data_dir ->
      ...
      let manifest_path = _manifest_path_for ~data_dir symbol in
...
(* Returns the reconcile-log directory path for a given data_dir. *)
let _reconcile_dir_of data_dir = Fpath.(data_dir / "_reconcile_log")

(* True iff a per-symbol reconcile entry has been written under any date
   shard below [recon_dir]. Used to assert presence/absence without baking
   in today's UTC date as a literal.

   Returns [None] if [recon_dir] is missing OR is a regular file (which
   happens transiently inside [test_reconcile_failure_is_non_fatal]'s
   failure-injection setup); readdir on a non-directory would raise. *)
let _reconcile_log_for ~recon_dir symbol =
  let recon_path = Fpath.to_string recon_dir in
  ...
  | `Yes ->
      ...
      List.find_map dates ~f:(fun d ->
          let p =
            Fpath.(recon_dir / d / (symbol ^ ".sexp")) |> Fpath.to_string
          in
...
let test_reconcile_no_op_when_content_unchanged _ =
  _with_test_dir (fun data_dir ->
      let recon_dir = _reconcile_dir_of data_dir in
      let symbol = "REC1" in
      let storage = create ~data_dir symbol |> ok_or_failwith_status in
      ok_or_failwith_status (save storage prices);
      ok_or_failwith_status (save storage ~override:true prices);
      assert_that (_reconcile_log_for ~recon_dir symbol) is_none)
...
let test_reconcile_writes_entry_when_content_changes _ =
  _with_test_dir (fun data_dir ->
      ...
      in
      let today_shard =
        Time_ns.to_date (Time_ns.now ()) ~zone:Time_float.Zone.utc
        |> Date.to_string
      in
      let log_path =
        Fpath.(recon_dir / today_shard / (symbol ^ ".sexp")) |> Fpath.to_string
      in
      ...
      assert_that entry
        (all_of
           [
             field
               (fun e -> e.Csv.Csv_storage_manifest.symbol)
               (equal_to symbol);
             field
...
(* The reconcile log is sharded by UTC date directory. After a
   content-changing refetch the date directory exists, contains the
   per-symbol sexp, and the directory's name parses as a valid date. *)
let test_reconcile_log_path_layout _ =
  _with_test_dir (fun data_dir ->
      ...
      let date_shards =
        match Sys_unix.is_directory (Fpath.to_string recon_dir) with
        | `Yes -> Sys_unix.readdir (Fpath.to_string recon_dir) |> Array.to_list
        | `No | `Unknown ->
            assert_failure
              (Printf.sprintf
                 "_reconcile_dir is missing or not a directory after save: %s"
                 (Fpath.to_string recon_dir))
      in
      ...
      let symbol_file =
        Fpath.(recon_dir / shard_name / (symbol ^ ".sexp")) |> Fpath.to_string
      in
      let exists =
...
let test_reconcile_failure_is_non_fatal _ =
  _with_test_dir (fun data_dir ->
      ...
      (* Plant a regular file where the reconcile-log directory would live so
         ...
         This file is inside data_dir and cleaned up automatically by
         _with_test_dir's Fun.protect finalizer. *)
      let recon_dir_str = Fpath.to_string recon_dir in
      let oc = Stdlib.open_out recon_dir_str in
