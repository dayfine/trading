...
let iter_entries entries =
  let total_wanted =
    List.fold entries ~init:0 ~f:(fun acc { Registry.csv_path; _ } ->
        match open_metadata_file ~csv_path with
        | Ok metadata ->
            let wanted = wanted metadata in
            if wanted then acc + 1 else acc
        | Error _ -> acc)
  in
  printf "Total wanted: %d\n" total_wanted

let show_entry t symbol =
  match Registry.get t ~symbol with
...
let () =
  ...
  Command.basic ~summary:"List CSV registry entries"
    [%map_open
      ...
      in
      fun () ->
        let registry = Registry.create ~csv_dir in
        match symbol with
        | Some sym -> show_entry registry sym
        | None ->
            (match n with
            | Some n -> List.take (Registry.list_all registry) n
            | None -> Registry.list_all registry)
            |> iter_entries]
  |> Command_unix.run
