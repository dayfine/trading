...
let parse_line line =
  ...
  | [date_str; open_str; high_str; low_str; close_str; adj_close_str; volume_str] ->
      ...
      with
      | Failure "Invalid date format, expected YYYY-MM-DD" ->
          Error (Invalid_date line)
      | Failure "float_of_string" ->
          Error (Invalid_number line)
      | Failure "int_of_string" ->
          Error (Invalid_volume line)
      | _ ->
          Error (Invalid_csv_format line)
  | _ ->
      Error (Invalid_csv_format ("Expected 7 columns, line: " ^ line))
