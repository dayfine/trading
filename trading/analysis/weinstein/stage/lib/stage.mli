...
(** Stage classifier for the Weinstein methodology.
    ...
    Classifies a stock into one of four stages based on weekly price bars
    and the relationship between price and its moving average.

    All functions are pure: same inputs always produce the same output. *)

(** Configuration for stage classification. All thresholds are configurable
    to enable parameter tuning via backtesting. *)
type config = {
  ma_period : int;
      (** Number of weeks for the moving average. Default: 30. *)
  ma_weighted : bool;
  ...
  confirm_weeks : int;
      (** Number of recent weeks used to determine whether price is
          consistently above or below the MA. Default: 6. *)
...
type result = {
  ...
  ma_value : float;
      (** Current MA value (last computed data point). *)
  ma_slope : Weinstein_types.ma_slope;
      (** Slope direction of the MA. *)
  ma_slope_pct : float;
...
(** [classify ~config ~bars ~prior_stage] classifies the current stage.
    ...
    @param bars Weekly price bars in chronological order (oldest first).
      Must contain at least [config.ma_period] bars for a valid result.
      Fewer bars returns the most recent inferable stage or [Stage1].
    @param prior_stage The stage from the previous week's classification.
      Used to disambiguate a flat MA (Stage 1 vs Stage 3).
      [None] on the first call — the classifier uses the long-term MA trend
