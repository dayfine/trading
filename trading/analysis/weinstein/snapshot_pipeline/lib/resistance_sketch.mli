(** Per-day resistance sketch columns for the Phase B snapshot pipeline
    ...
    For every daily index [i] this module computes weekly-cadence, point-in-time
    overhead-supply sketches from the same per-day weekly prefix the Stage/RS
    columns use ({!Weekly_prefix}): the weekly window at day [i] is the
    finalized weeks strictly before [i]'s week plus the partial week through day
    [i] — exactly the window the v1 resistance mapper sees at a Friday
    screening, so the sketch-derived virgin test [breakout >= max_high] is
    bit-equal to v1's [_is_virgin_territory] over the same span (v1: virgin iff
    no high STRICTLY exceeds the breakout, so the derived test must be [>=],
    preserving the [max_high = breakout] tie). Pinned against
    [Resistance.analyze] directly by the parity test in
    ...
    - [bars_seen].(i): true weekly-bar count available at day [i], capped at 520
      — the honest [Insufficient_history] input.
    - [hist].(cell).(i): {b age-banded} count at daily index [i]. The
      ...
      (age 0 = the partial current week, age 1 = the most recent finalized week,
      ...): [0-26w / 26-78w / 78-130w / 130-520w] (half-open). Within a band,
      [bucket] counts weekly bars whose mid-price [(high + low) / 2] lies in the
      log band \[C * 2^(bucket/20), C * 2^((bucket+1)/20)) above the day's raw
      close [C] and whose high exceeds [C]. Bucket indices past
    ...
    Corrupt-bar guard: when day [i]'s raw close is non-positive or non-finite,
    every sketch cell at [i] is [Float.nan].

    Cost: O(chart window) per day for the histogram (the anchor [C] moves daily,
    so the bucketing cannot be shared across days) and amortized O(1) per day
    for the rolling maxima (monotonic-deque sliding max over the finalized
    weekly highs). *)
