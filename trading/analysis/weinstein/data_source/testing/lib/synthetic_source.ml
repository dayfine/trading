...
(* Trading days in one calendar year (approximate). Used to size the pre-generation
   window so any practical query range is covered without unbounded allocation. *)
let _trading_days_per_year = 252

(* Pre-generate this many bars per symbol. Three years covers all realistic
   backtests and query windows. *)
let _max_gen_bars = _trading_days_per_year * 3
...
(** Dispatch pattern to its generator, producing [n] bars from [start_date]. *)
let _dispatch_pattern ~start_date pattern ~n =
  match pattern with
  | Trending { start_price; weekly_gain_pct; volume } ->
      _gen_trending ~start_date ~start_price ~weekly_gain_pct ~volume ~n
  | Basing { base_price; noise_pct; volume } ->
      _gen_basing ~start_date ~base_price ~noise_pct ~volume ~n
  | Breakout
      {
        base_price;
        base_weeks;
  ...
  | Declining { start_price; weekly_loss_pct; volume } ->
      _gen_declining ~start_date ~start_price ~weekly_loss_pct ~volume ~n
...
let _gen_bars ~(start_date : Date.t) ~(end_date : Date.t option) pattern :
  ...
  let raw = _dispatch_pattern ~start_date pattern ~n:_max_gen_bars in
